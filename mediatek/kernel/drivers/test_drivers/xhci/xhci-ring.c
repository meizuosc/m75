		...
		bool consumer, bool more_trbs_coming)
{
	...
	if (ring == xhci->event_ring)
		xhci_dbg(xhci, "Event ring enq = 0x%x, 0x%llx (DMA)\n", ring->enqueue, addr);
	else if (ring == xhci->cmd_ring)
		xhci_dbg(xhci, "Command ring enq = 0x%x, 0x%llx (DMA)\n", ring->enqueue, addr);
	else
		xhci_dbg(xhci, "Ring enq = 0x%x 0x%llx (DMA)\n", ring->enqueue, addr);
}
	...
	struct xhci_virt_ep *ep, int *status, bool skip)
{
	...
	xhci_dbg(xhci, "td is 0x%x, last_trb is 0x%x\n", td, td->last_trb ) ;
	...
	if (trb_comp_code == COMP_STOP_INVAL ||
		...
		if (trb_comp_code == COMP_STALL) {
			...
			xhci_dbg(xhci, "inc dequeue of ring: dequeue(0x%x), td(0x%x), last_trb(0x%x)\n", ep_ring->dequeue, td, td->last_trb);
...
#endif
	xhci_dbg(xhci,"td(0x%x), prev(0x%x), next(0x%x)\n", td, td->td_list.prev, td->td_list.next);
	...
	struct xhci_virt_ep *ep, int *status)
{
	...
	xhci_dbg(xhci, "td is 0x%x, last_trb is 0x%x, td->urb->status is %d\n", td, td->last_trb , td->urb->status) ;
...
//			port->port_id = 0;
			...
			if (g_otg_hnp_become_host){ //fast switch back to device ..
			    ...
			    //writel(readl(SSUSB_U2_CTRL(0)) &(~SSUSB_U2_PORT_HOST_SEL) , SSUSB_U2_CTRL(0));	
	...
	if((port_status & (USB_PORT_STAT_C_RESET << 16)) && (port_status & (USB_PORT_STAT_CONNECTION)) && !(port_status & USB_PORT_STAT_SUPER_SPEED)){
		...
		if(g_otg_hnp_become_host && g_otg_dev_B){  // recieve SSUSB_CHG_A_ROLE_A from U3D test driver
			if(!g_otg_wait_con){
		...
		struct xhci_transfer_event *event)
{
	...
	xhci_dbg(xhci, "deg_seg_dma(0x%x)trb(0x%x), dequeue(0x%x), td(0x%x), last_trb(0x%x), event_seg_dma(0x%x), event_dma(0x%x)trb(0x%x)\n",
            ep_ring->deq_seg->dma, ep_ring->deq_seg->trbs, ep_ring->dequeue, td, td->last_trb, event_seg->dma, event_dma, event_trb);
...
cleanup:
	...
	if (ret == FINISH_TD_CALL){
		td->urb->status = status;  // let the test driver can call the mtktest_xhci_urb_free_priv after whole dequeue is done
	}
...
int mtktest_xhci_handle_event(struct xhci_hcd *xhci){
	...
	// USBIF OTG
	// mtktest_xhci_handle_event(xhci);
	return 1;
}
 ...
 * Update interval as mtktest_xhci_queue_intr_tx does. Just use xhci frame_index to
...
//		remainder |
	...
	td->last_trb = ep_ring->enqueue;
	xhci_dbg(xhci, "set the td 0x%x 's last_trb 0x%x to 0x%x\n", td, td->last_trb , ep_ring->enqueue) ;	
