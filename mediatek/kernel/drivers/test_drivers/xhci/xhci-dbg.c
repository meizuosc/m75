...
void mtktest_xhci_debug_trb(struct xhci_hcd *xhci, union xhci_trb *trb)
{
	...
	case TRB_TYPE(TRB_LINK):
		...
		mtktest_xhci_print_trb_offsets(xhci, trb);

		address = trb->link.segment_ptr;
		xhci_dbg(xhci, "Next ring segment DMA address = 0x%llx(0x%x)\n", address, address);
	...
	case TRB_TYPE(TRB_TRANSFER):
		 ...
		 */
		xhci_dbg(xhci, "DMA address or buffer contents= %llu(0x%x)\n", address, address);
		break;
	case TRB_TYPE(TRB_COMPLETION):
		address = trb->event_cmd.cmd_trb;
		xhci_dbg(xhci, "Command TRB pointer = %llu(0x%x)\n", address, address);
...
void mtktest_xhci_dbg_ring_ptrs(struct xhci_hcd *xhci, struct xhci_ring *ring)
{
	xhci_dbg(xhci, "Ring deq = %p (virt), 0x%llx (dma)\n",
			ring->dequeue,
			(unsigned long long)mtktest_xhci_trb_virt_to_dma(ring->deq_seg,
							    ring->dequeue));
	xhci_dbg(xhci, "Ring deq updated %u times\n",
			ring->deq_updates);
	xhci_dbg(xhci, "Ring enq = %p (virt), 0x%llx (dma)\n",
			ring->enqueue,
			(unsigned long long)mtktest_xhci_trb_virt_to_dma(ring->enq_seg,
							    ring->enqueue));
...
void mtktest_xhci_debug_ring(struct xhci_hcd *xhci, struct xhci_ring *ring)
{
	...
	mtktest_xhci_debug_segment(xhci, first_seg);
	...
	for (seg = first_seg->next; seg != first_seg; seg = seg->next)
		mtktest_xhci_debug_segment(xhci, seg);
}
	...
	if (ep->ep_state & EP_HAS_STREAMS) {
		for (i = 1; i < ep->stream_info->num_streams; i++) {
			...
			xhci_dbg(xhci, "Dev %d endpoint %d stream ID %d:\n",
				slot_id, ep_index, i);
			mtktest_xhci_debug_segment(xhci, ring->deq_seg);
		}
	} else {
		ring = ep->ring;
		if (!ring)
...
{
	struct xhci_slot_ctx *slot_ctx = mtktest_xhci_get_slot_ctx(xhci, ctx);
...
void mtktest_mtk_xhci_dbg_slot_ctx(struct xhci_hcd *xhci, struct xhci_container_ctx *ctx)
{
...
void mtktest_mtk_xhci_dbg_ep_ctx(struct xhci_hcd *xhci,
		     struct xhci_container_ctx *ctx,
		     unsigned int last_ep)
{
	...
	for (i = 0; i < last_ep_ctx; ++i) {
		struct xhci_ep_ctx *ep_ctx = mtktest_xhci_get_ep_ctx(xhci, ctx, i);
		dma_addr_t dma = ctx->dma +
...
void mtktest_xhci_dbg_ep_ctx(struct xhci_hcd *xhci,
		     struct xhci_container_ctx *ctx,
		     unsigned int last_ep)
{
	int i, j;
		  ...
		  unsigned int last_ep)
{
	...
	if (ctx->type == XHCI_CTX_TYPE_INPUT) {
		struct xhci_input_control_ctx *ctrl_ctx =
			mtktest_xhci_get_input_control_ctx(xhci, ctx);
	...
	slot_ctx = mtktest_xhci_get_slot_ctx(xhci, ctx);
	mtktest_mtk_xhci_dbg_slot_ctx(xhci, ctx);
	mtktest_mtk_xhci_dbg_ep_ctx(xhci, ctx, last_ep);
}

void mtktest_xhci_dbg_ctx(struct xhci_hcd *xhci,
		  struct xhci_container_ctx *ctx,
		  unsigned int last_ep)
{
	int i;
	...
	slot_ctx = mtktest_xhci_get_slot_ctx(xhci, ctx);
	mtktest_xhci_dbg_slot_ctx(xhci, ctx);
	mtktest_xhci_dbg_ep_ctx(xhci, ctx, last_ep);
}
