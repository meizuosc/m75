...
#define XHC_IRQ 147
#define XHC_IO_START 0x11270000 /* = SSUSB_U3_PHYSICAL_BASE	0x11270000 */ //ICUSB IO_VIRT_TO_PHYS(USB3_BASE)
...
static int usb_hcd_versatile_probe(struct platform_device *pdev)
{
	...
	//	struct resource *res;
        ...
        printk(KERN_ERR "====%s(%d)==== hcd->pool[0] = 0x%x!\n", __func__, __LINE__, hcd->pool[0]);

		printk(KERN_ERR "Create xHC Base address 0x%x!\n", hcd->rsrc_start);
