...
int f_enable_slot(struct usb_device *dev){
	...
	if(dev == NULL){
		...
		printk("[OTG_H] rhdev = my_hcd->self.root_hub = 0x%x", rhdev) ;
		
		udev = rhdev->children[g_port_id-1];
		udev = mtk_usb_alloc_dev(rhdev, rhdev->bus, g_port_id);
		printk("[OTG_H] f_enable_slot => mtk_usb_alloc_dev udev = 0x%x", udev) ;
		udev->speed = port->port_speed;
		udev->level = rhdev->level + 1;
		rhdev->children[g_port_id-1] = udev;
		printk("[OTG_H] g_port_id %d, rhdev->children[g_port_id-1] = 0x%x", g_port_id, rhdev->children[g_port_id-1]) ;
	}
	else{
		udev = dev;
	}
...
, dma_addr_t dma_mapping, struct usb_device *usbdev){
	...
	xhci_dbg(xhci, "buffer(0x%x), dma(0x%x)\n", buffer, mapping);
...
int f_power_config_lpm(u32 slot_id, u32 hirdm, u32 L1_timeout, u32 rwe, u32 besl, u32 besld, u32 hle
	...
	printk("[OTG_H] read SSUSB_XHCI_HSCH_CFG2 address is 0x%x , value is 0x%x\n", addr, temp) ;
	//pdn
	...
	printk("[OTG_H] read PORTPMSC address is 0x%x", addr) ;
	...
	printk("[OTG_H] read port_lpm_ctrl_base address is 0x%x , value is 0x%x\n", addr, temp) ;
...
int f_host_test_mode(int case_num){
	...
	printk("USB2_PORT_SC(0x%x) is 0x%x\n", addr, temp);
	...
	case HOST_CMD_TEST_SE0_NAK:
		...
		printk("USB2_PORT_PMSC(0x%x) is 0x%x\n", addr, temp);
	...
	while(g_stopped == false){
		...
		if(g_otg_iddig != cur_iddig ){
			if(g_otg_iddig == 1){  //DEVICE mode
				...
				//turn on vbus
				printk("[OTG_H] iddig down, turn on vbus, g_otg_iddig %d, cur_iddig %d, OTG_STS: 0x%x\n", g_otg_iddig, cur_iddig);
#if 0				
