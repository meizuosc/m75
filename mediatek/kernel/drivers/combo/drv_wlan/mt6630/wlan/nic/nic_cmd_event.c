    ...
    )
{
    ...
    if (prCmdInfo->fgIsOid) {
        ...
        if(prEvent->u4Flags & BIT(0)) {
			...
			prLteSafeChnInfo->rLteSafeChnList.u4SafeChannelBitmask = prEvent->rLteSafeChn.u4SafeChannelBitmask;            
			DBGLOG(P2P, INFO,("[Query-info Auto Channel]LTE safe channels 0x%08x\n", prLteSafeChnInfo->rLteSafeChnList.u4SafeChannelBitmask));
        }
