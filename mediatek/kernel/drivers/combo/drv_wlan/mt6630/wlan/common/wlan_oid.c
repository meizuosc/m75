    ...
    *pu4SetInfoLen = 0;
    ...
    for(i = 0 ; i < prScanRequest->u4SsidNum ; i++) {
        if(prScanRequest->rSsid[i].u4SsidLen > ELEM_MAX_LEN_SSID){
            DBGLOG(REQ, ERROR, ("[%s] SSID(%s) Length(%ld) is over than ELEM_MAX_LEN_SSID(%d)\n",
                        __func__,
    ...
    )
{
    ...
    *pu4SetInfoLen = 0;
    ...
    if (prAdapter->fgEnArpFilter) {
        ...
        for (i = 0, u4IPv4AddrIdx = 0 ; i < prNetworkAddressList->u4AddressCount ; i++) {
            if (prNetworkAddress->u2AddressType == PARAM_PROTOCOL_ID_TCP_IP &&
                ...
                DBGLOG(INIT, INFO, ("%s: IPv4 Addr [%u]["IPV4STR"]\n", __FUNCTION__, u4IPv4AddrIdx, IPV4TOSTR(prNetworkAddress->aucAddress)));
    ...
    DBGLOG(INIT, INFO, ("%s: Set %lu IPv4 address for BSS[%u]\n", __FUNCTION__, u4IPv4AddrCount, prCmdNetworkAddressList->ucBssIndex));
    ...
    )
{
    ...
    if (!wextSrchDesiredWAPIIE((PUINT_8)pvSetBuffer,
            u4SetBufferLen,
            (PUINT_8 *)&prWapiInfo) )  {
        return WLAN_STATUS_INVALID_LENGTH;
    }
    ...
    )
{
    ...
    *pu4WritenLen = 0;
    ...
    if (!kalStrnCmp(head, BATCHING_SET, kalStrLen(BATCHING_SET))) {
        ...
        tokens = kalSScanf(head, "SCANFREQ=%ld MSCAN=%ld BESTN=%ld",
            &scanfreq, &mscan, &bestn);
        if (tokens != 3) {
