    ...
    )
{
	...
	if(meizu_target_ssid(prAdapter)){
		...
		//kalScanDone(prAdapter->prGlueInfo, 0, 0);
		cfg80211_scan_done(request,0);
		//aisFsmStateAbort_SCAN(prAdapter);
		printk(KERN_ERR "report scan result:%s\n",__func__);	
	}
    ...
    )
{
    ...
    prWifiVar->ucP2pGoVht = (UINT_8)wlanCfgGetUint32(prAdapter, "P2pGoVHT", FEATURE_DISABLED);
    
    prWifiVar->ucP2pGcHt = (UINT_8)wlanCfgGetUint32(prAdapter, "P2pGcHT", FEATURE_ENABLED);
    prWifiVar->ucP2pGcVht = (UINT_8)wlanCfgGetUint32(prAdapter, "P2pGcVHT", FEATURE_DISABLED);    
    ...
    prWifiVar->ucP2p5gBandwidth = (UINT_8)wlanCfgGetUint32(prAdapter, "P2p5gBw", MAX_BW_20MHZ);
    prWifiVar->ucApBandwidth = (UINT_8)wlanCfgGetUint32(prAdapter, "ApBw", MAX_BW_80MHZ);
