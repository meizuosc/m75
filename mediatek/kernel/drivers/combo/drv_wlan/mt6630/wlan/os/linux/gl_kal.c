    ...
    )
{
    ...
    if (IS_ERR(filp)) {
        pcFwName = "/system/etc/firmware/"CFG_FW_FILENAME"_MT6630";
        filp = filp_open(pcFwName, O_RDONLY, 0);
        // if we are in ubuntu, we need to look under also under /system/etc
        if (IS_ERR(filp)) {
            pcFwName = "/system/etc/firmware/"CFG_FW_FILENAME"_MT6630";
            filp = filp_open(pcFwName, O_RDONLY, 0);
        }
...
#define GENERAL_IOCTL 0 
#define NETWORK_IOCTL (GENERAL_IOCTL+1)
#define SECURITY_FRAME (NETWORK_IOCTL+1)
#define MANAGEMENT_FRAME (SECURITY_FRAME+1)

static inline cmdtype2string(int cmd)
{
	switch(cmd){
	case GENERAL_IOCTL :
		 return "GENERAL" ;
    ...
    )
{
    ...
    DBGLOG(INIT, INFO, ("%s: EN-Q CMD_TYPE[%u][%s]ID[0x%02X][%s] SEQ[%u] to CMD Q\n", 
        __FUNCTION__,
        prCmdInfo->eCmdType,cmdtype2string(prCmdInfo->eCmdType),
        prCmdInfo->ucCID,nicCmd2String(prCmdInfo->ucCID),
