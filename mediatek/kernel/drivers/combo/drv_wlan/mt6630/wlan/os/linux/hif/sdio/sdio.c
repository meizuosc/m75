    ...
    )
{
    ...
    do {
        ...
        *pu4Value = sdio_readl(prGlueInfo->rHifInfo.func, u4Register, &ret);
        ...
        if(ret || ucRetryCount) {
            DBGLOG(HAL, ERROR, ("sdio_readl() addr: 0x%08x value: 0x%08x status: %x retry: %u\n", 
                u4Register, *pu4Value, ret, ucRetryCount));
    ...
    if (ret) {
        kalSendAeeWarning(HIF_SDIO_ERR_TITLE_STR, HIF_SDIO_ERR_DESC_STR "sdio_readl() reports error: %x retry: %u", 
            ret, ucRetryCount);
    ...
    )
{
    ...
    do {
        ...
        if(ret || ucRetryCount) {
            DBGLOG(HAL, ERROR, ("sdio_writel() addr: 0x%08x status: %x retry: %u\n", u4Register, ret, ucRetryCount));
        }
