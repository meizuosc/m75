    ...
    )
{
	...
	printk(KERN_ERR"%s_%d:recive scan command from user space\n",__func__,__LINE__);
    ...
    if(prGlueInfo->prScanRequest != NULL || prGlueInfo->prSchedScanRequest != NULL) {
		printk(KERN_ERR"%s:pending scan/sched_scan not yet finished\n",__func__);
		return -EBUSY;
    }
    ...
    )
{
    ...
    printk(KERN_INFO"[wlan]mtk_cfg80211_connect\n");
    ...
    IN P_GLUE_INFO_T prGlueInfo)
{
    ...
    rStatus = kalIoctl(prGlueInfo,
            	 	...
            	 	&u4BufLen);
    u4TempSafeChannelBitmask = rQueryLTEChn.rLteSafeChnList.u4SafeChannelBitmask;
    DBGLOG(P2P, INFO,("   rLteSafeChnList.u4SafeChannelBitmask=%08x\n", rQueryLTEChn.rLteSafeChnList.u4SafeChannelBitmask));
#if 0
    if (fgIsPureAP)
    {
