...
/*----------------------------------------------------------------------------*/
BOOL
LoaderEntry(HINSTANCE hInstance,
            ULONG     Reason,
            LPVOID    pReserved)
{
    if (Reason == DLL_PROCESS_ATTACH) {
...
#define INSTANCE_NUMBER_SZ  10
    ...
    if (token != NULL) {
        ...
        SdDbgPrint(SDCARD_ZONE_ERROR,
            (TEXT("SDNDISLDR: Failed to get miniport name \n")));
        return FALSE;
    }
    ...
    SdDbgPrint(SDCARD_ZONE_INIT,
        (TEXT("SDNDISLDR: Miniport instance path %s \n"), stringBuff));
    ...
    if (RegSetValueEx(hKey,
        ...
        SdDbgPrint(SDCARD_ZONE_ERROR,
            (TEXT("SDNDISLDR: Failed to set ActiveKey path \n")));
        RegCloseKey(hKey);
        return FALSE;
    }
    ...
    SdDbgPrint(SDCARD_ZONE_INIT,
        (TEXT("SDNDISLDR: Miniport name path %s \n"), stringBuff));
    ...
    if (RegSetValueEx(hKey,
        ...
        SdDbgPrint(SDCARD_ZONE_ERROR,
            (TEXT("SDNDISLDR: Failed to set Group entry \n")));
        RegCloseKey(hKey);
        return FALSE;
    }
...
NDL_Init(DWORD dwContext)
{
    ...
    if (SDGetRegPathFromInitContext((PWCHAR)dwContext,
        ...
        SdDbgPrint(SDCARD_ZONE_ERROR,
            (TEXT("SDNDISLDR: Failed to get reg path \n")));
        LocalFree(pInstance);
        return 0;
    }
