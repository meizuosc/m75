...
SD_DEBUG_INSTANTIATE_ZONES(TEXT("SDIO NDIS Sample"), /*  module name */
                           (ZONE_ENABLE_INIT  |
                            ...
                            ZONE_ENABLE_WARN  |
                            ENABLE_ZONE_SEND  |
                            ENABLE_ZONE_SPECIAL),    /*  initial settings */
                           TEXT(""),
                           TEXT(""),
                           TEXT(""),
                           TEXT(""),
...
sdioLockPathru (
    IN P_GL_PATHRU_INFO_T prPathruInfo
    )
{
    ASSERT(prPathruInfo);
...
/*----------------------------------------------------------------------------*/
static VOID
sdioIndicateSlotStateChange (
    IN PSDCARD_HC_CONTEXT pHCContext,
    IN DWORD              SlotNumber,
    IN SD_SLOT_EVENT      Event,
    ...
    )
{
    ...
    //4 <1> Check if initialized
    if (FALSE == prPathruInfo->fgInitialized) {
        DBGLOG(INIT, WARN, ("SDNdis: enable/disable PATHRU mode but is not initialized \n"));
        goto exit_en_pathru;
    }
    ...
    if (FALSE != fgEnable) {
        ...
        fgResult = DeviceIoControl(prPathruInfo->hSHCDev,
                             ...
                             sizeof(prPathruInfo->rInfoOut),
                             &dwBytesReturned,
                             NULL);

        if (FALSE == fgResult) {
            DBGLOG(INIT, ERROR, ("SDNdis: DeviceIoControl() failed to enable SDHC PATHRU mode \n"));
            prPathruInfo->fgEnabled = FALSE;
        }
        else {
        ...
        if (FALSE == fgResult) {
            DBGLOG(INIT, ERROR, ("SDNdis: DeviceIoControl() failed to disable SDHC PATHRU mode \n"));
        }
    }
    ...
    )
{
    ...
    if (sdStatus == SD_API_STATUS_FAST_PATH_SUCCESS) {
        ...
        DBGLOG(HAL, ERROR, ("PATHRU+Fast-path: request handler do NOT return Fast-path success: 0x%08x\n", sdStatus));
    }
...
/*----------------------------------------------------------------------------*/
static __inline SD_API_STATUS
SDIO_CONNECT_INTERRUPT (
    IN P_GL_HIF_INFO_T  prHifInfo,
    IN PSD_INTERRUPT_CALLBACK pfnISTHandler
    )
{
    ASSERT(prHifInfo);
...
)
{
    ...
    status = SDGetTuple(pDevice->hDevice,
                        ucTupleCode,
                        NULL,
                        &length,
                        fgCommonCIS);
    ...
    if ((length == 0) || !SD_API_SUCCESS(status)) {
        DbgPrintZo(SDCARD_ZONE_ERROR,
            (TEXT("SDApiTest- SDGetTuple failed 0x%08X, length =%d \n, ucTupleCode = 0x%x"),
    ...
    )
{
    ...
    if (length == 0) {
        DbgPrintZo(SDCARD_ZONE_ERROR,
            (TEXT("SDApiTest- MANFID tuple is missing \n")));
        return FALSE;
    }
    ...
    if (length == 0) {
        DbgPrintZo(SDCARD_ZONE_ERROR,
            (TEXT("SDApiTest- CUSTOM tuple is missing \n")));
        return FALSE;
    }

    if (length != CUSTOM_TUPLE_BODY) {
        DbgPrintZo(SDCARD_ZONE_ERROR,
            (TEXT("SDApiTest- custom tuple reports size of %d bytes, should be %d \n"),
    ...
    )
{
    ...
    DbgPrintZo(SDCARD_ZONE_INIT,
        (TEXT("SDNdis: +SlotEventCallBack - %d \n"), SlotEventType));
    ...
    )
{
    ...
    if (NdisParameterString != pConfigParm->ParameterType) {
        DbgPrintZo(SDCARD_ZONE_ERROR,
            (TEXT("SDNdis: PARAMETER TYPE NOT STRING!!!\n")));
             /* close our registry configuration */
        NdisCloseConfiguration(configHandle);
        return status;
    }

    if (pConfigParm->ParameterData.StringData.Length >
            sizeof(prGlueInfo->rHifInfo.ActivePath)) {
        DbgPrintZo(SDCARD_ZONE_ERROR,
            (TEXT("SDNdis: Active path too long!\n")));
    ...
    status = prGlueInfo->rHifInfo.hDevice ?
    ...
    )
{
    ...
    case SD_BUS_WIDTH_REGISTRY_1BIT_MODE:
        ...
        break;
    default:
        DBGLOG(INIT, TRACE, ("MT6620 NDIS:registry use default mode  %d. ( 0:1bit, 1:4bits)\n",\
          cardInterface.InterfaceMode));
        break;
    }
    ...
    if (!SD_API_SUCCESS(status)) {
        DBGLOG(INIT, ERROR, ("MT6620 NDIS: Failed to query SDIO info ! 0x%08X  \n", status));
        return status;
    }
    ...
    status = SDCardInfoQuery(pDevice->hDevice,
                             ...
                             &sdHostBlockCap,
                             sizeof(sdHostBlockCap));

    DBGLOG(INIT, TRACE, ("MT6620 NDIS: SDIO host R_BlockSize %d W_BlockSize %d R_Blocks %d W_Blocks%d\n",\
    ...
    if (!SD_API_SUCCESS(status)) {
        ...
        status = SDSetCardFeature(pDevice->hDevice,
                                  ...
                                  0);
        if (!SD_API_SUCCESS(status)) {
            DBGLOG(INIT, TRACE, ("MT6620 NDIS: SD bus driver Fast-Path is available but failed to enable\n"));
        }
        else {
            DBGLOG(INIT, TRACE, ("MT6620 NDIS: SD bus driver Fast-Path is available and enabled\n"));
#if CFG_SDIO_PATHRU_MODE
...
/*----------------------------------------------------------------------------*/
NDIS_STATUS
windowsUnregisterIsrt (IN P_GLUE_INFO_T prGlueInfo)
{
    ...
    )
{
    ...
    if (pDevice->hDevice != NULL) {
        ...
        if (!SD_API_SUCCESS(sdStatus)) {
            DBGLOG(INIT, ERROR, ("SDNDIS: Failed to register client : 0x%08X \n", sdStatus));
            return NdisStatus;
        }
        ...
        if (!SD_API_SUCCESS(sdStatus)) {
            DBGLOG(INIT, ERROR, ("SDNDIS: sdioSetUpCardFeature return failed :0x%08X", sdStatus));
             return NdisStatus;
        }
...
#if CFG_SDIO_PATHRU_MODE
    status = SD_SYNC_BUS_REQ(prSdInfo,
                             ...
                             0);
#else
    status = SDSynchronousBusRequest(prSdInfo->hDevice,
                                     SD_CMD_IO_RW_EXTENDED,
                                     argument,
                                     SD_READ,
                                     ResponseR5,
...
/*----------------------------------------------------------------------------*/
BOOL
kalDevReadAfterWriteWithSdioCmd52 (
    IN P_GLUE_INFO_T    prGlueInfo,
    IN UINT_32          u4Addr,
    IN OUT PUINT_8      pucRwBuffer,
    IN UINT_32          u4RwBufLen
    )
{
    P_GL_HIF_INFO_T     prSdInfo = &prGlueInfo->rHifInfo;
    SD_API_STATUS       status = SD_API_STATUS_SUCCESS; /* intermediate status */
