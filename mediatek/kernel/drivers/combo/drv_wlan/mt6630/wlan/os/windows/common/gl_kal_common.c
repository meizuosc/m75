            ...
            if (_InterlockedAnd(&prGlueInfo->rHifInfo.u4ReqFlag, ~REQ_FLAG_OID) & REQ_FLAG_OID) {
                ...
                if (prGlueInfo->fgSetOid) {
                    ...
                    if(rStatus != WLAN_STATUS_PENDING) {
                        ...
                        NdisMSetInformationComplete(prGlueInfo->rMiniportAdapterHandle, rStatus);
                    ...
                    if(rStatus != WLAN_STATUS_PENDING) {
                        ...
                        NdisMQueryInformationComplete(prGlueInfo->rMiniportAdapterHandle, rStatus);
                        KeLowerIrql(rOldIrql);
                    }
                    else {
                        wlanoidTimeoutCheck(prGlueInfo->prAdapter, prEntry->pfOidSetHandler);
    ...
    )
{
    ...
    while (prNdisBuffer && u4ByteCount < ucLookAheadLen) {
        ...
        NdisQueryBufferSafe(prNdisBuffer,
                            &pvAddr,
                            &u4Len,
                            HighPagePriority);
#else
        ...
        NdisGetNextBuffer(prNdisBuffer,
                          &prNextNdisBuffer);
        ...
        *pfgIs802_3 = TRUE;
        ...
        if(ucDSAP == ETH_LLC_DSAP_SNAP &&
            ...
            *pfgIsPAL = TRUE;
            ...
            if((*(PUINT_8) &aucLookAheadBuf[ETH_SNAP_OFFSET + 3]) == ((BOW_PROTOCOL_ID_SECURITY_FRAME & 0xFF00) >> 8) &&
                    (*(PUINT_8) &aucLookAheadBuf[ETH_SNAP_OFFSET + 4]) == (BOW_PROTOCOL_ID_SECURITY_FRAME & 0xFF)) {
                *pfgIs1X = TRUE;
            }
        }
    }
    ...
    )
{
    ...
    NdisMInitializeTimer(&prGlueInfo->rMasterTimer,
            ...
            (PNDIS_TIMER_FUNCTION) kalTimerEvent,
            (PVOID) prGlueInfo);
}
    ...
    )
{
    ...
    RtlStringCbPrintfW (pucPath, u4size,
        L"\\SystemRoot\\dump_%ld_0x%08lX_%ld.hex", 
        fmt1,               
        fmt2,
        fmt3);
#else
    RtlStringCbPrintfW (pucPath, u4size,
        L"\\SystemRoot\\dump.hex");
...
UINT_32
kal_sprintf_done_ddk(
    const PUINT_8 pucPath,
    UINT_32		u4size
    )
{
    RtlStringCbPrintfW (pucPath, u4size,
        L"\\SystemRoot\\dump_done.txt");
