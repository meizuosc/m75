    ...
    )
{
    ...
    if (NdisAllocateMemoryWithTag(&pvAddr, u4Size, NIC_MEM_TAG) ==
        NDIS_STATUS_SUCCESS) {
        return pvAddr;
    }
    ...
    )
{
    ...
    prNdisAssocInfo->OffsetRequestIEs =
    ...
    )
{
    ...
    u4AvailableAssocRespIEBufLen = (sizeof(prGlueInfo->aucNdisAssocInfoIEs) >
        ...
        sizeof(prGlueInfo->aucNdisAssocInfoIEs) -
            prNdisAssocInfo->RequestIELength :
            0;
    ...
    if (u4FrameBodyLen) {
        ...
        kalMemCopy(
            &prGlueInfo->aucNdisAssocInfoIEs[prNdisAssocInfo->RequestIELength],
            cp,
            u4FrameBodyLen);
    ...
    )
{
    ...
    prPktExt->NdisPacketInfo[TcpIpChecksumPacketInfo] =
            (PVOID)rChecksumPktInfo.Value;
