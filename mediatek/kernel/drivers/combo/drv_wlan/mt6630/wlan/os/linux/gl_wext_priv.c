...
WLAN_STATUS
batchConvertResult(
    IN  P_EVENT_BATCH_RESULT_T prEventBatchResult,
    OUT PVOID       pvBuffer,
    ...
    )
{
    ...
    for (j=0; j<CFG_BATCH_MAX_MSCAN; j++) {
        ...
        for (i=0; i<pBr->ucScanCount; i++)
        {
            ...
            ssid[(prEntry->ucSSIDLen < (ELEM_MAX_LEN_SSID - 1) ? prEntry->ucSSIDLen : (ELEM_MAX_LEN_SSID - 1))] = '\0';
            nsize2 = kalSnprintf(text2, TMP_TEXT_LEN_L, "ssid=%s\n",
                ssid);

            freq = batchChannelNum2Freq(prEntry->ucFreq);
            nsize3 = kalSnprintf(text3, TMP_TEXT_LEN_L, "freq=%lu\nlevel=%d\ndist=%lu\ndistSd=%lu\n====\n", 
                freq,
                prEntry->cRssi,
                prEntry->u4Dist,
                prEntry->u4Distsd);
    ...
    case PRIV_CMD_POWER_MODE:
        {
            ...
            kalIoctl(prGlueInfo,
                        ...
                        &rPowerMode, /* pu4IntBuf[0] is used as input SubCmd */
    ...
    )
{
    ...
    if(i4Argc >= 3) {
        ...
        if(u4DbgIdx == 0xFFFFFFFF) {
            ...
            i4BytesWritten = snprintf(pcCommand, i4TotalLen, "Set ALL DBG module log level to [0x%02x]!", (UINT_8)u4DbgMask);
        }
            ...
            i4BytesWritten = snprintf(pcCommand, i4TotalLen, "Set DBG module[%lu] log level to [0x%02x]!", u4DbgIdx, (UINT_8)u4DbgMask);
        }
    }
    ...
    )
{
    ...
    if(i4Argc >= 2) {
        ...
        if(wlanGetDebugLevel(u4DbgIdx, &u4DbgMask) == WLAN_STATUS_SUCCESS) {
            fgIsCmdAccept = TRUE;
            i4BytesWritten = snprintf(pcCommand, i4TotalLen, "Get DBG module[%lu] log level => [0x%02x]!", u4DbgIdx, (UINT_8)u4DbgMask);
        }
    }
