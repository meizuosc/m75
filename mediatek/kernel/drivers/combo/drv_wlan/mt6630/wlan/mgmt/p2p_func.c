    ...
    )
{
    ...
    do {
        ...
        prScanReqV2->prSsid = (P_PARAM_SSID_T)((UINT_32)prScanReqV2 + sizeof(MSG_SCN_SCAN_REQ_V2));
    ...
    )
{
    do {
        ...
        bssDetermineApBssInfoPhyTypeSet(prAdapter, (prP2pConnReqInfo->eConnRequest == P2P_CONNECTION_TYPE_PURE_AP) ? TRUE:FALSE, prBssInfo);
    ...
    do {
                    ...
                    if (rsnParseCheckForWFAInfoElem(prAdapter, pucIEBuf, &ucOuiType, &u2SubTypeVersion)) {
                        ...
                        if (ucOuiType == VENDOR_OUI_TYPE_P2P) {
                            ...
                            if( (sizeof(prAdapter->prGlueInfo->prP2PInfo->aucWFDIE) 
                                ...
                                fgIsWFDIE = TRUE;
                                kalMemCopy(prAdapter->prGlueInfo->prP2PInfo->aucWFDIE, pucIEBuf, IE_SIZE(pucIEBuf));
                                prAdapter->prGlueInfo->prP2PInfo->u2WFDIELen += IE_SIZE(pucIEBuf);
                            }
                        } /*  VENDOR_OUI_TYPE_WFD */
#endif 
...
#if CFG_SUPPORT_WFD
        ASSERT(sizeof(prAdapter->prGlueInfo->prP2PInfo->aucWFDIE) >= prAdapter->prGlueInfo->prP2PInfo->u2WFDIELen);
        if (fgIsWFDIE) {
        ...
        if (fgIsWFDIE > 0) {
            ...
            kalMemCopy((PUINT_8)((UINT_32)prRetMsduInfo->prPacket + (UINT_32)prRetMsduInfo->u2FrameLength), 
                    prAdapter->prGlueInfo->prP2PInfo->aucWFDIE, prAdapter->prGlueInfo->prP2PInfo->u2WFDIELen);
