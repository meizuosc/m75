    ...
    )
{
    ...
    do {
        ...
        case P2P_ROLE_STATE_IDLE:
            ...
            break;
        case P2P_ROLE_STATE_GC_JOIN:
            DBGLOG(P2P, ERROR, ("Current P2P Role State P2P_ROLE_STATE_GC_JOIN is unexpected for FSM timeout event(wait SAA msg...).\n"));
            break;
        default:            
            ...
            if (prJoinCompMsg->ucSeqNum == prJoinInfo->ucSeqNumOfReqMsg) {
                ...
                if (prJoinCompMsg->rJoinStatus == WLAN_STATUS_SUCCESS) {
                    ...
                    if(prJoinInfo->prTargetBssDesc)					
                            scanReportBss2Cfg80211(prAdapter,OP_MODE_P2P_DEVICE,prJoinInfo->prTargetBssDesc);
                    kalP2PGCIndicateConnectionStatus(prAdapter->prGlueInfo,
                                                                    prP2pRoleFsmInfo->ucRoleIndex,
                                                                    &prP2pRoleFsmInfo->rConnReqInfo,
                                                                    prJoinInfo->aucIEBuf,
