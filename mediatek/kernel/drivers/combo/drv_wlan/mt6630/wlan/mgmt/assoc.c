    ...
    )
{
	...
	if(prStaRec->ucDTIMPeriod==1){
		u2ListenInterval = prStaRec->ucDTIMPeriod*MZ_LISTEN_INTERVAL ;
	}else if ((prStaRec->ucDTIMPeriod)&&(prStaRec->ucDTIMPeriod!=1)) {
        u2ListenInterval = prStaRec->ucDTIMPeriod * DEFAULT_LISTEN_INTERVAL_BY_DTIM_PERIOD;
    }else {
        DBGLOG(INIT, INFO, ("Use default listen interval\n"));
        u2ListenInterval = DEFAULT_LISTEN_INTERVAL;
    }
	
    prStaRec->u2ListenInterval = u2ListenInterval;
	DBGLOG(INIT,INFO,("Use listen_interval %d,DTIM:%d\n",u2ListenInterval,prStaRec->ucDTIMPeriod));
    ...
    )
{
    ...
    if (rsnCheckBipKeyInstalled(prAdapter, prStaRec)) {
        ...
        prDisassocFrame = (P_WLAN_DISASSOC_FRAME_T)((UINT_32)(prMsduInfo->prPacket) + MAC_TX_RESERVED_FIELD);
