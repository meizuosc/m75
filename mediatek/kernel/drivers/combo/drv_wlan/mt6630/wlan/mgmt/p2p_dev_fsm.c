...
{
    P_P2P_DEV_FSM_INFO_T prP2pDevFsmInfo = (P_P2P_DEV_FSM_INFO_T)u4Param;
        ...
        if (!LINK_IS_EMPTY(&prChnlReqInfo->rP2pChnlReqLink)) {
            ...
            P_MSG_P2P_CHNL_REQUEST_T prP2pMsgChnlReq = (P_MSG_P2P_CHNL_REQUEST_T)NULL;
                ...
                LINK_FOR_EACH(prLinkEntry, &prChnlReqInfo->rP2pChnlReqLink) {
                    prP2pMsgChnlReq = (P_MSG_P2P_CHNL_REQUEST_T)LINK_ENTRY(prLinkEntry, MSG_HDR_T, rLinkEntry);
                    ...
                    if (prP2pMsgChnlReq->eChnlReqType == CH_REQ_TYPE_P2P_LISTEN) {
                        ...
                        DBGLOG(P2P, TRACE, ("p2pDevFsmRunEventChannelAbort: Channel Abort, cookie found:%d\n", prChnlAbortMsg->u8Cookie));
                        fgIsChnlFound = TRUE;
                    break;
                }
            }
    ...
    )
{
    ...
    do {
        ...
        if ((prChnlAbortMsg->u8Cookie == prChnlReqInfo->u8Cookie) &&
            ...
            DBGLOG(P2P, WARN, ("p2pDevFsmRunEventChannelAbort: Channel Abort Fail, cookie not found:%d\n", prChnlAbortMsg->u8Cookie));
        }
    ...
    do {
        ...
        if ((!prMgmtTxMsg->fgIsOffChannel) || 
            ...
            if (prP2pDevFsmInfo->eCurrentState != P2P_DEV_STATE_OFF_CHNL_TX) {
                ...
                kalMemCopy(&prP2pMsgChnlReq->rChannelInfo, &prMgmtTxMsg->rChannelInfo, sizeof(RF_CHANNEL_INFO_T));
    ...
    )
{
    ...
    do {
        ...
        case OP_MODE_ACCESS_POINT:
            ASSERT(prP2pBssInfo->ucBssIndex < P2P_DEV_BSS_INDEX);
            p2pRoleFsmRunEventChnlGrant(prAdapter, prMsgHdr, P2P_ROLE_INDEX_2_ROLE_FSM_INFO(prAdapter, prP2pBssInfo->u4PrivateData));
            break;
        default:
            ASSERT(FALSE);
            break;
    ...
    )
{
    ...
    do {
        ...
        case OP_MODE_ACCESS_POINT:
            ASSERT(prP2pBssInfo->ucBssIndex < P2P_DEV_BSS_INDEX);
            p2pRoleFsmRunEventScanDone(prAdapter, prMsgHdr, P2P_ROLE_INDEX_2_ROLE_FSM_INFO(prAdapter, prP2pBssInfo->u4PrivateData));
            break;
        default:
            ASSERT(FALSE);
            break;
...
#if 0
TODO: use chip config more easy 
