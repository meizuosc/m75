    ...
    )
{
    ...
    if (!prStaRec || (prRxStatus->u2StatusFlag & RXS_DW2_RX_CLASSERR_BITMAP) == RXS_DW2_RX_CLASSERR_VALUE) {
        ...
        DBGLOG(RSN, TRACE, ("prStaRec=%x RX Status = %x RX_CLASSERR check!\n", prStaRec, prRxStatus->u2StatusFlag));
    ...
    )
{
    ...
    if (prAdapter->aprBssInfo[ucBssIndex]->fgTxBcKeyExist) {
        ...
        DBGLOG(RSN, INFO, ("[Wlan index] secGetBmcWlanIndex = %d\n", prAdapter->aprBssInfo[ucBssIndex]->ucBMCWlanIndex));
    ...
    )
{
...
#if 0
    if ((ucNetTypeIdx == NETWORK_TYPE_AIS) && (prAdapter->aprBssInfo[ucBssIndex]->eCurrentOPMode == OP_MODE_IBSS))  {
...
#if DBG
        if (i < 10) {
            DBGLOG(RSN, TRACE, ("idx=%d use=%d P=%d BSSIdx=%d Addr="MACSTR" keyid=%d\n",
                i, prWtbl[i].ucUsed, prWtbl[i].ucPairwise, prWtbl[i].ucBssIndex, MAC2STR(prWtbl[i].aucMacAddr), prWtbl[i].ucKeyId ));
        }
#endif        
        if (prWtbl[i].ucUsed && !prWtbl[i].ucPairwise && prWtbl[i].ucBssIndex == ucBssIndex && 
            1 /* (EQUAL_MAC_ADDR(prWtbl[i].aucMacAddr, pucAddr) || (prBSSInfo && EQUAL_MAC_ADDR(prWtbl[i].aucMacAddr, prBSSInfo->aucOwnMacAddr)))*/) {
            if (!fgCheckKeyId || prWtbl[i].ucKeyId == 0xff || (fgCheckKeyId && prWtbl[i].ucKeyId == ucKeyId)) {
                ucEntry = i;
                DBGLOG(RSN, TRACE, ("[Wlan index]: Reuse entry #%d\n", i));
                break;
            }
    ...
    if (ucEntry < WTBL_SIZE) {
        ...
        DBGLOG(RSN, TRACE, ("[Wlan index] BSS#%d keyid#%d P=%d use WlanIndex#%d STAIdx=%d "MACSTR" \n",
    ...
    )
{
    ...
    for (i = 0; i <= WTBL_SIZE; i++){
        DBGLOG(RSN, INFO, ("#%d Used=%d  BSSIdx=%d keyid=%d pairwise=%d STAIdx=%d"MACSTR"\n", i, 
            prWtbl[i].ucUsed, prWtbl[i].ucBssIndex, prWtbl[i].ucKeyId, prWtbl[i].ucPairwise, prWtbl[i].ucStaIndex, MAC2STR(prWtbl[i].aucMacAddr)));
    }

}

#if DBG
void secPrivacyDumpWTBL3(
    IN P_ADAPTER_T          prAdapter,
    IN UINT_8               ucIndex
    )
{
    ...
    if (ucIndex < WTBL_SIZE) {
        ...
        //for (i = 0; i <= WTBL_SIZE; i++){
            i = ucIndex;
            DBGLOG(RSN, INFO, ("#%d(%d) BSS=%d "MACSTR" RV=%d RKV=%d RCA2=%d ALG=%d RCID=%d keyid=%d RCA1=%d MUAR=%02x IKV=%d PN=%d\n", i, 
                prWtbl[i].ucUsed, prWtbl[i].ucBssIndex, MAC2STR(prWtbl[i].aucMacAddr),  
                prWtblCfg[i].fgRV, prWtblCfg[i].fgRKV,  prWtblCfg[i].fgRCA2, prWtblCfg[i].ucCipherSuit, 
                prWtblCfg[i].fgRCID, prWtbl[i].ucKeyId, prWtblCfg[i].fgRCA1, prWtblCfg[i].ucMUARIdx, 
                prWtblCfg[i].fgIKV, prWtblCfg[i].fgEvenPN));
        //}
    }
