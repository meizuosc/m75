    ...
    )
{
    ...
    if (prBssRsnInfo->u4PairwiseKeyCipherSuiteCount == 1 &&
        ...
        DBGLOG(RSN, TRACE, ("eEncStatus %d %lu 0x%lx\n", prAdapter->rWifiVar.rConnSettings.eEncStatus,
...
#if CFG_ENABLE_WIFI_DIRECT
        if (prAdapter->fgIsP2PRegistered && GET_BSS_INFO_BY_INDEX(prAdapter, ucBssIndex)->eNetworkType == NETWORK_TYPE_P2P)
        {
    ...
    )
{
        ...
        (prAdapter->rWifiVar.rConnSettings.eAuthMode == AUTH_MODE_WPA2_PSK))))
    {
        ...
        if (GET_BSS_INFO_BY_INDEX(prAdapter, ucBssIndex)->eNetworkType == NETWORK_TYPE_AIS && prAdapter->rWifiVar.rAisSpecificBssInfo.fgMgmtProtection) {
            if (kalGetMfpSetting(prAdapter->prGlueInfo) == RSN_AUTH_MFP_REQUIRED /* prAdapter->rWifiVar.rAisSpecificBssInfo.fgAPApplyPmfReq */) {
        ...
        if ((GET_BSS_INFO_BY_INDEX(prAdapter, ucBssIndex)->eNetworkType == NETWORK_TYPE_AIS) && 
            prAdapter->rWifiVar.rAisSpecificBssInfo.fgMgmtProtection &&
            (kalGetMfpSetting(prAdapter->prGlueInfo) != RSN_AUTH_MFP_DISABLED) /* (mgmt_group_cipher == WPA_CIPHER_AES_128_CMAC) */ ) {
            cp += 2;
            RSN_IE(pucBuffer)->ucLength += 2;
        }
#endif
...
{
    if (prStaRec && GET_BSS_INFO_BY_INDEX(prAdapter, prStaRec->ucBssIndex)->eNetworkType == (UINT_8)NETWORK_TYPE_AIS)
...
#if 1
        if (prAdapter->prAisBssInfo->eConnectionState == PARAM_MEDIA_STATE_CONNECTED /* STA_STATE_3 == prStaRec->ucStaState */) {
    ...
    )
{
    ...
    DBGLOG_MEM8(RSN, INFO, prRxFrame->ucTransId,
