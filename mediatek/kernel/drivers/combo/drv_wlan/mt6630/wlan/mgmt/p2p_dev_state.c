    ...
    )
{
    ...
    do {
        ...
        default:
            ...
            DBGLOG(P2P, ERROR, ("Unexpected State Transition(eNextState)\n",eNextState));
            ASSERT(FALSE);
            break;
        }
    } while (FALSE);
    ...
    )
{
    ...
    do {
        ...
        if (!LINK_IS_EMPTY(&(prP2pMgmtTxInfo->rP2pTxReqLink))) {
            prP2pOffChnlTxPkt = LINK_PEEK_HEAD(&(prP2pMgmtTxInfo->rP2pTxReqLink), P2P_OFF_CHNL_TX_REQ_INFO_T, rLinkEntry);
            ...
            if (prChnlReqInfo->ucReqChnlNum != prP2pOffChnlTxPkt->rChannelInfo.ucChannelNum) {
                ...
                LINK_REMOVE_HEAD(&(prP2pMgmtTxInfo->rP2pTxReqLink), prP2pOffChnlTxPkt, P_P2P_OFF_CHNL_TX_REQ_INFO_T);
