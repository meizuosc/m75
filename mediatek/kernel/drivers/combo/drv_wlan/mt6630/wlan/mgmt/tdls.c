	...
	)
{
	...
	switch(prCmd->ucActionCode) {
		...
		case TDLS_FRM_ACTION_SETUP_RSP:
            ...
            /* fix sigma bug 5.2.4.2, 5.2.4.7, we sent Status code decline, but the sigma recogniezis it as scucess, and it will fail */
...
/*----------------------------------------------------------------------------*/
WLAN_STATUS//TDLS_STATUS
TdlsDataFrameSend_DISCOVERY_REQ(
	ADAPTER_T							*prAdapter,
	STA_RECORD_T						*prStaRec,
	UINT_8								*pPeerMac,
	UINT_8								ucActionCode,
...
WLAN_STATUS
TdlsDataFrameSend_DISCOVERY_RSP(
	ADAPTER_T							*prAdapter,
	STA_RECORD_T						*prStaRec,
	UINT_8								*pPeerMac,
	UINT_8								ucActionCode,
	...
	)
{
	...
	if (u4TearDownSubId == TDLS_HOST_EVENT_TD_PTI_TIMEOUT)
	{
		...
		prStaRec->aucMacAddr, NL80211_TDLS_TEARDOWN, WLAN_REASON_TDLS_TEARDOWN_UNREACHABLE, GFP_ATOMIC);
	}	
