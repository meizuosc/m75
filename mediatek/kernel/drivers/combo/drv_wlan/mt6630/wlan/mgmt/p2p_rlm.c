    ...
    )
{
    ...
    if (RLM_AP_IS_BW_40_ALLOWED(prAdapter, prBssInfo)) {
        ...
        else if (cnmBss40mBwPermitted(prAdapter, prBssInfo->ucBssIndex)) {
    ...
    )
{
    ...
    if (prRxFrame->ucAction != ACTION_PUBLIC_20_40_COEXIST ||
        ...
        prSwRfb->prStaRec->ucBssIndex !=   //HIF_RX_HDR_GET_NETWORK_IDX(prSwRfb->prHifRxHdr) !=
           prStaRec->ucBssIndex) {
        return;
    }
