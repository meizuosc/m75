    ...
    )
{
    ...
    prAisBssInfo->ucBMCWlanIndex = WTBL_RESERVED_ENTRY; /* secGetBmcWlanIndex(prAdapter, NETWORK_TYPE_AIS, prAisBssInfo->ucBssIndex);*/
            ...
            if (prJoinCompMsg->ucSeqNum == prAisFsmInfo->ucSeqNumOfReqMsg) {
                ...
                if (prJoinCompMsg->rJoinStatus == WLAN_STATUS_SUCCESS) {
                    ...
                    if (prAisBssInfo->eConnectionState == PARAM_MEDIA_STATE_CONNECTED) {
                        ...
                        nicUpdateRSSI(prAdapter, prAdapter->prAisBssInfo->ucBssIndex, (INT_8)(RCPI_TO_dBm(prStaRec->ucRCPI)), 0);
                ...
                //4 <2> JOIN was not successful
                    ...
                    if (aisFsmStateInit_RetryJOIN(prAdapter, prStaRec) == FALSE) {
                        ...
                        else if(CHECK_FOR_TIMEOUT(rCurrentTime, prAisFsmInfo->rJoinReqTime, SEC_TO_SYSTIME(AIS_JOIN_TIMEOUT))) {
    ...
    )
{
    ...
    if (prAisBssInfo->eBand == BAND_2G4) {
        ...
        prAisBssInfo->ucPhyTypeSet =
            prAdapter->rWifiVar.ucAvailablePhyTypeSet & PHY_TYPE_SET_802_11ANAC; /* Depend on eBand */
