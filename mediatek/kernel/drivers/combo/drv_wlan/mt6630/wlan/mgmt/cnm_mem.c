    ...
    )
{
    ...
    DBGLOG(MEM, INFO, ("Alloc MSDU_INFO[0x%08x] by [%s]\n", prMsduInfo, pucStr));
...
{
    DBGLOG(MEM, INFO, ("Free MSDU_INFO[0x%08x] by [%s]\n", prMsduInfo, pucStr));
    ...
    )
{
	...
	*pu4SetInfoLen = sizeof(CMD_PEER_ADD_T);
	...
	if (prStaRec == NULL)
	{
		prStaRec = cnmStaRecAlloc(prAdapter, STA_TYPE_DLS_PEER  ,(UINT_8) prAdapter->prAisBssInfo->ucBssIndex, prCmd->aucPeerMac );
...
/*----------------------------------------------------------------------------*/
P_STA_RECORD_T
cnmGetTdlsPeerByAddress (
    P_ADAPTER_T     prAdapter,
    UINT_8          ucBssIndex,
    UINT_8          aucPeerMACAddress[]
