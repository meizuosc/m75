    ...
    )
{
    ...
    DBGLOG(SAA, INFO, ("Rx Deauth frame from BSSID["MACSTR"] DA["MACSTR"] ReasonCode[0x%x]\n",
        ...
        if (IS_STA_IN_AIS(prStaRec)) {
            ...
            if (prStaRec->ucStaState > STA_STATE_1) {
                ...
                if (authProcessRxDeauthFrame(prSwRfb,
                    ...
                    if (prAisSpecBssInfo->fgMgmtProtection && HAL_RX_STATUS_IS_CIPHER_MISMATCH(prSwRfb->prRxStatus) /* HAL_RX_STATUS_GET_SEC_MODE(prSwRfb->prRxStatus) != CIPHER_SUITE_BIP */)
                    {
                        prDeauthFrame = (P_WLAN_DEAUTH_FRAME_T) prSwRfb->pvHeader;
    ...
    )
{
    ...
    DBGLOG(SAA, INFO, ("Rx Disassoc frame from BSSID["MACSTR"] DA["MACSTR"] ReasonCode[0x%x]\n",
        ...
        if (IS_STA_IN_AIS(prStaRec)) {
            ...
            if (prStaRec->ucStaState > STA_STATE_1) {
                ...
                if (assocProcessRxDisassocFrame(prAdapter,
                    ...
                    if (IS_STA_IN_AIS(prStaRec) &&
                        prAisSpecBssInfo->fgMgmtProtection && HAL_RX_STATUS_IS_CIPHER_MISMATCH(prSwRfb->prRxStatus) /* HAL_RX_STATUS_GET_SEC_MODE(prSwRfb->prRxStatus) != CIPHER_SUITE_CCMP*/) 
                    {
                        //prDisassocFrame = (P_WLAN_DISASSOC_FRAME_T) prSwRfb->pvHeader;
