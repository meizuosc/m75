        ...
        if ((prBssDesc->fgIsConnected) &&             // P2P GC connected.
            ...
            for (u4Idx = 0; u4Idx < BSS_INFO_NUM; u4Idx++) {
                ...
                if ((prP2pBssInfo->eNetworkType != NETWORK_TYPE_P2P) || 
                        ...
                        (!EQUAL_SSID(prP2pBssInfo->aucSSID, prP2pBssInfo->ucSSIDLen, 
    ...
    do {
        ...
        LINK_FOR_EACH_ENTRY(prBssDesc, prBssDescList, rLinkEntry, BSS_DESC_T) {
            ...
            if (UNEQUAL_SSID(prConnReqInfo->rSsidStruct.aucSsid,
                ...
                /* No need trigger assert here; because SSID will be different with different WFD/DIRECT
                 * sessions; In normal case this code should not be executed beacause supplicant will
                 * guarantee that driver has scanned the target bss before trigger connection; One of another cases is
		 * that devices have the same MAC address.
                 **/		
                //ASSERT(FALSE);
                continue;
            }
