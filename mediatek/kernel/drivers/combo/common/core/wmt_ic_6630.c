...
static UCHAR WMT_SET_DAI_MODE_REG_CMD[] = {0x01, 0x08, 0x28, 0x00/*length*/
        , 0x01 /* op: w */
        , 0x01 /*type: reg */
        , 0x00 /*rev*/
        , 0x03 /*2 registers*/
...
static UCHAR WMT_SET_DAI_MODE_REG_EVT[] = {0x02, 0x08, 0x04, 0x00/*length*/
        , 0x00 /*S: 0*/
        , 0x00 /*type: reg */
        , 0x00 /*rev*/
        , 0x03 /*2 registers*/
...
static UCHAR WMT_COEX_EXT_COMPONENT_CMD[] = {0x01,0x10,0x03,0x00,0x0d,0x00,0x00};

static UCHAR WMT_COEX_FILTER_SPEC_CMD_TEST[] = {0x01,0x10,0x45,0x00,0x11,
					0x00,0x00,0x01,0x00,0x25,
					0x25,0x25,0x00,0x00,0x00,
					0x00,0x00,0x63,0x63,0x63,
					0x00,0x43,0x43,0x43,0x43,
					0x04,0x04,0x04,0x00,0x01,
					0x01,0x01,0x01,0x0e,0x0e,
					0x0e,0x00,0x0a,0x0a,0x0a,
...
static UCHAR WMT_COEX_LTE_FREQ_IDX_TABLE_CMD[] = {0x01,0x10,0x21,0x00,0x12,
					...
					0xF6,0x09,0x28,0x0a,0x5a,
					0x0a,0x14,0x09,0x2d,0x09,
					0x46,0x09,0x60,0x09,0xf5,
					0x09,0x27,0x0a,0x59,0x0a,
					0x8B,0x0a};
...
//          iRet = (*kal_stp_tx_raw)(&WMT_SET_WAKEUP_WAKE_CMD_RAW[0], 1, &u4Res);
	...
	ctrlData.au4CtrlData[1] = (UINT32)gp_mt6630_info->cChipVersion;
	ctrlData.au4CtrlData[2] = (UINT32)&gp_mt6630_patch_info;
...
static MTK_WCN_BOOL mt6630_trigger_stp_assert(VOID)
{
	INT32 iRet = -1;
	UINT32 u4Res = 0;
	UCHAR STP_DO_ASSERT_CMD[] = {0x80, 0x50, 0x0a, 0x00, 'd', 'o', 'c', 'o','r', 'e','d', 'u', 'm', 'p', 0x00, 0x00};
	
	iRet = wmt_core_tx((PUINT8) &STP_DO_ASSERT_CMD[0], sizeof(STP_DO_ASSERT_CMD), &u4Res, MTK_WCN_BOOL_TRUE);
...
)
{
    ...
    const UINT32 size = osal_array_size(mt6630_info_table);
    INT32 index;
    ...
    WMT_ERR_FUNC("MT6630: find no ic info for hw_ver(0x%x) by full match nor major num match!\n", hw_ver);
    return NULL;
}
...
mt6630_crystal_triming_set(VOID)
{
    ...
    if (0 != iRet) {
        ...
        WMT_DBG_FUNC("MT6630: nvram pBuf(0x%08x), bufLen(%d)\n", pbuf, bufLen);

        if (bufLen < (uCryTimOffset + 1)) {
            WMT_ERR_FUNC("MT6630: nvram len(%d) too short, crystalTimging value offset(%d)\n", bufLen, uCryTimOffset);
            bIsNvramExist = MTK_WCN_BOOL_FALSE;
            bIsCrysTrimEnabled = MTK_WCN_BOOL_FALSE;
            cCrystalTimingOffset = 0x0;
            cCrystalTiming = 0x0;
