...
static int _stp_dbg_dmp_in(MTKSTP_DBG_T *stp_dbg, char *buf, int len){
    ...
    if(internalFlag){
        ...
        memcpy(&(stp_dbg->logsys->queue[stp_dbg->logsys->in].buffer[0]),buf, in_len);
...
int stp_dbg_dmp_printk(MTKSTP_DBG_T *stp_dbg)
{
    ...
    while(dumpSize > 0){
		...
		#else
		printk("STP-DBG:%d.%ds, %s:pT(%d)n(%d)l(%d)s(%d)a(%d)\n", \
            pHdr->sec,
            pHdr->usec,
            pHdr->dir == PKT_DIR_TX ? "Tx" : "Rx",
            pHdr->type,
    ...
    )
{
	...
	if(hdr->dbg_type == STP_DBG_FW_DMP)
    {
    	...
    	{
    		unsigned int avl_num = stp_dbg_get_avl_entry_num(stp_dbg);
			if(!avl_num)
			{
...
INT32 stp_dbg_cpupcr_infor_format(UINT8 **buf, UINT32 *str_len)
{
    ...
    if ((STP_FW_NOACK_ISSUE == g_stp_dbg_cpupcr->issue_type) ||
        ...
        len += osal_sprintf(*buf + len, "%s\n\t\t</rc>\n\t</issue>\n\t", g_stp_dbg_cpupcr->assert_type);
        len += osal_sprintf(*buf + len, "<hint>\n\t\t");
		len += osal_sprintf(*buf + len, "<time_align>NULL</time_align>\n\t\t");
		len += osal_sprintf(*buf + len, "<assert_source>%s</assert_source>\n\t\t", \
			(0 == g_stp_dbg_cpupcr->host_assert_info.assert_from_host) ? "firmware" : "host");
		if (0 != g_stp_dbg_cpupcr->host_assert_info.assert_from_host) {
			len += osal_sprintf(*buf + len, "<host>\n\t\t\t");
			len += osal_sprintf(*buf + len, "<assert_sys_type>%s</assert_sys_type>\n\t\t\t", \
				gStpDbgType[g_stp_dbg_cpupcr->host_assert_info.drv_type]);
			len += osal_sprintf(*buf + len, "<reason_code>%d</reason_code>\n\t\t", \
				g_stp_dbg_cpupcr->host_assert_info.reason);
		}
		else
		{
			len += osal_sprintf(*buf + len, "<host> NULL ");
		}
		len += osal_sprintf(*buf + len, "</host>\n\t\t");
