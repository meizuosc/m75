...
static unsigned int gDbgLevel = ANT_LOG_DBG;
...
static int ANT_Power(long int ver);
/*
...
long ANT_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case COMBO_IOCTL_ANT_IC_HW_VER:
		ANT_INFO_FUNC("get hw version setup 1\n");
	...
	case COMBO_IOCTL_ANT_IC_FW_VER:		
		ANT_INFO_FUNC("get hw version setup 2\n");
...
static int ANT_DownLoad_RAM_Code(long int ver)
{
	struct file *pPatchExtFile = NULL;
	UCHAR pbPatchExtBin[read_ram_code_length];
	long lFileLen = 0;
