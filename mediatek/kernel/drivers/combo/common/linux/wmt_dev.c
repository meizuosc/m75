...
INT32 wmt_dev_tm_temp_query(void)
{
	...
	/*when wifi/bt/gps is off,dont query temp */
	if(!mtk_wcn_stp_is_ready()|| !wmt_any_function_on()){
	  WMT_INFO_FUNC("wmt stp is not ready\n");
	  return 40 ;/* */
	}
	...
	if(current_temp!=0xFF&&current_temp!=0){
