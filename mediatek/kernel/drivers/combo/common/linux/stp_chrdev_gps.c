...
#define GPS_DBG_FUNC(fmt, arg...)    if(gDbgLevel >= GPS_LOG_DBG){ pr_debug(PFX "%s: "  fmt, __FUNCTION__ ,##arg);}
...
long GPS_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        case 0: // enable/disable STP
            GPS_DBG_FUNC(KERN_INFO "GPS_ioctl(): disable STP control from GPS dev\n");
            retval = -EINVAL;
#if 1
#else
        ...
        case 1: // send raw data
            GPS_DBG_FUNC(KERN_INFO "GPS_ioctl(): disable raw data from GPS dev \n");
            retval = -EINVAL;
            break;
        ...
        case COMBO_IOC_GPS_HWVER:             
            ...
            GPS_DBG_FUNC(KERN_INFO "GPS_ioctl(): get hw version = %d, sizeof(hw_ver_sym) = %d\n", hw_ver_sym, sizeof(hw_ver_sym));
         ...
         case COMBO_IOC_GPS_IC_HW_VERSION:             
            ...
            GPS_DBG_FUNC(KERN_INFO "GPS_ioctl(): get hw version = 0x%x\n", hw_version);
		...
		case COMBO_IOC_GPS_IC_FW_VERSION:             
            ...
            GPS_DBG_FUNC(KERN_INFO "GPS_ioctl(): get fw version = 0x%x\n", fw_version);
        ...
        default:
            retval = -EFAULT;
            GPS_DBG_FUNC(KERN_INFO "GPS_ioctl(): unknown cmd (%d)\n", cmd);
            break;
    }
