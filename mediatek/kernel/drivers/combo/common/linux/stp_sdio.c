...
UINT32 gStpSdioDbgLvl = STPSDIO_LOG_WARN;
...
ssize_t stp_sdio_rxdbg_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
    ...
    for (i = 0; i < STP_SDIO_RXDBG_COUNT; ++i) {
        ...
        if (0 == len) {
            STPSDIO_INFO_FUNC("idx(0x%x) 0 == len dump skip\n", stp_sdio_rxdbg_cnt);
        }
        STPSDIO_INFO_FUNC("idx(0x%x) chisr_rxlen(%d) bus_rxlen(%d) ts(%d)\n",
        ...
        for (j = 0; j < STP_SDIO_RX_BUF_SIZE && j < len; j += 16) {
            ...
            STPSDIO_INFO_FUNC("[0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x   0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x]\n",
                pbuf[0], pbuf[1], pbuf[2], pbuf[3], pbuf[4], pbuf[5], pbuf[6], pbuf[7],
                pbuf[8], pbuf[9], pbuf[10], pbuf[11], pbuf[12], pbuf[13], pbuf[14], pbuf[15]);
            msleep(10);
        }
        STPSDIO_INFO_FUNC("dump ok\n");
    }
...
{
    ULONG len = count;
    STPSDIO_INFO_FUNC("write parameter len = %d\n\r", (int)len);
...
stp_sdio_rxdbg_setup(VOID)
{
    ...
    if (gStpSdioRxDbgEntry == NULL){
        STPSDIO_WARN_FUNC("Unable to create /proc entry\n\r");
        return -1;
    }
    ...
    STPSDIO_INFO_FUNC("Create /proc entry succeed\n\r");
#endif
...
static VOID stp_sdio_txperf_dump (VOID)
{
    ...
    STPSDIO_INFO_FUNC("txwait_fifo_left(%d), txwait_to_send(%d), txwait_count(%d)\n",
        fifo, data, cnt);
    if (cnt) {
        STPSDIO_INFO_FUNC("avg left(%d), to_send(%d)\n",
            (fifo / cnt), (data / cnt));
    }
    STPSDIO_INFO_FUNC("tx_worker_cnt(%d), pkt_num(%d), pkt_num_lmt_cnt(%d)\n",
        wkr, pkt_num, lmt_cnt);
