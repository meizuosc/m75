...
{
	trace_ccci_bm(skb_pool_4K.qlen, skb_pool_1_5K.qlen, skb_pool_16.qlen);
...
void ccci_cmpt_mem_dump(void *start_addr, int len)
{
	...
	if(NULL == curr_p) {
		printk(KERN_DEBUG "[CCCI-DUMP]NULL point to dump!\n");
		return;
	}
	if(0 == len){
		printk(KERN_DEBUG "[CCCI-DUMP]Not need to dump\n");
		return;
	}

	printk(KERN_DEBUG "[CCCI-DUMP]Base: %08x\n", (unsigned int)start_addr);
	// Fix section
	for(i=0; i<_32_fix_num; i++){
		printk(KERN_DEBUG "[CCCI-DUMP]%03X: %X %X %X %X %X %X %X %X\n", 
				i*32, *curr_p, *(curr_p+1), *(curr_p+2), *(curr_p+3),
	...
	if(tail_num > 0){
		...
		for(; j<32; j++)
			buf[j] = 0;
		curr_p = (unsigned int*)buf;
		printk(KERN_DEBUG "[CCCI-DUMP]%03X: %X %X %X %X\n", 
				i*32, *curr_p, *(curr_p+1), *(curr_p+2), *(curr_p+3),
				*(curr_p+4), *(curr_p+5), *(curr_p+6), *(curr_p+7));
	}
}
