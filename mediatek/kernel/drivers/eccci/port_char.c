...
#define __CCCI_INERNAL__
#include "port_ipc.h"

#ifdef FEATURE_GET_MD_BAT_VOL // must be after ccci_platform.h
...
#endif
static unsigned char critical_user_active[4]; // hardcode
...
#if 0
	CCCI_INF_MSG(md->index, CHAR, "command %X for EEMCS IT\n", cmd);
...
#define CCCI_IOC_CLR_HEADER             _IO(CCCI_IOC_MAGIC,  22)
	...
	case CCCI_IOC_SEND_RUN_TIME_DATA:
		if(ch == CCCI_MONITOR_CH) {
			ret = md->ops->send_runtime_data(md, md_sbp_code);
		} else {
