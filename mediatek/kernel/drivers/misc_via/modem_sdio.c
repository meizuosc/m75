	...
	switch(msg_id)
	{
		...
		case FLOW_CONTROL_MSG_ID:
			...
			if(chan_num > 0 && chan_num < (SDIO_TTY_NR + 1)){
				...
				LOGPRT(LOG_ERR,  "%s %d: unkown channel num%!\n", __func__, __LINE__, chan_num);
			}
			break;
...
static void modem_sdio_irq(struct sdio_func *func)
{
	...
	if(ret){
		LOGPRT(LOG_ERR,  "read iir err ret= %d\n", __func__,__LINE__,ret);
		goto err_out;
	}
	...
	if(ret){
		LOGPRT(LOG_ERR,  "read data cnt err ret= %d\n", __func__,__LINE__,ret);
		goto err_out;
	}
	...
	if(ret){
		LOGPRT(LOG_ERR,  "read data cnt ret= %d\n", __func__,__LINE__,ret);
		goto err_out;
	}
	bytecnt |= (reg << 8);
...
int modem_buffer_push(int port_num, void *buf, int count)
{
	...
	spin_unlock_irqrestore(&port->write_lock, flags);
    if(data_len < count) {
        LOGPRT(LOG_DEBUG, "%s: SDIO driver buffer is full!\n", __FUNCTION__,__LINE__);
        return -ENOMEM;
    } 
...
void modem_reset_handler(void)
{
	...
	for (index= 0; index< SDIO_TTY_NR; index++) {
		...
		LOGPRT(LOG_NOTICE,  "port%d rawbulk_unbind_sdio_channel before.\n", __func__, __LINE__, index);
		rawbulk_unbind_sdio_channel(index);
		LOGPRT(LOG_NOTICE,  "port%d rawbulk_unbind_sdio_channel after.\n", __func__, __LINE__, index);
	}
	
	spin_lock_irqsave(&modem_remove_lock, flags);
	modem_remove = 1;
	spin_unlock_irqrestore(&modem_remove_lock, flags);
	LOGPRT(LOG_NOTICE,  "cancel_work_sync(&dtr_work) before.\n", __func__, __LINE__);
	cancel_work_sync(&dtr_work);
	LOGPRT(LOG_NOTICE,  "cancel_work_sync(&dtr_work) after.\n", __func__, __LINE__);
	cancel_work_sync(&dcd_query_work);
	LOGPRT(LOG_NOTICE,  "cancel_work_sync(&dcd_query_work) after.\n", __func__, __LINE__);
	dcd_state = 0;
