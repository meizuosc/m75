...
#define mtk_cooler_kshutdown_dprintk(fmt, args...) \
  do { xlog_printk(ANDROID_LOG_DEBUG, "thermal/cooler/kshutdown", fmt, ##args); } while(0)
#else
#define mtk_cooler_kshutdown_dprintk(fmt, args...) 
#endif
...
static int 
mtk_cl_kshutdown_get_max_state(struct thermal_cooling_device *cdev,
                              unsigned long *state)
{        
  *state = 1;
...
static int 
mtk_cl_kshutdown_get_cur_state(struct thermal_cooling_device *cdev,
                              unsigned long *state)
{
...
static int 
mtk_cl_kshutdown_set_cur_state(struct thermal_cooling_device *cdev,
                              unsigned long state)
{
  ...
  *((unsigned long*) cdev->devdata) = state;
  ...
  if(1 == state)
  {
    mtk_cooler_kshutdown_dprintk("mtk_cl_kshutdown_set_cur_state() %s invokes machine_power_off\n", cdev->type);
    machine_power_off();
  }
