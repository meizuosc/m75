...
static ssize_t store_blink(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if(buf != NULL && size != 0)
	{
		LEDS_DRV_DEBUG("store_blink: size:%d,address:0x%s\n", size,buf);
		bb = simple_strtoul(buf,NULL,10);

		//led_switch_breath_pmic(led_data->cust.data, &nled_tmp_setting,1);

		mt_mt65xx_blink_set(led_cdev,&bb,&bb);
		//if(bb == 0)
		//{
		// mt65xx_led_set(led_cdev,led_cdev->brightness); 
		//}
		printk("[LED]%s: %d.\n",__func__,bb);
	}
...
static DEVICE_ATTR(blink, 0644, show_blink, store_blink);

/****************************************************************************
 * one-shot blink 
 bit0: 1: blink immedate 0:off
 bit1: 1:blink with touch panel 0:off
 ***************************************************************************/
static ssize_t store_tsp_blink(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned long bb = 0;
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	...
	if(buf != NULL && size != 0)
	{
		...
		bb = simple_strtoul(buf,NULL,10);
		if( (bb & 0x02 ) == 0x00)
			led_trigger_fadeonoff_func = NULL;
		else
			led_trigger_fadeonoff_func = mt_mt65xx_led_tigger_func;

		if( (bb & 0x01 ) == 0x01 )
			mt_mt65xx_led_tigger_func();
		printk("[LED]%s: 0x%.2X.\n",__func__,bb);
	}
...
static ssize_t show_tsp_blink(struct device *dev,struct device_attribute *attr, char *buf)
{
	return ((led_trigger_fadeonoff_func != NULL) ? 1 : 0);
}

static DEVICE_ATTR(tsp_blink, 0644, show_tsp_blink, store_tsp_blink);

#ifdef CONFIG_HAS_EARLYSUSPEND
static void mt65xx_leds_early_suspend(struct early_suspend *h)
{
...
static int mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
		...
		g_leds_data[i]->cdev.brightness = 0;
		...
		{
			g_leds_data[i]->cdev.brightness_set(&g_leds_data[i]->cdev,50);	
				...
				rc = device_create_file(g_leds_data[i]->cdev.dev, &dev_attr_tsp_blink);
				if(rc)
				{
				 LEDS_DRV_DEBUG("[LED]device_create_file tsp_blink fail!\n");
				}
		}

		
#ifdef CONFIG_HAS_EARLYSUSPEND
			 g_leds_data[i]->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN;
			 g_leds_data[i]->early_suspend.suspend = mt65xx_leds_early_suspend;
			 g_leds_data[i]->early_suspend.resume = mt65xx_leds_late_resume;
			 register_early_suspend(&g_leds_data[i]->early_suspend);
#endif
...
#ifdef CONFIG_HAS_EARLYSUSPEND
		unregister_early_suspend(&g_leds_data[i]->early_suspend);
#endif
		led_classdev_unregister(&g_leds_data[i]->cdev);
		cancel_work_sync(&g_leds_data[i]->work);
		kfree(g_leds_data[i]);
...
static int mt65xx_leds_resume(struct platform_device *pdev)
{
	int i;
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {

		if( g_leds_data[i] != NULL )
				g_leds_data[i]->cdev.brightness_set(&g_leds_data[i]->cdev,g_leds_data[i]->cdev.brightness);
		}
...
static int mt65xx_leds_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
		...
		if( g_leds_data[i] != NULL )
		{
				int brightness = g_leds_data[i]->cdev.brightness;
				g_leds_data[i]->cdev.brightness_set(&g_leds_data[i]->cdev,0);
				g_leds_data[i]->cdev.brightness = brightness;
		}
	}
