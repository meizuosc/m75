...
///BSP{@
void check_musb_mode(void *data)
{
	*(CABLE_MODE*)(data) = _mu3d_musb->usb_mode;
}
EXPORT_SYMBOL_GPL(check_musb_mode);
///@}
	...
	const char *buf, size_t count)
{
	...
	if (!dev) {
		...
		os_printk(K_INFO, "%s %s --> %s\n", __func__, usb_mode_str[cable_mode],
			cmode >= CABLE_MODE_MAX ? "<out-of-range>" : usb_mode_str[cmode]);
