...
void qmu_done_tx(struct musb *musb, u8 ep_num, unsigned long flags)
{
	...
	gpd_current = gpd_phys_to_virt((unsigned long)gpd_current, USB_TX, ep_num);
	...
	*/
	...
	if (gpd == gpd_current) {
		qmu_printk(K_DEBUG, "[TXD]""%s gpd(%p) == gpd_current(%p)\n", __func__, gpd, \
			gpd_current);
		return;
	}
	...
	while (gpd!=gpd_current && !TGPD_IS_FLAGS_HWO(gpd)) {
		...
		gpd = gpd_phys_to_virt((unsigned long)gpd, USB_TX, ep_num);
...
void qmu_done_rx(struct musb *musb, u8 ep_num, unsigned long flags)
{
	...
	/*Transfer PHY addr got from QMU register to VIR addr*/
	gpd_current = gpd_phys_to_virt((unsigned long)gpd_current, USB_RX, ep_num);
	...
	while(gpd != gpd_current && !TGPD_IS_FLAGS_HWO(gpd)) {
		...
		gpd = gpd_phys_to_virt((unsigned long)gpd, USB_RX, ep_num);
