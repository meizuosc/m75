...
void _ex_mu3d_hal_free_qmu_mem(void){
	DEV_UINT32 i ;
	...
	for ( i=1; i<=MAX_QMU_EP; i++) {
		...
		kfree(Tx_gpd_head[i]) ;
#else
		dma_free_coherent(NULL, size, Rx_gpd_head[i], Rx_gpd_ioptr[i]) ;
		dma_free_coherent(NULL, size, Tx_gpd_head[i], Tx_gpd_ioptr[i]) ;
#endif
	}
	qmu_printk(K_INFO, "_ex_mu3d_hal_free_qmu_mem -\n");
}

void _ex_mu3d_hal_alloc_qmu_mem(void){
	DEV_UINT32 i, size;
	TGPD *ptr,*io_ptr;
	dma_addr_t  dma_handle;
	...
	for ( i=1; i<=MAX_QMU_EP; i++) {
		...
		ptr = (TGPD*)dma_alloc_coherent(NULL, size, &dma_handle, GFP_KERNEL);
		memset(ptr, 0 , size);
		io_ptr = (TGPD *)(dma_handle);
		Rx_gpd_ioptr[i] = dma_handle ;
...
void _ex_mu3d_hal_flush_qmu(DEV_INT32 Q_num,  USB_DIR dir)
{
	...
	if (dir == USB_TX) {
		...
		if(!gpd_current) {
			...
			qmu_printk(K_DEBUG, "gpd_current(P) %p\n", gpd_current);
			gpd_current = gpd_phys_to_virt((unsigned long)gpd_current,USB_TX, Q_num);
			qmu_printk(K_DEBUG, "gpd_current(V) %p\n", (void *)gpd_current);
		}
		...
		if(!gpd_current) {
			...
			/*Switch physical to virtual address*/
			qmu_printk(K_DEBUG, "gpd_current(P) %p\n", gpd_current);
			gpd_current = gpd_phys_to_virt((unsigned long)gpd_current, USB_RX, Q_num);
			qmu_printk(K_DEBUG, "gpd_current(V) %p\n", (void *)gpd_current);
		}
...
void mu3d_hal_flush_qmu(DEV_INT32 Q_num,  USB_DIR dir)
{
	...
	os_printk(K_DEBUG,"%s flush QMU %s\n", __func__, ((dir==USB_TX)?"TX":"RX"));
