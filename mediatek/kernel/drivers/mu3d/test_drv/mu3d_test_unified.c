...
DEV_INT32 otg_wait_event(DEV_UINT8 mode)
{
	...
	do
	{
		...
		else if (mode == OTG_EVENT_CONNECT_OR_RESUME && (g_otg_connect || g_otg_resume || g_otg_disconnect || (u3d_get_vbus_mV()<700)))
		{
			if (g_otg_connect)
				os_printk(K_ERR, "otg_wait_event : g_otg_connect\n");
			if (g_otg_resume)
				os_printk(K_ERR, "otg_wait_event : g_otg_resume\n");
			if (g_otg_disconnect || (u3d_get_vbus_mV()<700))
			{
				g_otg_disconnect = 1 ;
		...
		else
		{
			...
			if (cnt <= 0)
			{
				...
				{
					os_printk(K_ERR, "otg_wait_event : == OTG_EVENT_CONNECT_OR_RESUME timeout ==\n");
