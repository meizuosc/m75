...
/* 6. GPU freq */
MT_DEBUG_ENTRY(gpu_freq);

static int mt_gpu_freq_show(struct seq_file *m, void *v)
{
    unsigned int level_count = 0;
    mtk_custom_get_gpu_freq_level_count(&level_count);
	SEQ_printf(m, "level_count:%d\n", level_count);
    return 0;
}

static ssize_t mt_gpu_freq_write(struct file *filp, const char *ubuf,
	   size_t cnt, loff_t *data)
{
    char buf[16];
    unsigned int level;
    ...
    if (cnt >= sizeof(buf)){
		printk("gpu_freq input stream size to large.\n");
		return -EINVAL;
	}
	...
	buf[cnt] = 0;
	//printk("gpu_freq input stream:%s\n", buf);

    if(sscanf(buf, "%u", &level) != 1)
        return -EINVAL;

    printk("gpu_freq level:%d\n", level);
    mtk_custom_boost_gpu_freq(level);
...
static int __init init_mtsched_prof(void)
{
    ...
    pe = proc_create("mtprof/gpu_freq", 0664, NULL, &mt_gpu_freq_fops);
    if (!pe)
	return -ENOMEM;
