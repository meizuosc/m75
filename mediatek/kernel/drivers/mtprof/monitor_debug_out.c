...
void mt_aee_dump_sched_traces(void)
{
    ...
    aee_wdt_printf("[ISR]last#%d,dur:%lld s:%lld\n\n",
        (int)b_isr->last_event, usec_high(b_isr->last_te - b_isr->last_ts) , usec_high(b_isr->last_ts)):
    aee_wdt_printf("[In ISR]Current irq#:%d, Start:%lld (elapsed: %lld), last irq#:%d, s:%lld, e:%lld\n\n",
        (int)b_isr->cur_event, usec_high(b_isr->cur_ts), usec_high(sched_clock() - b_isr->cur_ts), (int)b_isr->last_event, usec_high(b_isr->last_ts), usec_high(b_isr->last_te));

    b_sq->cur_event == 0?
    aee_wdt_printf("[Softirq]last#%d,dur:%lld s:%lld\n\n",
        (int)b_sq->last_event, usec_high(b_sq->last_te - b_sq->last_ts), usec_high(b_sq->last_ts)):
    aee_wdt_printf("[In Softirq]Current softirq#:%d, Start:%lld(elapsed: %lld), last softirq#:%d(dur:%lld), s:%lld, e:%lld\n\n",
        (int)b_sq->cur_event, usec_high(b_sq->cur_ts), usec_high(sched_clock() -b_sq->cur_ts), (int)b_sq->last_event, usec_high(b_sq->last_te - b_sq->last_ts) , usec_high(b_sq->last_ts), usec_high(b_sq->last_te));


    b_tk->cur_event == 0?
    aee_wdt_printf("[Tasklet]%d/SoftIRQ\n %pS dur:%lld s:%lld\n\n",
        (int)b_tk->last_count, (void *)b_tk->last_event, usec_high(b_tk->last_te - b_tk->last_ts), usec_high(b_tk->last_ts)):
    aee_wdt_printf("[In Tasklet]\n Occurs: cur:%d, last:%d\n Current:%pS, Start:%lld(elapsed: %lld), last#:%pS(dur:%lld), last_start:%lld, last_end:%lld\n\n",
        (int)b_tk->cur_count, (int)b_tk->last_count, (void *)b_tk->cur_event, usec_high(b_tk->cur_ts), usec_high(sched_clock() -b_tk->cur_ts), (void *)b_tk->last_event, usec_high(b_tk->last_te - b_tk->last_ts) ,usec_high(b_tk->last_ts), usec_high(b_tk->last_te));

    b_hrt->cur_event == 0?
    aee_wdt_printf("[HRTimer]%d/ISR\n %pS dur:%lld s:%lld\n\n",
        (int)b_hrt->last_count, (void *)b_hrt->last_event, usec_high(b_hrt->last_te - b_hrt->last_ts), usec_high(b_hrt->last_ts)):
    aee_wdt_printf("[In HRTimer]\n Occurs: cur:%d, last:%d\n Current:%pS, Start:%lld(elapsed: %lld), last#:%pS(dur:%lld), last_start:%lld, last_end:%lld\n\n",
        (int)b_tk->cur_count, (int)b_tk->last_count, (void *)b_hrt->cur_event, usec_high(b_hrt->cur_ts), usec_high(sched_clock() -b_hrt->cur_ts), (void *)b_hrt->last_event, usec_high(b_hrt->last_te - b_hrt->last_ts), usec_high(b_hrt->last_ts), usec_high(b_hrt->last_te));

    b_sft->cur_event == 0?
    aee_wdt_printf("[SoftTimer]%d/SoftIRQ\n %pS dur:%lld s:%lld\n\n",
        (int)b_sft->last_count, (void *)b_sft->last_event, usec_high(b_sft->last_te - b_sft->last_ts), usec_high(b_sft->last_ts)):
    aee_wdt_printf("[In SoftTimer]\n Occurs: cur:%d, last:%d\n Current:%pS, Start:%lld(elapsed: %lld), last#:%pS(dur:%lld), last_start:%lld, last_end:%lld\n\n",
        (int)b_sft->cur_count, (int)b_sft->last_count, (void *)b_sft->cur_event, usec_high(b_sft->cur_ts), usec_high(sched_clock() -b_sft->cur_ts), (void *)b_sft->last_event, usec_high(b_sft->last_te - b_sft->last_ts), usec_high(b_sft->last_ts), usec_high(b_sft->last_te));
...
void mt_show_current_irq_counts(void)
{
    ...
    for(cpu = 0; cpu < num_possible_cpus(); cpu++){
        ...
        for(irq = 0; irq< NR_IRQS; irq++){
            ...
            if(count != 0)
                printk(" IRQ[%3d:%14s] = %8d, (+%d times in %lld us) \n", irq, isr_name(irq), count, count - mt_irq_count[cpu][irq], usec_high(t_diff));
        }
    }
#ifdef CONFIG_SMP
    for(cpu = 0; cpu < num_possible_cpus(); cpu++){
    ...
    for(eint_num =0 ;eint_num < eint_drv_get_max_channel(); eint_num++){
        ...
        if(eint_drv_get_count(eint_num) == 0)
            continue;
        printk("EINT[%d] = %8d, (+%d times in %lld us)", eint_num, eint_drv_get_count(eint_num), eint_drv_get_count(eint_num) - mt_eint_count[eint_num], usec_high(t_diff));
    }
}
void mt_show_timer_info(void)
{
...
static void mt_aee_show_current_irq_counts(void)
{
    ...
    t_diff = t_cur - mt_save_irq_count_time;
    aee_wdt_printf("\nIRQ Status\n");
    aee_wdt_printf("Dur:%lld us,(now:%lld,last:%lld)\n", usec_high(t_diff), usec_high(t_cur), mt_save_irq_count_time);
    for(cpu = 0; cpu < num_possible_cpus(); cpu++){
        aee_wdt_printf(" CPU%d\n", cpu);
        for(irq = 0; irq< NR_IRQS; irq++){
            count = kstat_irqs_cpu(irq, cpu);
            if(count != 0)
                aee_wdt_printf(" %d:%s +%d(%d)\n", irq, isr_name(irq), count - mt_irq_count[cpu][irq], count);
        }
    }
#ifdef CONFIG_SMP
    for(cpu = 0; cpu < num_possible_cpus(); cpu++){
        ...
        for(irq = 0; irq< NR_IPI; irq++){
            count =  __get_irq_stat(cpu, ipi_irqs[irq]);
            if(count != 0)
                aee_wdt_printf(" %d:IPI +%d(%d)\n", irq, count - mt_local_irq_count[cpu][irq], count);
        }
    }
#endif
    for(eint_num =0 ;eint_num < eint_drv_get_max_channel(); eint_num++){
        ...
        if(eint_drv_get_count(eint_num) == 0)
            continue;
        aee_wdt_printf("EINT[%d] = %8d, (+%d times in %lld us)", eint_num, eint_drv_get_count(eint_num), eint_drv_get_count(eint_num) - mt_eint_count[eint_num], usec_high(t_diff));
    }
    //spin_unlock_irqrestore(&mt_irq_count_lock, flags);
}
static void mt_aee_show_timer_info(void)
