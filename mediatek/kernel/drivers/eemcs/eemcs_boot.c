...
static KAL_INT32 eemcs_boot_cmd_process(void)
{
    ...
    do {
        //result = wait_event_interruptible(boot_node->rx_waitq, (atomic_read(&boot_node->rx_pkt_cnt) > 0) || time_out_abort);
        wait_event(boot_node->rx_waitq, (atomic_read(&boot_node->rx_pkt_cnt) > 0) || time_out_abort);
        
        //if (result == -ERESTARTSYS) {
        //    DBGLOG(BOOT, ERR, "[RX]PORT0 Interrupted by syscall.signal=0x%llx,try again", *(long long *)current->pending.signal.sig);
        //    goto _exit;
        //} else 
        if (time_out_abort) {
