...
#ifdef KEY_HALL_DEBUG
static ssize_t key_hall_press_show(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
		printk("[key hall]buf is %s and size is %d \n",buf,size);
		return sprintf(buf,"%d\n",key_hall_press);
}

static DEVICE_ATTR(key_hall_state, 0664, key_hall_press_show, NULL);
#endif
...
{
	unsigned char pressed = RELEASED,eint_pol_state = RISING,pol,input_value = 0xff;
	unsigned long flags;
	unsigned int code;
...
//	pol = mt_eint_get_polarity(bdata->button->irq);
	...
	if( bdata->irq != CUST_EINT_HALL_INT_NUM ) {
...
//	mt_eint_set_polarity(bdata->button->irq, eint_pol_state);
//	mt_eint_unmask(bdata->button->irq);
	pr_debug("@@@@@@@@@@@@@@@@@@@@@@ %s--%s gpio_in:%d\n",
			bdata->button->desc,(atomic_read(&bdata->key_pressed) == PRESSED)?"Pressed":"Released",input_value);
	mt_eint_unmask(bdata->button->irq);
}

static void gpio_keys_gpio_delayed_work_func(struct delayed_work *delayed_work)
{
	struct gpio_button_data *bdata =
		container_of(delayed_work, struct gpio_button_data, delayed_work);
	...
	pr_debug("&&&&&&&&&&&&&&&&&& %s---%s gpio_in:%d pol:%s\n",bdata->button->desc,atomic_read(&bdata->key_pressed)==PRESSED?"Press":"Release",gpio_in,pol==RISING?"Rise":"Fall");
...
static void gpio_keys_gpio_report_event(struct gpio_button_data *bdata)
{
	unsigned char pressed = RELEASED,state_prev = RELEASED, eint_pol_state = RISING,pol,input_value = 0xff;
	unsigned long flags;
	...
	if( pressed==state_prev ) {
		//eint_pol_state = (state_prev == RELEASED)?FALLING:RISING;
		//mt_eint_set_polarity(bdata->button->irq, eint_pol_state);

		pr_debug("@@@@@@@@@ ERROR @@@@@@@@@ %s--prev-%s gpio_in_now:%d\n",
				bdata->button->desc,(state_prev == PRESSED)?"Pressed":"Released",input_value);
	}
...
//	printk("@@@@@@@@@@@@@@@@@@@@@@ %s---%s\n",bdata->button->desc,(atomic_read(&bdata->key_pressed) == PRESSED)?"Released":"Pressed");
...
{
	/*
	 * Useless for mt6595, change to mediatek/kernel/drivers/keypad/volume_keys.c
	 * kpd_voldown_eint_interrupt_handler() and kpd_volup_eint_interrupt_handler()
	 */
	return IRQ_HANDLED;
}
					 ...
					 const struct gpio_keys_button *button)
{
	...
	if(bdata->irq != CUST_EINT_HALL_INT_NUM) {
		...
		atomic_set(&bdata->key_pressed, REMOVED); 
#ifdef KEY_HALL_DEBUG
		key_hall_press = atomic_read(&bdata->key_pressed);
#endif
	}  
...
static int  gpio_keys_probe(struct platform_device *pdev)
{
	 ...
	 * Register gpio irq
	 */
	external_kpd_eint_init();
	pr_debug("[gpio_keys_probe] ok!\n");
	return 0;
...
static int gpio_keys_resume(struct device *dev)
{
	...
	for (i = 0; i < ddata->n_buttons; i++) {
		...
		if (bdata->button->gpio) {
			...
			// do nothing
#endif
			pr_debug("@@@@@@@@@@@@@@@@@@@@@@ %s--do nothing check_status:%s\n",
...
{
	/*
	 * mediatek/kernel/drivers/keypad/board-m75-input.c
	 *
	 */
	mtk_external_input_init();
	/*
	 * mediatek/kernel/drivers/keypad/volume_keys.c
	 *
	 */
	/*
	 * external_kpd_eint_init should be loaded after module gpio_keys_init,
