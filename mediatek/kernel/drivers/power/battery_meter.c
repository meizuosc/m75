...
///MEIZU_BSP{@
/* M75 will Support SDI and SONY battery, SDI:0; SONY:1*/
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
kal_int32 g_Q_MAX_POS_50[] = {2808, 2807};
kal_int32 g_Q_MAX_POS_25[] = {2822, 2838};
kal_int32 g_Q_MAX_POS_0[] = {2515, 2664};
kal_int32 g_Q_MAX_NEG_10[] = {1750, 2505};

kal_int32 g_Q_MAX_POS_50_H_CURRENT[] = {3023, 3021};
kal_int32 g_Q_MAX_POS_25_H_CURRENT[] = {3000, 3047};
kal_int32 g_Q_MAX_POS_0_H_CURRENT[] = {2163, 2752};
kal_int32 g_Q_MAX_NEG_10_H_CURRENT[] = {847, 2197};
#endif
///@}
...
{
 /*
       SONY BATTERY THE BADID IS 1, PULL DOWN RESISTOR 100K;
       SDI BATTERY BATID IS 0, PULL DOWN RESISTOR 0K
  */
    g_fg_battery_id = mt_get_gpio_in(GPIO_BATTER_ID_PIN);
}
#else
void fgauge_get_profile_id(void)
{
...
void fg_qmax_update_for_aging(void)
{
    ...
    if(hw_charging_done == KAL_TRUE) // charging full, g_HW_Charging_Done == 1
    {
	    ...
	    /* MEIZU_BSP modify by txy 20141016 
	     modify the capacity change quickly problem*/
            gFG_BATT_CAPACITY_aging -= 150;
...
static void battery_aging_check(void)
{
	...
	suspend_time += rtc_time_after_sleep.tv_sec - g_rtc_time_before_sleep.tv_sec;
#if defined (MD_SLEEP_CURRENT_CHECK)
	...
	if (hw_ocv_after_sleep != g_hw_ocv_before_sleep) {
		if(suspend_time > OCV_RECOVER_TIME) {   //35 mins
			suspend_current = (columb_after_sleep - columb_before_sleep) * 3600 / suspend_time;
	...
	if(hw_ocv_after_sleep < vbat) {
		bm_print(BM_LOG_CRTI, "Ignore HW_OCV : smaller than VBAT\n");
		 ...
		 if(suspend_time > OCV_RECOVER_TIME) {	 //30 mins
...
{
    platform_device_unregister(&battery_meter_device);
    platform_driver_unregister(&battery_meter_driver);
}
