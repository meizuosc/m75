...
INT32	temperature_set;
static int battery_charger_dis = 0;
...
#define BATON_ABSENT_ADC 1000
#define BAT_VOLTAGE_DIFF 145

/******************charger type notifier ***********************/
#ifdef CONFIG_MUIC_TSU6721
extern int tsu6721_notifier_chain_register(struct notifier_block *nb);
extern void get_charger_device_type(void *data);
#else
static int tsu6721_notifier_chain_register(struct notifier_block *nb) {return 0;};
extern void get_charger_device_type(void *data) {return 0;};
#endif 
...
//MEIZU_BSP {
/*add by tangxingyan 2014/09/03*/
static int battery_update_time = BAT_TASK_PERIOD;
//}
    ...
    union power_supply_propval *val)
{
	...
	/*SONY Battery BATID is 1, pulldown resistor 100K
	 SDI Battery BATID is 0, pulldown resistor 0K*/
	battery_id = mt_get_gpio_in(GPIO_BATTER_ID_PIN);
	if (battery_id == 1) 
		val->strval = "SONY";
	else
		val->strval = "SDI";
...
static void battery_update(struct battery_data *bat_data)
{
	...
	/* Transient voltage compared with the average voltage */
...
static void ac_update(struct ac_data *ac_data)
{
    ...
    {         
        ...
        {
			pmic_read_interface(0x8060, &val, 0xFFFF, 0x0);
			if (val & 0x4000){
				ac_data->AC_ONLINE = 0;
			} else {
				ac_data->AC_ONLINE = 1;
...
PMU_STATUS do_batt_temp_state_machine(void)
{
	...
	if (BMT_status.temperature <= MIN_CHARGE_TEMPERATURE)
	{
		...
		BMT_status.batt_health = POWER_SUPPLY_HEALTH_COLD;
		return PMU_STATUS_FAIL;       
	}
	...
	if (BMT_status.temperature >= MAX_CHARGE_TEMPERATURE) 
	{
		...
		BMT_status.batt_health = POWER_SUPPLY_HEALTH_OVERHEAT;
		return PMU_STATUS_FAIL; 
	}
	else if(g_batt_temp_status == TEMP_POS_HIGH)
	{
		if (BMT_status.temperature <= MAX_CHARGE_TEMPERATURE_MINUS_X_DEGREE) {             
			printk("[BATTERY] Battery Temperature down from %d to %d(%d), allow charging!!\n\r", 
	...
	if(BMT_status.charger_exist == KAL_TRUE)
	{
		if(previous_charger_exist == KAL_FALSE)
		{
			...
			if (BMT_status.charger_type == STANDARD_CHARGER) {
				data = STANDARD_CHARGER_CURRENT / 100;
...
//add by willcai 8-14
static kal_uint32  mt_battery_averge_vchg(kal_uint32 *bufferdata, kal_uint32 data, kal_int32 *sum ,kal_uint8 vchgIndex)
{
    kal_uint32 avgdata;
	kal_uint32 i ;
	for (i=0; i<BATTERY_AVERAGE_SIZE; i++)
		...
		*sum = data * BATTERY_AVERAGE_SIZE;
		*sum -=	bufferdata[vchgIndex];
	*sum +=  data;
	bufferdata[vchgIndex] = data;
    avgdata = (*sum)/BATTERY_AVERAGE_SIZE;

	battery_xlog_printk(BAT_LOG_FULL, "bufferdata[%d]= (%d) \n", vchgIndex,bufferdata[vchgIndex]);
	return avgdata;
...
static kal_uint32 mt_battery_average_method(kal_uint32 *bufferdata, kal_uint32 data, kal_int32 *sum, kal_uint8 batteryIndex)
{
	...
	*sum +=  data;
	...
	battery_xlog_printk(BAT_LOG_FULL, "bufferdata[%d]= (%d), avgdata %d\n", batteryIndex,bufferdata[batteryIndex], avgdata);
	return avgdata;
}
...
static void mt_battery_notify_VBatTemp_check(void)
{
	...
	if(BMT_status.temperature >= MAX_TEMPERATURE_WARNING)
    {
    ...
    if( upmu_is_chr_det() == KAL_TRUE )
    {
		...
		if(BMT_status.charger_type == CHARGER_UNKNOWN)
		{
			...
			if (BMT_status.charger_type == NONSTANDARD_CHARGER) {
				get_charger_device_type(&CHR_Type_num);
				if (CHR_Type_num == 4)
					BMT_status.charger_type = CHR_Type_num;
				else
...
static BLOCKING_NOTIFIER_HEAD(battery_chain);

int register_battery_chain_notifier(struct notifier_block *nb)
{
        return blocking_notifier_chain_register(&battery_chain, nb);
}
EXPORT_SYMBOL(register_battery_chain_notifier);

void unregister_battery_chain_notifier(struct notifier_block *nb)
{
        blocking_notifier_chain_unregister(&battery_chain, nb);
}
EXPORT_SYMBOL(unregister_battery_chain_notifier);

static int battery_chain_notify(unsigned long val, void *v)
{
	return blocking_notifier_call_chain(&battery_chain, val, v);
}
...
void do_chrdet_int_task(void)
{
    if(g_bat_init_flag == KAL_TRUE)
    {
        if( upmu_is_chr_det() == KAL_TRUE )
        {
            ...
            wake_lock(&battery_suspend_lock);
			battery_chain_notify(TRUE,NULL);
            #if defined(CONFIG_POWER_EXT)
            mt_usb_connect();
        ...
        else
        {
            ...
            BMT_status.charger_exist = KAL_FALSE;
    		battery_chain_notify(FALSE,NULL);	
#ifdef MTK_KERNEL_POWER_OFF_CHARGING
...
void BAT_thread(void)
{
    ...
    CHARGER_TYPE CHR_Type_num = CHARGER_UNKNOWN;
    //add by willcai
    static kal_int32 vchgBuffer[BATTERY_AVERAGE_SIZE];
    static kal_uint8 vchgIndex = 0;
    static kal_int32  vchg_sum ,charger_vol;
...
///MEIZU_BSP{@//when high or low temperature shutdown, android will do it
#if 0
#ifndef X2_CHARGING_STANDARD_SUPPORT
...
///@}
   ...
   if(lenovo_battery_charging_thread(&g_BatteryNotifyCode) == KAL_FALSE){
    ...
    if( BMT_status.charger_exist == KAL_TRUE )
    {
	    if (BMT_status.charger_type == NONSTANDARD_CHARGER) {
		    get_charger_device_type(&CHR_Type_num);
		    if (CHR_Type_num == MEIZU_2_0A_CHARGER)
			    BMT_status.charger_type = CHR_Type_num;
	    }
	//add by willcai  for the PE+
        charger_vol = battery_meter_get_charger_voltage();
	BMT_status.charger_vol=mt_battery_averge_vchg(&vchgBuffer[0],charger_vol, &vchg_sum, vchgIndex);
	vchgIndex++;
	if (vchgIndex >= BATTERY_AVERAGE_SIZE)
	{
		vchgIndex = 0;
	}
	//end 8-14
...
int bat_thread_kthread(void *x)
{
    ...
    while (1) {               
        ...
        ktime = ktime_set(battery_update_time, 0);  // 10s, 10* 1000 ms
...
static int battery_probe(struct platform_device *dev)    
{
    ...
    /*modify by tangxingyan 2014/11/27*/
#if 0	
	/* Initialization BMT Struct */
    baton_adc_value = PMIC_IMM_GetOneChannelValue(AUX_BATON_AP, 2, 1);
    if (baton_adc_value > BATON_ABSENT_ADC)
...
static void battery_shutdown(struct platform_device *dev)    
{
	...
	bat_vol = battery_meter_get_battery_voltage();
    printk("%s:BMT_status.bat_vol %d, bat_vol %d, BMT_status.UI_SOC %d\n",
		    __func__, BMT_status.bat_vol, bat_vol, BMT_status.UI_SOC);
