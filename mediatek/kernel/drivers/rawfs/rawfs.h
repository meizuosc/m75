...
#define RAWFS_PRINT(category, str, ...) do { \
    if (category & rawfs_debug_msg_mask)    { \
        printk(RAWFS_DBG "rawfs: " str, ##__VA_ARGS__); \
    } } while(0)
#else
#define RAWFS_PRINT(...)
#endif
...
#define RAWFS_NAND_BLOCKS(sb)         (2) // We use only two block
...
/* Inode operations */
extern int __init rawfs_init_inodecache(void);
extern void __exit rawfs_destroy_inodecache(void);
extern void rawfs_hash_init(struct super_block *sb);
extern struct inode *rawfs_alloc_inode(struct super_block *sb);
extern void rawfs_destroy_inode(struct inode *inode);
extern int rawfs_fill_inode(struct inode *inode,
    struct rawfs_file_info *file_info, int block_no, int page_no,
    umode_t mode, dev_t dev);
extern struct inode *rawfs_iget(struct super_block *sb, const char* name,
    int folder);

/* Mount-time analysis */
extern int rawfs_block_level_analysis(struct super_block *sb);
extern int rawfs_page_level_analysis(struct super_block *sb);
extern int rawfs_file_level_analysis(struct super_block *sb);
extern int rawfs_page_get(struct super_block *sb, int block_no, int page_no,
    struct rawfs_file_info *file_info, void *data);
extern int rawfs_block_is_valid(struct super_block *sb, int block_no,
    struct rawfs_block_header *block_head_out,
    struct rawfs_gc_marker_page *gc_page_out);

/* Device Operation */
extern int rawfs_dev_free_space(struct super_block *sb);
extern int rawfs_dev_garbage_collection(struct super_block *sb);
extern void rawfs_page_signature(struct super_block *sb, void *buf);

extern int rawfs_dev_mtd_erase_block(struct super_block *sb, int block_no);
extern int rawfs_dev_mtd_read_page_user(struct super_block *sb, int block_no,
    int block_offset, const struct iovec *iov, unsigned long nr_segs, int size);
extern int rawfs_dev_mtd_write_page(struct super_block *sb,
    int block_no, int page_no, void *buffer);
extern int rawfs_dev_mtd_read_page(struct super_block *sb,
    int block_no, int page_no, void *buffer);

extern int rawfs_dev_ram_erase_block(struct super_block *sb, int block_no);
extern int rawfs_dev_ram_read_page_user(struct super_block *sb, int block_no,
    int block_offset, const struct iovec *iov, unsigned long nr_segs, int size);
extern int rawfs_dev_ram_write_page(struct super_block *sb,
    int block_no, int page_no, void *buffer);
extern int rawfs_dev_ram_read_page(struct super_block *sb,
    int block_no, int page_no, void *buffer);

/* File Operations */
extern int rawfs_reg_file_delete(struct inode *dir, struct dentry *dentry);
extern int rawfs_reg_file_create(struct inode *dir, struct dentry *dentry,
    umode_t mode, struct nameidata *nd);
extern int rawfs_reg_file_copy(struct inode *src_dir, struct dentry *src_dentry,
    struct inode *dest_dir, struct dentry *dest_dentry);
extern int rawfs_reserve_space(struct super_block *sb, int chunks);

/* File info */
extern void rawfs_fill_file_info(struct inode *inode,
    struct rawfs_file_info *file_info);
extern void rawfs_fill_fileinfo_by_dentry(struct dentry *dentry,
    struct rawfs_file_info *file_info);

/* File and Folder lists */
extern struct rawfs_file_list_entry *rawfs_file_list_get(struct super_block *sb,
    const char *name, int folder_id);
extern struct rawfs_file_list_entry * rawfs_file_list_get_by_id(
    struct super_block *sb, umode_t mode, int id);
extern void rawfs_file_list_init(struct super_block *sb);
extern int rawfs_file_list_add(struct super_block *sb,
    struct rawfs_file_info *fi, int block_no, int page_no);
extern void rawfs_file_list_remove(struct super_block *sb,
    struct rawfs_file_info *fi);
extern void rawfs_file_list_destroy(struct super_block *sb);
extern int rawfs_file_list_count(struct super_block *sb,
    unsigned int *entry_count, unsigned int *used_blocks,
    unsigned int *free_blocks);
extern __u32 rawfs_page_crc_data(struct super_block *sb, void *data_page);
extern __u32 rawfs_page_crc_gcmarker(struct super_block *sb,
    void *gcmarker_page);

/* Utility */
extern uint32_t rawfs_div(uint64_t n, uint32_t base);
