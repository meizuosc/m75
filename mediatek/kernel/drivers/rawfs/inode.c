...
// file operations
extern struct file_operations rawfs_block_file_operations;
extern struct file_operations rawfs_file_operations;
...
// dentry operations
extern struct dentry_operations rawfs_dentry_ops;
extern struct dentry_operations rawfs_ci_dentry_ops;

// misc operations
extern struct address_space_operations rawfs_aops;
...
struct inode *rawfs_alloc_inode(struct super_block *sb)
{
	...
	if (!ei) {
        RAWFS_PRINT(RAWFS_DBG_INODE, "rawfs_alloc_inode, allocation failed, "
            "out of memory\n");
		return NULL;
	}
...
int __init rawfs_init_inodecache(void)
{
	...
	if (rawfs_inode_cachep == NULL) {
        RAWFS_PRINT(RAWFS_DBG_INODE, KERN_ERR "rawfs_init_inodecache, "
            "failed, -ENOMEM\n");
		return -ENOMEM;
	}
...
struct inode *rawfs_iget(struct super_block *sb, const char* name, int folder)
{
    ...
    RAWFS_PRINT(RAWFS_DBG_INODE, "rawfs_iget, by name %s @ folder %X in "
        "inode_hashtable[%ld]=%X\n", name, folder,
        rawfs_hash(name), (unsigned int)head);

    // Search inode list for the file
    spin_lock(&sbi->inode_hash_lock);

	hlist_for_each_entry(inode_info, ptr, head, i_rawfs_hash) {
        RAWFS_PRINT(RAWFS_DBG_INODE, "rawfs_iget, inode_hashtable[%ld][%d] %s "
            "@ folder %X, %X -> %X\n",
    ...
    int block_no, int page_no, umode_t mode, dev_t dev)
{
    ...
    {
        default:
            RAWFS_PRINT(RAWFS_DBG_INODE, KERN_INFO "rawfs_fill_inode: "
                "special inode\n");
        ...
        case S_IFREG:
            if ((block_no >= 0) && (page_no <0)) { /* Speical block file */
                ...
                RAWFS_PRINT(RAWFS_DBG_INODE, "rawfs_fill_inode: "
                    "reg file inode %s, size %lld bytes, %d blocks, "
    ...
    struct rawfs_file_info *file_info, int block_no, int page_no)
{
    ...
    if (file_info) {
        ...
        // Not exist in inode cache, search in file list
        {
...
{
    struct inode *inode = NULL;
    static struct dentry *result = NULL;
...
rawfs_mknod(struct inode *dir, struct dentry *dentry, umode_t mode, dev_t dev)
{
    ...
    if (inode) {
            RAWFS_PRINT(RAWFS_DBG_INODE, "rawfs_mknod: "
                "parent dir ID=%X, ino=%X\n", RAWFS_I(dir)->i_id, (unsigned)dir->i_ino);
...
static int rawfs_mkdir(struct inode * dir, struct dentry * dentry, umode_t mode)
{
    ...
    RAWFS_PRINT(RAWFS_DBG_DIR, "rawfs_mkdir: dentry name %s, "
        "parent id=%X, parent ino=%X\n", dentry->d_name.name,
        RAWFS_I(dir)->i_id,
        (unsigned)dir->i_ino);

    if (dir->i_ino != RAWFS_ROOT_INO) {
        RAWFS_PRINT(RAWFS_DBG_DIR, "rawfs_mkdir: "
            "sub-folder only allowed in root dir\n");
        result = -EACCES;
        goto out;
    }
    ...
    if (dentry->d_name.len > RAWFS_MAX_FILENAME_LEN) {
        RAWFS_PRINT(RAWFS_DBG_DIR, "rawfs_mkdir: "
            "foldername is too long -ENAMETOOLONG\n");
        result = -ENAMETOOLONG;
        goto out;
    }
    ...
    umode_t mode, struct nameidata *nd)
{
    ...
    RAWFS_PRINT(RAWFS_DBG_DIR, "rawfs_dir_create: %s, len %d\n",
        dentry->d_name.name, dentry->d_name.len);

    if (dentry->d_name.len > RAWFS_MAX_FILENAME_LEN) {
        RAWFS_PRINT(RAWFS_DBG_DIR, "rawfs_dir_create: "
            "filename is too long -ENAMETOOLONG\n");
        result = -ENAMETOOLONG;
        goto out;
    }
		...
		struct inode *new_dir, struct dentry *new_dentry)
{
    ...
    if (new_dentry->d_name.len > RAWFS_MAX_FILENAME_LEN) {
        RAWFS_PRINT(RAWFS_DBG_DIR, "rawfs_dir_rename: "
            "filename is too long -ENAMETOOLONG, len %d\n",
    ...
    if (entry) {
        ...
        RAWFS_PRINT(RAWFS_DBG_DIR, "rawfs_dir_rename: "
            "new location block %d, page %d\n",
            inode_info->i_location_block, inode_info->i_location_page);
    } else {
        RAWFS_PRINT(RAWFS_DBG_DIR, "rawfs_dir_rename:"
            "new entry missing after rename\n");
    }
...
static int rawfs_setattr(struct dentry *dentry, struct iattr *attr)
{
	...
	if (attr->ia_valid & ATTR_SIZE && (attr->ia_size > size_limit)) {
		RAWFS_PRINT(RAWFS_DBG_DIR, "rawfs_setattr: "
            "set length %llx beyond block size limit %d\n",
