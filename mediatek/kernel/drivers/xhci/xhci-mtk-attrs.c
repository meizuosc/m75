...
{
	imod = xhci_readl(mtk_xhci, (__le32 __iomem *) &mtk_xhci->ir_set->irq_control);
	imod &= ~0xFFFF0000;
			  ...
			  const char *buf, size_t count)
{
	...
	temp = xhci_readl(mtk_xhci, (__le32 __iomem *) &mtk_xhci->ir_set->irq_control);
	temp &= ~0xFFFF;
	temp |= imod;
	xhci_writel(mtk_xhci, temp, (__le32 __iomem *) &mtk_xhci->ir_set->irq_control);
...
{
	dburst = xhci_readl(mtk_xhci, (__le32 __iomem *) SSUSB_XHCI_HDMA_CFG);
	return sprintf(buf, "0x%x\n", dburst);
}
			    ...
			    const char *buf, size_t count)
{
	...
	temp = xhci_readl(mtk_xhci, (__le32 __iomem *) SSUSB_XHCI_HDMA_CFG);
	/* clear dma r/w fields */
	temp &= ~0x0C0C;
	/* set r/w burst size */
	temp |= (dburst << 2 | dburst << 10);
	xhci_writel(mtk_xhci, temp, (__le32 __iomem *) SSUSB_XHCI_HDMA_CFG);
