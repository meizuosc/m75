...
static void mtk_chk_usb_ip_ck_sts(void)
{
	...
	ret =
	    wait_for_value(SSUSB_IP_PW_STS1, SSUSB_SYS125_RST_B_STS, SSUSB_SYS125_RST_B_STS, 1, 10);
	if (ret == false)
		mtk_power_log("sys125_ck is still active!!!\n");

	/* do not check when SSUSB_U2_PORT_PDN = 1, because U2 port stays in reset state */
	if (num_u2_port && !(readl((void __iomem *)SSUSB_U2_CTRL(0)) & SSUSB_U2_PORT_PDN)) {
		ret =
		    wait_for_value(SSUSB_IP_PW_STS2, SSUSB_U2_MAC_SYS_RST_B_STS,
	...
	if (num_u3_port && !(readl((void __iomem *)SSUSB_U3_CTRL(0)) & SSUSB_U3_PORT_PDN)) {
		ret =
		    wait_for_value(SSUSB_IP_PW_STS1, SSUSB_U3_MAC_RST_B_STS, SSUSB_U3_MAC_RST_B_STS,
				   1, 10);
		if (ret == false)
