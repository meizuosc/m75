...
int __conn_md_dmp_in (ipc_ilm_t *p_ilm, CONN_MD_MSG_TYPE msg_type, P_CONN_MD_DMP_MSG_LOG p_msg_log)
{
	...
	/*Log msg content*/
	memcpy(&p_msg->data, p_ilm->local_para_ptr->data, p_msg->msg_len > LENGTH_PER_PACKAGE ? LENGTH_PER_PACKAGE : p_msg->msg_len);
...
int __conn_md_dmp_msg_filter(P_CONN_MD_DMP_MSG_STR p_msg, uint32 src_id, uint32 dst_id)
{
	...
	{
		__conn_md_log_print(DFT_TAG "%d.%d s, <%s> src_id:0x%08x, dst_id:0x%08x, msg_len:%d, dump_len:%d:\n", \
							p_msg->sec, \
							p_msg->usec, \
