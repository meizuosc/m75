...
for f in ${subdirs} ${files} ; do
    ...
    # $f has the form of ./bin/app_process, and we use ${f:2} to takeout the 2 characters ./
    # we must send file name in "system/bin/app_process" form
    # so that fs_get_stats can correctly report results
    f_with_prefix=$2/${f:2}
    new_info=`${fs_get_stats} ${curr_perms} ${is_dir} ${f_with_prefix}`
