...
static DEVICE_ATTR(profile_enable, S_IRUSR | S_IWUSR, cmdqCorePrintProfileEnable, cmdqCoreWriteProfileEnable);
...
static int cmdq_open(struct inode *pInode,
                     struct file  *pFile)
{
...
static int cmdq_release(struct inode *pInode,
                        struct file  *pFile)
{
    cmdqFileNodeStruct *pNode;
    unsigned long      flags;
    ...
    if (pNode->mutexFlag)
    {
        for (index = DISP_MUTEX_MDP_FIRST; index < DISP_MUTEX_MDP_FIRST + DISP_MUTEX_MDP_COUNT; index++)
        {
...
static int cmdq_driver_create_reg_address_buffer(cmdqCommandStruct *pCommand)
{
    ...
    if (0 != pCommand->debugRegDump)
    {
        ...
        if (0 != status)
        {
            CMDQ_ERR("cmdqCoreDebugRegDumpBegin returns %d, ignore kernel reg dump request\n", status);
            kernelRegCount = 0;
            kernelRegAddr = NULL;
        }
    }
    ...
    else
    {
        ...
        if(userRegCount)
        {
            if(copy_from_user(regAddrBuf, pCommand->regRequest.regAddresses, userRegCount * sizeof(uint32_t)))
            {
                kfree(regAddrBuf);
                return -EFAULT;
            }
...
static void cmdq_driver_process_read_address_request(cmdqReadAddressStruct *req_user)
{
    ...
    do
    {
        if (NULL == req_user ||
            0 == req_user->count ||
            NULL == req_user->values ||
            NULL == req_user->dmaAddresses)
        {
        ...
        // copy from user
        if (copy_from_user(addrs, req_user->dmaAddresses, req_user->count * sizeof(uint32_t)))
        {
                       ...
                       unsigned long param)
{
    ...
    switch(code)
    {
        ...
        case CMDQ_IOCTL_EXEC_COMMAND:
            ...
            if (command.regRequest.count != command.regValue.count)
            {
            ...
            userRegValue = command.regValue.regValues;
            userRegCount = command.regValue.count;

            // create kernel-space address buffer
            status = cmdq_driver_create_reg_address_buffer(&command);
            if (0 != status)
            {
                return status;
            }
            ...
            command.regValue.regValues = kzalloc(command.regRequest.count * sizeof(uint32_t), GFP_KERNEL);
            command.regValue.count = command.regRequest.count;
            if (NULL == command.regValue.regValues)
            {
                kfree(command.regRequest.regAddresses);
                return -ENOMEM;
            }
            ...
            status = cmdqCoreSubmitTask(&command);
            if (0 > status)
            {
            ...
            if (0 != command.debugRegDump)
            {
                status = cmdqCoreDebugRegDumpEnd(command.debugRegDump,
                                        command.regRequest.count - userRegCount,
                                        command.regValue.regValues + userRegCount);
                if (0 != status)
                {
            ...
            if (userRegValue && userRegCount)
            {
                ...
                CMDQ_VERBOSE("regValue[0] is %d\n", command.regValue.regValues[0]);
                if (copy_to_user(userRegValue, command.regValue.regValues, userRegCount * sizeof(uint32_t)))
                {
        ...
        case CMDQ_IOCTL_ASYNC_JOB_EXEC:
            ...
            status = cmdqCoreSubmitTaskAsync(&job.command,
                                             NULL,
                                             0,
                                             &pTask);
            ...
            kfree(job.command.regRequest.regAddresses);
        ...
        case CMDQ_IOCTL_ASYNC_JOB_WAIT_AND_CLOSE:
            ...
            // verify job handle
            if (!cmdqIsValidTaskPtr((TaskStruct*)jobResult.hJob))
            {
                CMDQ_ERR("invalid task ptr = 0x%lx\n", jobResult.hJob);
                return -EFAULT;
            }
            pTask = (TaskStruct*)jobResult.hJob;
            ...
            jobResult.regValue.regValues = kzalloc(pTask->regCount * sizeof(uint32_t), GFP_KERNEL);
            ...
            if (regCount > regCountUserSpace)
            {
                ...
                status = cmdqCoreDebugRegDumpEnd(regUserToken,
                                                 ...
                                                 jobResult.regValue.regValues + regCountUserSpace);
                if (0 != status)
                {
                    CMDQ_ERR("cmdqCoreDebugRegDumpEnd returns %d\n", status);
                }
            ...
            if (copy_to_user((void*)userRegValue, (void*)jobResult.regValue.regValues, regCountUserSpace * sizeof(uint32_t)))
            {
                CMDQ_ERR("Copy REGVALUE to user space failed\n");
                return -EFAULT;
            }
...
static irqreturn_t cmdq_irq_handler(int IRQ, void *pDevice)
{
    ...
    do
    {
        if(cmdq_dev_get_irq_id() == IRQ)
        {
            ...
            for (index = 0; (irqStatus != 0xFFFF) && index < CMDQ_MAX_THREAD_COUNT ; index++)
            {
...
static int cmdq_probe(struct platform_device *pDevice)
{
    ...
    status = request_irq(cmdq_dev_get_irq_id(), cmdq_irq_handler, IRQF_TRIGGER_LOW | IRQF_SHARED, CMDQ_DRIVER_DEVICE_NAME, gCmdqCDev);
    if (status != 0)
    {
...
static int __init cmdq_init(void)
{
    ...
    cmdqCoreRegisterCB(CMDQ_GROUP_VENC,
                        NULL,
                        cmdqVEncDumpInfo,
                        NULL,
                        NULL);
