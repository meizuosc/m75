...
void cookie(char *strbuf)
{
		MET_PRINTK("%s\n", strbuf);
}

void cookie2(char *strbuf)
{
		MET_PRINTK("%s\n", strbuf);
}
	...
	if (user_mode(regs)) {
		...
		for (vma = find_vma(mm, pc); vma; vma = vma->vm_next) {
			...
			if (vma->vm_file) {
				...
				ret= snprintf(pinfo->strbuf + pinfo->strlen, LINE_SIZE - pinfo->strlen,
				                     ",%s,%lx", (char *)(ppath->dentry->d_name.name), off);
				pinfo->strlen += ret;
                outflag = 1;
...
static const char header2_1[] =
	"# cookie2: task_name,PC,cookie,offset";
static const char header2_2[] =
	"met-info [000] 0.0: cookie2_header: task_name,PC,cookie,offset";
