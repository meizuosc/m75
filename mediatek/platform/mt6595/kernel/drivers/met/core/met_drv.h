...
#define MET_PRINTK(FORMAT, args...)							\
	do { 													\
	     char *pmet_strbuf;                                                     \
            if (in_nmi())  \
		pmet_strbuf = get_cpu_var(met_strbuf_nmi); \
	    else if (in_irq())  \
		pmet_strbuf = get_cpu_var(met_strbuf_irq); \
	    else if (in_softirq()) \
		pmet_strbuf = get_cpu_var(met_strbuf_sirq); \
	    else \
		pmet_strbuf = get_cpu_var(met_strbuf); \
	    if (met_mode & MET_MODE_TRACE_CMD)					\
		...
		trace_puts(pmet_strbuf);		\
            if (in_nmi())  \
		put_cpu_var(met_strbuf_nmi); \
	    else if (in_irq())  \
		put_cpu_var(met_strbuf_irq); \
	    else if (in_softirq()) \
		put_cpu_var(met_strbuf_sirq); \
	    else \
		put_cpu_var(met_strbuf); \
	} while (0)
#endif
