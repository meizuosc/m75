...
PHY_INT32 u2_slew_rate_calibration(struct u3phy_info *info)
{
	...
	U3PhyWriteField32((USB3_SIF_BASE + 0x110)
		, E60802_RG_FRCK_EN_OFST, E60802_RG_FRCK_EN, 0x1);

	// => USBPHY base address + 0x100 = 0x04
	// Setting cyclecnt
	U3PhyWriteField32((USB3_SIF_BASE + 0x100)
	...
	os_printk(K_INFO, "Freq_Valid=(0x%08X)\n", U3PhyReadReg32(USB3_SIF_BASE + 0x110));
	...
	for(i=0; i<10; i++){
		...
		// Read result
		u4FmOut = U3PhyReadReg32(USB3_SIF_BASE + 0x10C);
