...
static int __init mt_pwrap_init(void)
{
	...
	//PWRAPERR("not init (%x)\n", is_pwrap_init_done);
    PWRAPREG("GPIO driving,0x10005B70=%x\n",WRAP_RD32(0xf0005B70));
	PWRAPREG("GPIO SMT,0x10005940 before set=%x\n",WRAP_RD32(0xf0005940));
	//log
	reg_value1=pwrap_read_nochk(0x0126,&reg_value);
	PWRAPREG("0x%x=0x%x\n",0x0126,reg_value);
	reg_value1=pwrap_read_nochk(0x801C,&reg_value);
	PWRAPREG("0x%x=0x%x\n",0x801C,reg_value);
	reg_value1=pwrap_read_nochk(0x012C,&reg_value);
	PWRAPREG("0x%x=0x%x\n",0x012C,reg_value);
	reg_value1=pwrap_read_nochk(0x8022,&reg_value);
	PWRAPREG("0x%x=0x%x\n",0x8022,reg_value);
	reg_value1=pwrap_read_nochk(0x8068,&reg_value);
	PWRAPREG("0x%x=0x%x\n",0x8068,reg_value);
	return ret;
}
