...
#define CMDQ_AEE(tag, string, args...) \
    do { \
        ...
        aee_kernel_warning_api(__FILE__, __LINE__, DB_OPT_DEFAULT | DB_OPT_PROC_CMDQ_INFO | DB_OPT_MMPROFILE_BUFFER, tag, "error: "string, ##args); \
    } while(0)

#define CMDQ_PROFILE

#ifdef CMDQ_PROFILE
#define CMDQ_PROF_INIT() do{if(cmdq_core_profile_enabled()){met_tag_init();}}while(0)
#define CMDQ_PROF_START(args...) do{if(cmdq_core_profile_enabled()){met_tag_start(args);}}while(0)
#define CMDQ_PROF_END(args...) do{if(cmdq_core_profile_enabled()){met_tag_end(args);}}while(0)
#define CMDQ_PROF_ONESHOT(args...) do{if(cmdq_core_profile_enabled()){met_tag_oneshot(args);}}while(0)
...
// engineFlag are bit fields defined in CMDQ_ENG_ENUM
typedef int32_t (*CmdqDumpInfoCB)(uint64_t engineFlag,
                                  int level);
...
typedef int32_t (*CmdqDebugRegDumpBeginCB)(uint32_t taskID, uint32_t *regCount, uint32_t **regAddress);
