...
//add flush cache here qiguo 29/4
		///TODO: HalFlushInvalidateDCache();
		  ...
		  printk("Compare CbCr data start .......\n");
                ...
                msleep(100);
                if(fgDecErr == TRUE)
                    printk("CbCr Data Compare fail\n");
                else
                    printk("CbCr Data Compare pass\n");
...
int vH265CRCComparison(UINT32 u4InstID, UINT32* u4CRC){
    ...
    // Load golden file
    if (_ptH265CurrFBufInfo[u4InstID]->bIsUFOEncoded){
        sprintf(file_name, "%scrc_ROME_ufo_Y.dat", bitstream_name, _u4PicCnt[u4InstID]);
    }else{
        sprintf(file_name, "%scrc_ROME_sao_Y.dat", bitstream_name, _u4PicCnt[u4InstID]);
    }

    fd = openFile(file_name,O_RDONLY,0);
    if (IS_ERR(fd)){
        printk("[Warning] No Y CRC golden, open file & record!!\n");
        write_fd = filp_open( file_name , O_CREAT|O_RDWR,0777 );
        write_fd->f_op->write( write_fd, u4CRC, 16, &write_fd->f_pos );
        closeFile(write_fd);

    } else {
        if(readFileSize(fd, u4GoldenCRC, 1) < _u4PicCnt[u4InstID]*16+16){
            closeFile(fd);
            write_fd = filp_open( file_name , O_APPEND|O_RDWR,0777 );
            write_fd->f_op->write( write_fd, u4CRC, 16, &write_fd->f_pos );
            closeFile(write_fd);
            printk("[Warning] Y CRC golden recorded!!\n");

        }else{
            readFileOffset(fd, u4GoldenCRC, 16, _u4PicCnt[u4InstID]*16);
            if (u4GoldenCRC[0]==u4CRC[0] && u4GoldenCRC[1]==u4CRC[1] && u4GoldenCRC[2]==u4CRC[2] && u4GoldenCRC[3]==u4CRC[3]){
                printk("[INFO] Frame %d Y CRC pass!!\n",_u4PicCnt[u4InstID]);
            } else {
                printk("[ERROR] Frame %d Y CRC fail!! Golden: 0x%08x 0x%08x 0x%08x 0x%08x \n", u4GoldenCRC[0], u4GoldenCRC[1], u4GoldenCRC[2], u4GoldenCRC[3] );
                bIsFail =1;
            }
            closeFile(fd);
        }
    ...
    // Load golden file
    if (_ptH265CurrFBufInfo[u4InstID]->bIsUFOEncoded){
        sprintf(file_name, "%scrc_ROME_ufo_C.dat", bitstream_name, _u4PicCnt[u4InstID]);
    }else{
        sprintf(file_name, "%scrc_ROME_sao_C.dat", bitstream_name, _u4PicCnt[u4InstID]);
    }

    fd = openFile(file_name,O_RDONLY,0);
    if (IS_ERR(fd)){
        printk("[Warning] No C CRC golden, open file & record!!\n");
        write_fd = filp_open( file_name , O_CREAT|O_RDWR,0777 );
        write_fd->f_op->write( write_fd, u4CRC+4 , 16, &write_fd->f_pos );
        closeFile(write_fd);
        ...
        if (readFileSize(fd, u4GoldenCRC, 1) < _u4PicCnt[u4InstID]*16+16){
            ...
            write_fd->f_op->write( write_fd, u4CRC+4 , 16, &write_fd->f_pos );
            closeFile(write_fd);
            printk("[Warning] C CRC golden recorded!!\n");

        }else{
            readFileOffset(fd, u4GoldenCRC+4, 16, _u4PicCnt[u4InstID]*16);
            if (u4GoldenCRC[4]==u4CRC[4] && u4GoldenCRC[5]==u4CRC[5] && u4GoldenCRC[6]==u4CRC[6] && u4GoldenCRC[7]==u4CRC[7]){
                printk("[INFO] Frame %d C CRC pass!!\n", _u4PicCnt[u4InstID]);
            } else {
                printk("[ERROR] Frame %d C CRC fail!! Golden: 0x%08x 0x%08x 0x%08x 0x%08x \n", u4GoldenCRC[4], u4GoldenCRC[5], u4GoldenCRC[6], u4GoldenCRC[7] );
                bIsFail =1;
            }
            closeFile(fd);
        }
...
SkipPixelCompare:
    ...
    if((fgOpen == FALSE) ||(fgDecErr == TRUE) || (_fgVDecErr[u4InstID] == TRUE))
    {
        ...
        {
            msleep(3000);
            sprintf(strMessage," %s Compare Finish==>  Pic count to [%d] \n", _bFileStr1[0][1], _u4FileCnt[u4InstID]);
            printk("%s\n",strMessage);
            DBG_H264_PRINTF("%s\n",strMessage);
        ...
        {
            msleep(3000);
            sprintf(strMessage," %s Compare Finish Error ==> Pic count to [%d] \n", _bFileStr1[0][1], _u4FileCnt[u4InstID] - 1); 
            printk("%s\n",strMessage);
            msleep(3000);
        }
        _u4VerBitCount[u4InstID] = 0xffffffff;
    ...
    else
    {
        ...
        if(fgOpen == FALSE)
        {
            sprintf(strMessage," %s Compare Finish [%d]==> Pic count to [%d] \n", _bFileStr1[0][1], u4InstID,_u4FileCnt[u4InstID] - 1);
            printk("%s\n",strMessage);
            if(_u4FileCnt[u4InstID] == 1)
            {
                if(fgOpen == FALSE)
        ...
        {
            sprintf(strMessage," %s Error[%d] ==> Pic count to [%d] \n", _bFileStr1[0][1], u4InstID,_u4FileCnt[u4InstID] - 1);     
            printk("%s\n",strMessage);
        }
        _u4VerBitCount[u4InstID] = 0xffffffff;
        }
...
BOOL fgVdecReadFileName(UINT32 u4InstID, VDEC_INFO_VERIFY_FILE_INFO_T *ptFileListInfo, VDEC_INFO_VERIFY_FILE_INFO_T *ptFileRecInfo, UINT32 *u4StartComp, UINT32 *u4EndComp, UINT32 *DumpPic)
{
    ...
    *DumpPic = 0; // only uncomment when want to dump single pic
    ...
    while(((*_pcBitstreamFileName[u4InstID]) != '\n')&&((*_pcBitstreamFileName[u4InstID]) != '\r') &&(_u4FileListIdx[u4InstID] < ptFileListInfo->u4FileLength))
    {
    ...
    printk("\nStart %d, End %d, Dump %d (%d)\n", u4StarFrm, u4EndFrm, u4DumpFrm, u4Length);
