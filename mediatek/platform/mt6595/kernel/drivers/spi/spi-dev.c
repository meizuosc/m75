...
#define SPI_CROSS_ALIGN_OFFSET 1008	
	...
	if(flag == 0){
		...
		xfer->tx_dma = dma_map_single(NULL, (void *)xfer->tx_buf, 	
				xfer->len,  DMA_TO_DEVICE);				
		if (dma_mapping_error(NULL, xfer->tx_dma)) {
			SPIDEV_LOG("dma mapping tx_buf error.\n");
			return -ENOMEM;	
		}
		xfer->rx_dma = dma_map_single(NULL, (void *)xfer->rx_buf, 	
				xfer->len,  DMA_TO_DEVICE);				
		if (dma_mapping_error(NULL, xfer->rx_dma)) {
			SPIDEV_LOG("dma mapping rx_buf error.\n");
			return -ENOMEM;	
		}
		SPIDEV_LOG("Transfer addr:Tx:0x%llu, Rx:0x%llu, before\n", (unsigned long long)xfer->tx_dma,(unsigned long long)xfer->rx_dma);
		xfer->len=32;
		p= xfer->tx_dma;
		...
		SPIDEV_LOG("Transfer addr:Tx:0x%llu, Rx:0x%llu\n", (unsigned long long)xfer->tx_dma,(unsigned long long)xfer->rx_dma);
...
static DEVICE_ATTR(spi, 200, NULL, spi_store);
static DEVICE_ATTR(spi_msg, 200, NULL, spi_msg_store);
