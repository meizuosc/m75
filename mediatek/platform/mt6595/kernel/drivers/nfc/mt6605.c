...
{	
  ...
  printk(KERN_DEBUG "mt6605_remove\n");
	...
	if (I2CDMAWriteBuf)
	{
		dma_free_coherent(NULL, MAX_BUFFER_SIZE, I2CDMAWriteBuf, (dma_addr_t)I2CDMAWriteBuf_pa);
		I2CDMAWriteBuf = NULL;
		I2CDMAWriteBuf_pa = 0;
	}
	...
	if (I2CDMAReadBuf)
	{
		dma_free_coherent(NULL, MAX_BUFFER_SIZE, I2CDMAReadBuf, (dma_addr_t)I2CDMAReadBuf_pa);
		I2CDMAReadBuf = NULL;
		I2CDMAReadBuf_pa = 0;
	}
...
static void mt6605_dump_gpio(struct mt6605_dev *dev)
{
	int irq,ven,eint,rst,sda,scl ;

	irq =  mt_get_gpio_in(dev->irq_gpio);
	ven =  mt_get_gpio_out(dev->ven_gpio);
	eint = mt_get_gpio_out(dev->eint_gpio);
	rst =  mt_get_gpio_out(dev->sysrstb_gpio);
	sda =  mt_get_gpio_in(GPIO_I2C3_SDA_PIN);
	scl =  mt_get_gpio_in(GPIO_I2C3_SCA_PIN);
	printk(KERN_ERR "%s:!!!!MT6605 ERROR DUMP GPIO VALUE !!!!:\n",__func__);
	printk(KERN_ERR"irq = %d,ven = %d,eint = %d,rst =%d,sda = %d,scl = %d\n",
		irq,ven,eint,rst,sda,scl);
}
...
static int __init mt6605_dev_init(void)
{
   ...
   printk(KERN_ERR "mt6605_dev_init\n");
   i2c_register_board_info(NFC_I2C_BUSNUM, &nfc_board_info, 1);
   printk(KERN_ERR "mt6605_dev_init2\n");
   i2c_add_driver(&mt6605_dev_driver);
   printk(KERN_ERR "mt6605_dev_init success\n");	
   return 0;
}
