...
static unsigned int gM4UBaseAddr[] = {M4U_BASE0, M4U_BASE1};
static unsigned int gLarbBaseAddr[] = {LARB0_BASE, LARB1_BASE, LARB2_BASE, LARB3_BASE, LARB4_BASE};
...
#define M4U1_PORT_INIT(port,tf_id)   1,0,5,port,tf_id
...
m4u_port_t gM4uPort[] = 
{
    ...
    { "VDEC_MC_EXT",                M4U0_PORT_INIT( 0, 1,  0    ),    },
    { "VDEC_PP_EXT",                M4U0_PORT_INIT( 0, 1,  1    ),    },
    { "VDEC_UFO_EXT",               M4U0_PORT_INIT( 0, 1,  2    ),    },
    { "VDEC_VLD_EXT",               M4U0_PORT_INIT( 0, 1,  3    ),    },
    { "VDEC_VLD2_EXT",              M4U0_PORT_INIT( 0, 1,  4    ),    },
    { "VDEC_AVC_MV_EXT",            M4U0_PORT_INIT( 0, 1,  5    ),    },
    { "VDEC_PRED_RD_EXT",           M4U0_PORT_INIT( 0, 1,  6    ),    },
    { "VDEC_PRED_WR_EXT",           M4U0_PORT_INIT( 0, 1,  7    ),    },
    { "VDEC_PPWRAP_EXT",            M4U0_PORT_INIT( 0, 1,  8    ),    },
                                  
    { "IMGO",                       M4U0_PORT_INIT( 0, 2,  0    ),    },
    { "RRZO",                       M4U0_PORT_INIT( 0, 2,  1    ),    },
    { "AAO",                        M4U0_PORT_INIT( 0, 2,  2    ),    },
    { "LCSO",                       M4U0_PORT_INIT( 0, 2,  3    ),    },
    { "ESFKO",                      M4U0_PORT_INIT( 0, 2,  4    ),    },
    { "IMGO_S",                     M4U0_PORT_INIT( 0, 2,  5    ),    },
    { "LSCI",                       M4U0_PORT_INIT( 0, 2,  6    ),    },
    { "LSCI_D",                     M4U0_PORT_INIT( 0, 2,  7    ),    },
    { "BPCI",                       M4U0_PORT_INIT( 0, 2,  8    ),    },
    { "BPCI_D",                     M4U0_PORT_INIT( 0, 2,  9    ),    },
    { "UFDI",                       M4U0_PORT_INIT( 0, 2,  10   ),    },
    { "IMGI",                       M4U0_PORT_INIT( 0, 2,  11   ),    },
    { "IMG2O",                      M4U0_PORT_INIT( 0, 2,  12   ),    },
    { "IMG3O",                      M4U0_PORT_INIT( 0, 2,  13   ),    },
    { "VIPI",                       M4U0_PORT_INIT( 0, 2,  14   ),    },
    { "VIP2I",                      M4U0_PORT_INIT( 0, 2,  15   ),    },
    { "VIP3I",                      M4U0_PORT_INIT( 0, 2,  16   ),    },
    { "LCEI",                       M4U0_PORT_INIT( 0, 2,  17   ),    },
    { "RB",                         M4U0_PORT_INIT( 0, 2,  18   ),    },
    { "RP",                         M4U0_PORT_INIT( 0, 2,  19   ),    },
    { "WR",                         M4U0_PORT_INIT( 0, 2,  20   ),    },
...
void m4u_invalid_tlb_sec(int m4u_id,int L2_en, int isInvAll, unsigned int mva_start, unsigned int mva_end)
{
    unsigned int reg = 0;
    unsigned int m4u_base = gM4UBaseAddr[m4u_id];
    unsigned int size=mva_end-mva_start+1;

    if(mva_start>=mva_end)
        isInvAll = 1;

    if(!isInvAll) //workaroud 
    {
        if(size >= SZ_1M)
            size = max((unsigned int)SZ_8M, size);
        mva_end = mva_start + size - 1;
    }
...
int __mau_dump_status(int m4u_id, int m4u_slave_id, int mau)
{
    ...
    if (status & (1 << mau))
    {
        ...
        M4UMSG("id=0x%x(%s),addr=0x%x,b32=0x%x\n", assert_id, 
        ...
        M4UMSG("mau_cfg: start=0x%x,end=0x%x,virt(%d),io(%d),wr(%d),s_b32(%d),e_b32(%d)\n",
...
int m4u_dump_main_tlb(int m4u_id, int m4u_slave_id) 
{
    ...
    for(i=0;i<gM4UTagCount[m4u_id];i++)
    {
        ...
        printk(KERN_INFO"%d:0x%x:0x%x  ", i, tlb.tag, tlb.desc);
        if((i+1)%8==0)
            printk(KERN_INFO"\n");
    }
...
int m4u_dump_pfh_tlb(int m4u_id) 
{
    ...
    for(way=0; way<way_nr; way++)
    {
        for(set=0; set<set_nr; set++)
        {
            int page;
            mmu_tlb_t tlb;
            regval = M4U_ReadReg32(m4u_base, REG_MMU_PFH_VLD(set, way));
...
static int smi_common_clock_on(void)
{
...
static int smi_common_clock_off(void)
{
...
//==================================================================    
    ...
    for(i=0; i<SEQ_NR_PER_M4U_SLAVE; i++)
    {
        if(1==pSeq[i].Enabled)
        {
            ...
            {
                M4ULOG_MID("insert range overlap!: larb=%d,module=%s\n", 
                     m4u_port_2_larb_id(port), m4u_get_port_name(port));
                M4ULOG_MID("warning: insert tlb range is overlapped with previous ranges, current process=%s,!\n",  current->comm);	
                M4ULOG_MID("module=%s, mva_start=0x%x, mva_end=0x%x \n", m4u_get_port_name(port), MVAStart, MVAEnd);
                M4ULOG_MID("overlapped range id=%d, module=%s, mva_start=0x%x, mva_end=0x%x \n", 
...
static int _m4u_config_port(int port, int virt, int sec, int dis, int dir)
{
    int m4u_index = m4u_port_2_m4u_id(port);
    unsigned int m4u_base = gM4UBaseAddr[m4u_index];
    unsigned int larb_base, larb, larb_port;
	int ret = 0;
    ...
    else
    {
        ...
        m4uHw_set_field_by_mask(0, REG_PERIAXI_BUS_CTL3,\
...
int m4u_monitor_stop(int m4u_id)
{
    ...
    M4UMSG("[M4U%d-%d] total:%d, main miss:%d, pfh miss(walk):%d, auto pfh:%d\n", 
        m4u_id, 0,
        cnt.transaction_cnt, cnt.main_tlb_miss_cnt, cnt.pfh_tlb_miss_cnt,cnt.pfh_cnt);

    m4u_get_perf_counter(m4u_index, 1, &cnt);
    //read register get the count
    M4UMSG("[M4U%d-%d] total:%d, main miss:%d, pfh miss(walk):%d, auto pfh:%d\n", 
        m4u_id, 1,
        cnt.transaction_cnt, cnt.main_tlb_miss_cnt, cnt.pfh_tlb_miss_cnt,cnt.pfh_cnt);
...
static unsigned int smi_common_reg_backup_buf[1];
...
static void larb_backup(struct larb_monitor *h, int larb_idx)
{
    ...
    M4ULOG_MID("larb(%d) backup 0x%x\n", larb_idx, COM_ReadReg32(0xf4000100));
    ...
    /*smi common is in larb0*/                                                        
    if(larb_idx == 0)                                                                 
    {                                                                                 
        __M4U_BACKUP(0, REG_SMI_BUS_SEL, smi_common_reg_backup_buf[0]);               
    }                                                                                 
    return;
}
...
static void larb_restore(struct larb_monitor *h, int larb_idx)
{
    ...
    larb_base = gLarbBaseAddr[larb_idx];                                 
    M4ULOG_MID("larb(%d) restore 0x%x\n", larb_idx, COM_ReadReg32(0xf4000100));
    ...
    if(larb_idx == 0)                                                                 
    {                                                                                 
        __M4U_RESTORE(0, REG_SMI_BUS_SEL, smi_common_reg_backup_buf[0]);               
    }                                                                                 
    return;
}
...
void m4u_print_port_status(struct seq_file *seq, int only_print_active)
{
    ...
    for(port=0; port<gM4u_port_num; port++)
    {
        ...
        else
        {
            ...
            mmu_en = m4uHw_get_field_by_mask(0, REG_PERIAXI_BUS_CTL3,F_PERI_MMU_EN(larb_port, 1));
        }
...
int m4u_enable_prefetch(M4U_PORT_ID PortID)
{
...
int m4u_disable_prefetch(M4U_PORT_ID PortID)
{
    unsigned int m4u_base = gM4UBaseAddr[m4u_port_2_m4u_id(PortID)];
...
irqreturn_t MTK_M4U_isr(int irq, void *dev_id)
{
    ...
    switch(irq)
    {
        case MM_IOMMU_IRQ_B_ID:
        	m4u_base = gM4UBaseAddr[0];
        	m4u_index = 0;
          break;
        case PERISYS_IOMMU_IRQ_B_ID:
...

    {
        ...
        M4UMSG("m4u L2 interrupt happened: sta=0x%x\n", regval);
        ...
        if(regval&F_INT_L2_PFH_IN_OUT_FIFO_ERROR)
        {
            MMU_INT_REPORT(m4u_index, 0, F_INT_L2_PFH_IN_OUT_FIFO_ERROR);
        }
        if(regval&F_INT_L2_MISS_FIFO_ERR)
        {
            MMU_INT_REPORT(m4u_index, 0, F_INT_L2_MISS_FIFO_ERR);
        }
...
int m4u_reg_init(m4u_domain_t* m4u_domain, unsigned int ProtectPA)
{
    unsigned int regval;
    int i;
...
//=============================================
    ...
    */
    regval = F_SMI_BUS_SEL_larb0(larb_2_m4u_slave_id(0))  \
            |F_SMI_BUS_SEL_larb1(larb_2_m4u_slave_id(1))  \
            |F_SMI_BUS_SEL_larb2(larb_2_m4u_slave_id(2))  \
            |F_SMI_BUS_SEL_larb3(larb_2_m4u_slave_id(3))  \
            |F_SMI_BUS_SEL_larb4(larb_2_m4u_slave_id(4));
...
//=============================================
    ...
    for(i=0;i<TOTAL_M4U_NUM;i++)
    {
        M4U_WriteReg32(gM4UBaseAddr[i], REG_MMUg_PT_BASE, (unsigned int)m4u_domain->pgd_pa);
        M4U_WriteReg32(gM4UBaseAddr[i], REG_MMUg_PT_BASE_SEC, (unsigned int)m4u_domain->pgd_pa);
        ...
        if(!gM4U_4G_DRAM_Mode)
            M4U_WriteReg32(gM4UBaseAddr[i], REG_MMU_IVRP_PADDR, F_MMU_IVRP_PA_SET((unsigned int)ProtectPA));
        else
            M4U_WriteReg32(gM4UBaseAddr[i], REG_MMU_IVRP_PADDR, F_MMU_IVRP_4G_DRAM_PA_SET((unsigned int)ProtectPA));

        //enable DCM
        M4U_WriteReg32(gM4UBaseAddr[i], REG_MMU_DCM_DIS, 0);
        
        m4u_invalid_tlb_all(i);
...
int m4u_hw_init(struct m4u_device *m4u_dev)
{
    unsigned int pProtectVA, ProtectPA;

    gM4U_4G_DRAM_Mode = enable_4G();
    M4UMSG("4G DRAM Mode is: %d\n", gM4U_4G_DRAM_Mode);

    m4u_domain_init(m4u_dev);
    
    pProtectVA = (unsigned int) kmalloc(TF_PROTECT_BUFFER_SIZE*2, GFP_KERNEL|__GFP_ZERO);
    ...
    {
        M4UMSG("protect buffer not align.\n");
        return -1;
    }
    ...
    m4u_reg_init(&gM4uDomain,ProtectPA);

    
    if (request_irq(MM_IOMMU_IRQ_B_ID, MTK_M4U_isr, IRQF_TRIGGER_LOW, "m4u", NULL))
    {
        M4UERR("request M4U0 IRQ line failed\n");
        return -ENODEV;
    }

    if (request_irq(PERISYS_IOMMU_IRQ_B_ID, MTK_M4U_isr, IRQF_TRIGGER_LOW, "m4u", NULL))
    {
...
int m4u_hw_deinit(struct m4u_device *m4u_dev)
{
