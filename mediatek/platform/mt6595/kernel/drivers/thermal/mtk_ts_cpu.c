...
*/
	switch(ts_name){
        case THERMAL_CA7://TS1
            x_roomt = g_x_roomt[0];
            break;
        case THERMAL_CA15://TS3
            x_roomt = g_x_roomt[2];
            break;
        case THERMAL_GPU://TS4
            x_roomt = g_x_roomt[3];
            break;
...
static int get_immediate_temp1(void)
{
	...
	printk("get_immediate_temp1 raw1=%d\n", curr_raw1);
	#if 0
...
static int get_immediate_temp2(void)
{
	...
	printk("get_immediate_temp2 raw1=%d\n", curr_raw2);
    #if 0
...
static void read_each_bank_TS(thermal_bank_name bank_num)
{
	...
	switch(bank_num){
	    case THERMAL_BANK0:
			//Bank 0 : CA7  (TS1 TS2)
			CA7_TS1_T = read_tc_raw_and_temp(TEMPMSR0,THERMAL_SENSOR1,&CA7_TS1_R);
			CA7_TS2_T = read_tc_raw_and_temp(TEMPMSR1,THERMAL_SENSOR2,&CA7_TS2_R);
	        break;
	    case THERMAL_BANK1:
			//Bank 1 : CA15 (TS1 TS3)
			CA15_TS1_T = read_tc_raw_and_temp(TEMPMSR0,THERMAL_SENSOR1,&CA15_TS1_R);
			CA15_TS3_T = read_tc_raw_and_temp(TEMPMSR1,THERMAL_SENSOR3,&CA15_TS3_R);
	        break;
	    case THERMAL_BANK2:
			//Bank 2 : GPU  (TS3 TS4)
			GPU_TS3_T = read_tc_raw_and_temp(TEMPMSR0,THERMAL_SENSOR3,&GPU_TS3_R);
			GPU_TS4_T = read_tc_raw_and_temp(TEMPMSR1,THERMAL_SENSOR4,&GPU_TS4_R);
	        break;
	    case THERMAL_BANK3:
            //Bank 3 : CORE (TS2 TS4 TSABB)
            #if 0
			CORE_TS4_T 	 = read_tc_raw_and_temp(TEMPMSR0,THERMAL_SENSOR4,&CORE_TS4_R);
			CORE_TSABB_T = read_tc_raw_and_temp(TEMPMSR1,THERMAL_SENSORABB,&CORE_TSABB_R);
            #else
			CORE_TS2_T 	 = read_tc_raw_and_temp(TEMPMSR0,THERMAL_SENSOR2,&CORE_TS2_R);
			CORE_TS4_T 	 = read_tc_raw_and_temp(TEMPMSR1,THERMAL_SENSOR4,&CORE_TS4_R);
			CORE_TSABB_T = read_tc_raw_and_temp(TEMPMSR2,THERMAL_SENSORABB,&CORE_TSABB_R);
            #endif
	        break;
		default:
			CA7_TS1_T = read_tc_raw_and_temp(TEMPMSR0,THERMAL_SENSOR1,&CA7_TS1_R);
...
int thermal_fast_init(void)
{
   ...
   temp = 0xDA1;
    ...
    DRV_WriteReg32(TEMPPNPMUXADDR, (UINT32) (PTPSPARE0- 0xE0000000));    // AHB address for pnp sensor mux selection
    DRV_WriteReg32(TEMPADCMUXADDR, (UINT32) (PTPSPARE0- 0xE0000000));    // AHB address for auxadc mux selection
    DRV_WriteReg32(TEMPADCENADDR,  (UINT32) (PTPSPARE1- 0xE0000000));     // AHB address for auxadc enable
    DRV_WriteReg32(TEMPADCVALIDADDR, (UINT32) (PTPSPARE2- 0xE0000000));  // AHB address for auxadc valid bit
    DRV_WriteReg32(TEMPADCVOLTADDR, (UINT32) (PTPSPARE2- 0xE0000000));   // AHB address for auxadc voltage output
