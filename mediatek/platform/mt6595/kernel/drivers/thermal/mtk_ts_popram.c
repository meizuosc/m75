...
static int __init mtktspopram_init(void)
{
    ...
    mtktspopram_dir = proc_mkdir("mtktspopram", NULL);
    if (!mtktspopram_dir)
    {
        mtktspopram_dprintk("[mtktspopram_init]: mkdir /proc/mtktspopram failed\n");
    }
    else
    {
        entry = proc_create("mtktspopram", S_IRUGO | S_IWUSR | S_IWGRP, mtktspopram_dir, &mtktspopram_fops);
        if (entry) {
