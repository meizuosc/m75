...
mtk_cl_mutt_set_mutt_limit(void)
{
    ...
    if (min_param != cl_mutt_cur_limit)
    {
        ...
        last_md_boot_cnt = ccci_get_md_boot_count(0); // need to sync with MD_SYS1 = 0 defined in ccci_core.h
        ret = exec_ccci_kern_func_by_md_id(0, ID_THROTTLING_CFG, (char*) &cl_mutt_cur_limit, 4); // need to sync with MD_SYS1 = 0 defined in ccci_core.h
        mtk_cooler_mutt_dprintk_always("[%s] ret %d param %x bcnt %ul\n", __func__, ret, cl_mutt_cur_limit, last_md_boot_cnt);
    }
    else if (min_param != 0)
    {
        unsigned long cur_md_bcnt = ccci_get_md_boot_count(0); // need to sync with MD_SYS1 = 0 defined in ccci_core.h
