...
//NTCG103JF103F(10K)
AP_TEMPERATURE AP_Temperature_Table7[] = {
    {-40,188500},
    {-35,144290},
    {-30,111330},
    {-25,86560},
...
static int __init mtkts_AP_init(void)
{
	...
	else
	{
		...
		entry = proc_create("mtktsAP", S_IRUGO | S_IWUSR | S_IWGRP, mtkts_AP_dir, &mtkts_AP_fops);
		if (entry) {
		...
		entry = proc_create("mtkts_AP_param", S_IRUGO | S_IWUSR | S_IWGRP, mtkts_AP_dir, &mtkts_AP_param_fops);
		if (entry) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
            proc_set_user(entry, 0, 1000);
#else // kernel ver
