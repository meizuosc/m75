...
//#if defined (MTK_FB_ION_SUPPORT)
	...
	else 
	{
		...
		dst->addr = (unsigned int)src->src_phy_addr;
#endif
	DISPMSG("_convert_fb_layer_to_disp_input, dst->addr=0x%08x\n", dst->addr);
...
#if 1
	DISPMSG("_convert_fb_layer_to_disp_input():id=%u, en=%u, next_idx=%u, vaddr=0x%x, paddr=0x%x, src fmt=%u, dst fmt=%u, pitch=%u, xoff=%u, yoff=%u, w=%u, h=%u\n",
						dst->layer,
						dst->layer_en,
						dst->buff_idx,
...
static int mtkfb_pan_display_impl(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	input.addr = (unsigned int)paStart;
	input.vaddr = (unsigned int)vaStart;
	...
	input.dst_h = var->yres;
		...
		input.isDirty = 1;
	idle_lock_spm(IDLE_SPM_LOCK_DISPLAY_CONFIG);
	ret = primary_display_config_input(&input);
	ret = primary_display_trigger(TRUE, NULL, 0);
	idle_unlock_spm(IDLE_SPM_LOCK_DISPLAY_CONFIG);
...
static int mtkfb_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
    ...
    case MTKFB_SLT_AUTO_CAPTURE:
    {
        ...
        else
        {
            ...
            primary_display_capture_framebuffer_ovl((unsigned int)capConfig.outputBuffer, format);
        }
...
static void _mtkfb_draw_block(unsigned int addr, unsigned int x, unsigned int y, unsigned int w, unsigned int h, unsigned int color)
{
	int i = 0;
	int j = 0;
	unsigned int start_addr = addr+MTK_FB_XRESV*4*y+x*4;
	for(j=0;j<h;j++)
	{
		for(i = 0;i<w;i++)
		{
...
// 0: success / 1: fail
void _parse_tag_videolfb(void)
{
#if 0
        struct tag_videolfb *videolfb_tag = NULL;

	DISPCHECK("[DT][videolfb] of_chosen = 0x%x, isvideofb_parse_done = %d\n", of_chosen,is_videofb_parse_done);

	if(is_videofb_parse_done) return;

	if(of_chosen)
	{
		videolfb_tag = (struct tag_videolfb*)of_get_property(of_chosen, "atag,videolfb", NULL);
		if(videolfb_tag)
		{
			memset((void*)mtkfb_lcm_name, 0, sizeof(mtkfb_lcm_name));
	...
	if(node)
	{
		...
		if (videolfb_tag)
		{
			...
			islcmconnected = videolfb_tag->islcmfound;
			vramsize = videolfb_tag->vram;
			fb_base  = (phys_addr_t)(videolfb_tag->fb_base);
			is_videofb_parse_done = 1;
			
			DISPCHECK("[DT][videolfb] fb_base    = %pa\n", &fb_base);
			DISPCHECK("[DT][videolfb] islcmfound = %d\n", islcmconnected);
			DISPCHECK("[DT][videolfb] fps        = %d\n", lcd_fps);
			DISPCHECK("[DT][videolfb] vram       = %d\n", vramsize);
			DISPCHECK("[DT][videolfb] lcmname    = %s\n", mtkfb_lcm_name);
		...
		{
			DISPCHECK("[DT][videolfb] FB info not available\n");
		}
...
static int mtkfb_probe(struct device *dev)
{
    ...
    {
        DISPERR("probed for an unknown device\n");
        r = -ENODEV;
        goto cleanup;
    }
    ...
    DISPMSG("[FB Driver] fbdev->fb_pa_base = %pa, fbdev->fb_va_base = 0x%x\n", &(fbdev->fb_pa_base), (unsigned int)(fbdev->fb_va_base));
	    ...
	    
	{
		...
		DAL_STATUS ret;
		unsigned int fbVA = fbdev->fb_va_base;
...
#if 0 //added by mars. do not diagnose.
	primary_display_diagnose();
#endif
...
int mtkfb_get_debug_state(char* stringbuf, int buf_len)
{
	...
	unsigned int pa = res->start;
	unsigned int resv_size = res->end - res->start+1;
