...
static int _draw_line(unsigned int addr, int l, int t, int r, int b, int linepitch, unsigned int color)
{
	int i = 0;
...
static void _draw_block(unsigned int addr, unsigned int x, unsigned int y, unsigned int w, unsigned int h, unsigned int linepitch, unsigned int color)
{
	int i = 0;
	int j = 0;
	unsigned int start_addr = addr+linepitch*y+x*4;
	DISPMSG("addr=0x%08x, start_addr=0x%08x, x=%d,y=%d,w=%d,h=%d,linepitch=%d, color=0x%08x\n", addr, start_addr, x,y,w,h,linepitch,color);
	for(j=0;j<h;j++)
	{
		for(i = 0;i<w;i++)
		{
			*(unsigned int*)(start_addr + i*4 + j*linepitch) = color;
		}
	}
}
