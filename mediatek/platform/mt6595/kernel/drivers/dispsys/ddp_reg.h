...
#define DISP_REG_MASK(handle, reg32, val, mask) 	\
	do { \
		dprec_reg_op(handle, reg32, val, mask);\
		 if(handle==NULL) \
		 { \
			mt_reg_sync_writel((unsigned int)(INREG32(reg32)&~(mask))|(val),(volatile unsigned int*)(reg32) );\
		 } \
		 else \
		 { \
			cmdqRecWrite(handle, (unsigned int)(reg32)&0x1fffffff, val, mask); \
		 }	\
	} while (0)
...
#define DISP_REG_SET(handle,reg32, val) \
        do { \
             ...
             { \
                cmdqRecWrite(handle, (unsigned int)(reg32)&0x1fffffff, val, ~0); \
             }  \
        } while (0)
...
#define DISP_REG_SET_FIELD(handle,field, reg32, val)  \
        do {  \
             ...
             { \
               cmdqRecWrite(handle, (unsigned int)(reg32)&0x1fffffff, val<<REG_FLD_SHIFT(field), REG_FLD_MASK(field)); \
             } \
        } while (0)
