...
#define DSI_POLLREG32(cmdq, addr,mask,value)  \
	do{\
		cmdqRecPoll(cmdq,((unsigned int)addr)&0x1fffffff, value, mask);\
	}while(0);
...
void dsi_analysis(DISP_MODULE_ENUM module)
{
	...
	for(i = DSI_MODULE_BEGIN(module);i <= DSI_MODULE_END(module);i++)
	{
		DISPMSG("DSI%d Start:%x, Busy:%d, DSI_EN:%d, DSI_DUAL_EN:%d, MODE:%s, High Speed:%d, FSM State:%s\n", i, 
		...
		DISPMSG("DSI%d IRQ,RD_RDY:%d, CMD_DONE:%d, EXT_TE:%d, SLEEPOUT_DONE:%d, TE_RDY:%d, VM_CMD_DONE:%d, VM_DONE:%d\n", i,
	...
	{	
		...
		for(i = 0; i < 3; i++)
		{
			...
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_REG[dsi_i]->DSI_START, 1);
			        ...
			        ret = cmdqRecWrite(cmdq_trigger_handle, 0x1401b00c, 0x0, 0x00000001);
    		        }
			else // DSI1
			{			
