...
#define OD_DBG_DEBUG    2
   ...
   #define OD_REG_SET(reg32, val) \
                             do { \
                                  (*(volatile unsigned int*)(reg32)) = val; \
                             } while (0)
   ...
   #define OD_REG_SET_FIELD(reg32, val, field)  \
       do {  \
              (*(volatile unsigned int*)(reg32)) = ((*(volatile unsigned int*)(reg32) & (unsigned int)(~REG_FLD_MASK((unsigned int)(field)))) |  (unsigned int)(REG_FLD_VAL((unsigned int)(field), (unsigned int)(val))));  \
          } while (0)
   ...
   #define OD_REG_SET(reg32, val) do { if (g_od_cmdq_p) DISP_REG_SET(*g_od_cmdq_p, reg32, val); else DISP_REG_SET(NULL, reg32, val); } while (0)
       
   #define OD_REG_SET_FIELD(reg32, val, field) do { if (g_od_cmdq_p) DISP_REG_SET_FIELD(*g_od_cmdq_p, (field), (reg32), (val)); else DISP_REG_SET_FIELD(NULL, (field), (reg32), (val)); } while (0)
       
   #define OD_REG_BEGIN(cmdq) do { cmdq ? g_od_cmdq_p = (cmdqRecHandle*)(&cmdq) : od_create_cmdq(); } while (0)
   #define OD_REG_END(cmdq) do { if ((cmdq == NULL) && g_od_cmdq_p) cmdqRecFlush(*g_od_cmdq_p); g_od_cmdq_p = NULL; } while (0)
...
/// init table
static const REG_INIT_TABLE_STRUCT OD_INIT_TABLE[]=
{
    {OD_REG00, 0x82040201, 0xffffffff},
    // {OD_REG01, 0x00071401, 0x000000ff},
    {OD_REG01, 0x00070E01, 0x000000ff},
    ...
    {OD_REG38, 0x011F1013, 0xffffffff}, //line size=0xf8 | (0x37 << 13)
    ...
    {OD_REG49, 0xC5C00000, 0xC7C00000},
...
static UINT32 g_od_buf_size = 0;
static void *g_od_buf_pa = NULL;
...
static void _od_set_dram_buffer_addr(int manual_comp, int image_width, int image_height)
{
    ...
    if (!is_inited)         
    {
        ...
        va = dma_alloc_coherent(NULL, u4ODDramSize + OD_GUARD_PATTERN_SIZE, &dma_addr, GFP_KERNEL);
        ...
        ODDBG(OD_DBG_ALWAYS, "OD: pa %08x size %d order %d va %08x\n", (UINT32)u4ODAddress, u4ODDramSize, get_order(u4ODDramSize), (UINT32)va);
                                
        is_inited = 1;
        
        g_od_buf_size = u4ODDramSize;
        g_od_buf_pa = (void*)u4ODAddress;
        g_od_buf_va = (void*)va;
        
        /* set guard pattern */            
        *((UINT32*)((UINT32)va + u4ODDramSize)) = OD_GUARD_PATTERN;
        
    } else {
        // TODO: FIX ME, 64-bit needs to use VA */
        ...
        ODDBG(OD_DBG_ALWAYS, "OD inited: pa %08x size %d order %d va %08x\n", (UINT32)u4ODAddress, u4ODDramSize, get_order(u4ODDramSize), (UINT32)g_od_buf_va);
    }
...
static void _od_set_param(int manual_comp, int image_width, int image_height)
{
    ...
    OD_REG_SET_FIELD(OD_REG42, (100*image_height)/1152 , GM_LGMIN_DIFF);
    OD_REG_SET_FIELD(OD_REG44, (400*image_height)/1152 , GM_LMIN_THR);
    OD_REG_SET_FIELD(OD_REG44, (200*image_height)/1152 , GM_GMIN_THR);
...
static void od_dbg_dump(void)
{
    ODDBG(OD_DBG_ALWAYS, "OD EN %d INPUT %d %d\n", OD_REG_GET(DISP_OD_EN), OD_REG_GET(DISP_OD_INPUT_COUNT) >> 16, OD_REG_GET(DISP_OD_INPUT_COUNT) & 0xFFFF);    
...
{            
    ...
    gp = *((UINT32*)((UINT32)g_od_buf_va + g_od_buf_size));
...
void disp_config_od(unsigned int width, unsigned int height, void *cmdq, unsigned int od_table_size, void *od_table)
{
    ...
    ODDBG(OD_DBG_ALWAYS, "OD conf start %x %x %x\n", (unsigned int)cmdq, od_table_size, (unsigned int)od_table);
...
int disp_od_ioctl(DISP_MODULE_ENUM module, int msg, unsigned long arg, void *cmdq)
{
    ...
    ODDBG(OD_DBG_ALWAYS, "OD ioctl cmdq %x\n", (unsigned)cmdq);
...
{
    printk("ddp_bypass_od\n");
    DISP_REG_SET(handle, DISP_REG_OD_SIZE, (width << 16) | height);

    // do not use OD relay mode (dither will be bypassed)
    // od_core_en = 0
...
static int od_config_od(DISP_MODULE_ENUM module, disp_ddp_path_config* pConfig, void* cmdq)
{
...
#ifdef MTK_OD_SUPPORT
	{
		...
		if ((lcm_param->type == LCM_TYPE_DSI) && (lcm_param->dsi.mode == CMD_MODE))
		{
			...
			disp_config_od(pConfig->dst_w, pConfig->dst_h, cmdq, pConfig->dispif_config.od_table_size, pConfig->dispif_config.od_table);
		}
	}
#else
	ddp_bypass_od(pConfig->dst_w, pConfig->dst_h, cmdq);
...
{
    enable_clock(MT_CG_DISP0_DISP_OD, "DDP");
...
{
    disable_clock(MT_CG_DISP0_DISP_OD , "DDP");
