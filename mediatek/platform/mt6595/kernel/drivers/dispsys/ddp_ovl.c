		...
		void *handle)
{
	...
	DDPDBG("ovl%d, layer=%d, off(x=%d, y=%d), dst(%d, %d, %d, %d),pitch=%d,"
		"fmt=%s, addr=%lu, keyEn=%d, key=%d, aen=%d, alpha=%d,sur_aen=%d,sur_alpha=0x%x\n",
		idx,
		layer,
		src_x,
		src_y,
			...
			void *handle)
{
	...
	if(handle) {
				...
				//{
					...
					cmdqRecPoll(handle, (DISP_REG_OVL_STA + offset)& 0x1fffffff, 0, 0x1);
				//}
			}
	...
	void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	if (state == CMDQ_CHECK_IDLE_AFTER_STREAM_EOF) {
		if (module == DISP_MODULE_OVL0) {
			...
			ret = cmdqRecPoll(cmdq_trigger_handle,
				(reg_pa+DISP_OVL_INDEX_OFFSET), 2, 0x3f);
		} else {
