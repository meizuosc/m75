...
#ifdef MT6290
#define AUTOK_PRINT(_fmt, args...)        pr_debug("[AUTO_K]" _fmt, ## args)
#else
                ...
                case RD_SCAN_PAD_BOUND_E:
                    if (filter) {
                        if (/*(prAutok_raw_scan->Reg1Cnt <= AUTOK_TUNING_INACCURACY) &&*/
                            ...
                            printk("[WARN] Try to filter the holes on raw data when CKGEN=%d\r\n", 
                ...
                case RD_SCAN_PAD_BOUND_E_2:
                    ...
                    if (filter) {
                        ...
                        /* In case of the hole locates near region1 */
                            ...
                            printk("[ERR] Find holes on raw data when CKGEN=%d, but can NOT filter! \r\n", 
                                prAutok_raw_scan->CurCKGEN);
                        ...
                        printk("[WARN] Find too much fail regions when CKGEN=%d(Invalid)\r\n", 
                            prAutok_raw_scan->CurCKGEN);
        ...
        if (!fDoubleChk) {
            ...
            if (CMDPadScan0.Reg1Cnt && !CMDPadScan0.Reg2Cnt &&
                ...
                if (CMDPadScan0.BoundReg1_S < AUTOK_CMDPAT_CHK_SHIFT) {
                    ...
                    printk("1 Fail Region, Near left boundary\r\n");
                }
            }
            ...
            if (CMDPadScan0.Reg1Cnt && CMDPadScan0.Reg2Cnt) {
                ...
                if (CMDPadScan0.BoundReg1_S < AUTOK_CMDPAT_CHK_SHIFT) {
                    printk("2 Fail Regions, Near left boundary\r\n");
                ...
                /* If the two region are nearby */
                    ...
                    printk("2 Fail Regions, nearby each other\r\n");
    ...
    P_AUTOK_CMDMAT_CHAR_POS prMatChar)
{
    ...
    if (!raw_scan.Reg1Cnt && !raw_scan.Reg2Cnt)
        printk("Can NOT find pad transition boundary!\r\n");
    ...
    for (x=0; x<SCALE_CMDMAT_RSP_DLY_SEL; x++) {
        ...
        if (x == 0) {
            ...
            if (!rRawScan.Reg1Cnt && !rRawScan.Reg2Cnt) {
                printk("Can NOT find transition boundary at INIT, quit the loop!\r\n");
                break;
            }
        } else {
    ...
    while(k <= rPadDlyRes.CKGenPeriodLen) {
        if ((!fRdatPatFound || !rPadDlyRes.fHoleCK) && !fStopRDAT) {
            ...
            if((reTuneCmd == 1) && (reTuneCmdCnt < 2)) {
                printk("[ERR] Re-start to tune CMD again!!\r\n");
                reTuneCmdCnt++;
                k = 0;
                continue;
            }
                            ...
                            if ((cross > 1) && (cross < (AUTOK_CMDMAT_CROSS_MAR - FakeCnt))) {
                                if (pass) {
                                    ...
                                    if (FakeCnt >= AUTOK_CMDMAT_CROSS_MAR) {
                                        printk("[ERR]Too many fake points!\r\n");
                                        AUTOK_ERR();
                                    }
                                }
                            }
...
CycDoubleChk:
    ...
    if (pad_delay_period_cycle == 0) {
        ...
        if (fReScanCycle) {
            ...
            printk("[ERR]Still can NOT find pad delay cycle anyway!\r\n");
            AUTOK_ERR();
        }
    }
...
ReTuneCMDPad:
    ...
    if (stg == TUNING_STG1) {
        ...
        if ((CMDIntMargin == AUTOK_SKIP_CMDTUNE_ON_STG2)) {
            printk("[WARN] The internal delay is not stable enough, skip the CMD tune!\r\n");
        ...
        if (CMDIntMargin) {
            ...
            printk("[WARN] The internal delay may NOT be stable enough! Reduce the range.\r\n");
        }
        else
    ...
    if (stg == TUNING_STG1) {
         ...
         */
            ...
            printk("[WARN] Can NOT find the best CMD pad delay, the timing between CMD & RDAT differs too much!\r\n");
            /* 
        ...
        /* If still can NOT find fail region after full range scan */
            ...
            printk("[ERR] Undefined scenario when scan CMD pad delay!!!\r\n");
            AUTOK_ERR();
        }
            ...
            if (CMDPadScanEdge) {
                ...
                if (CMDIntPassWin >= 32) {
                    ...
                    /* Can NOT find best margin, but try best */
                        ...
                        printk("[ERR]Should NOT goto here in CMD pad scan!!!\r\n");
                }
                else {
         ...
         */
            ...
            printk("[WARN] Temperature shift on stage2 exceeds our expectation! Scan rising edge again!\r\n");
        ...
        /* The scan can NOT cover the boundary */
                ...
                ((CMDDlyScanE - PadDlyCMDRef) < CMDScanRangeStg2))) {
            printk("[WARN] Can NOT find the suitable CMD pad delay! the temp shift is too much!!!\r\n");
            CMDDlySel = PadDlyCMDRef;
            rCmdTuneRes.fTimingShiftLarge = 1;
        }
        else {
        ...
        if (data.score == 32) {
            ...
            if (CMDIntPassWin >= 32) {
                ...
                /* Can NOT find best margin, but try best */
                    ...
                    printk("[ERR]Should NOT goto here in CMD internal scan!!!\r\n");
                    AUTOK_ERR();
                }
            }
            else {
    ...
    findLBound:
            ...
            if ((LeftBoundCnt >= 2) && (x < (LeftBoundCnt - 1))) {
                ...
                if (AUTOK_TUNING_INACCURACY < ABS_DIFF(bound_diff,
                    ...
                    printk("[WARN] The left Pad boundary NOT correct! Find again!\r\n");
                    x++;
                    goto findLBound;
                }
            } else {
                printk("[WARN] The left Pad boundary may NOT be correct!\r\n");
            }
            ...
            printk("[WARN] Can NOT find left pad boundary!!!\r\n");
        }
        ...
        if (FBoundFound) {
            ...
            if (FBoundFound == 2) {
                ...
                if (3 < ABS_DIFF(bound_diff, FBoundCnt))
                    printk("[WARN] The full Pad boundary may NOT correct!\r\n");
            }
            ...
            printk("[WARN] Can NOT find full pad boundary directly!!!\r\n");
        ...
        if (!FBoundCnt/* && !LBoundPad*/) {
            printk("[WARN] Can NOT find neither left and full pad boundary !\r\n");
    ...
    ChngBaseCK:
        ...
        if (!autok_rdata_scan[baseCKGEN].BoundReg1_S && 
            ...
            if (RealCKGEN > CKGENMax) {
                printk("[WARN] Exceeds the maximun CKGEN range[%d]\r\n", CKGENMax);
         ...
         */
            ...
            printk("[WARN] Timing is compressed by too large output delay(CK=%d)\r\n", baseCKGEN);
            baseCKGEN++;

            if (baseCKGEN <= CKGENMax)
                goto ChngBaseCK;
            else {
                printk("[ERR] Exceeds the Max. CKGEN limitation!\r\n");
                AUTOK_ERR();
            }   
        }
        /* In case of Right boundary at CKGEN = 0 */
            ...
            /* In case of un-enough margin */
                ...
                printk("[WARN] Un-enough margin to right boundary! Check layout!\r\n");
            ...
            if (RealCKGEN > CKGENMax) {
                printk("[WARN] Exceeds the maximun CKGEN range[%d]\r\n", CKGENMax);
                printk("Maybe NOT the best point due to CKGEN limit!\r\n");
        ...
        if (autok_rdata_scan[RealCKGEN].Reg1Cnt && 
            ...
            printk("[WARN] Try to select the middle score!\r\n");
            ...
            if (gfIOSS && gfCoreTT && (IntBound < 32)) {
                ...
                if (TinyMar > AUTOK_TINY_MAR_TH_BTM) {
                    ...
                    printk("[WARN] Tiny margin(%d), shift the pad delay(%d) to right\r\n", 
        ...
        /* In case of right boundary by shifting CKGEN */
             ...
             */
               ...
               ((autok_rdata_scan[RealCKGEN].BoundReg1_S - RealPadDelay) <= 
                ...
                printk("[WARN] The margin may NOT up to T/2 due to internal boundary\r\n");
                ...
                if (IntBound > RealPadDelay) {
                    ...
                    printk("[WARN] The internal boundary is too large!\r\n");
        ...
        if (((RealPadDelay >= IntBound) && IntBound) || (RealPadDelay >= 32)) {
            printk("[ERR] Invalid pad delay(%d): exceeds the range\r\n", RealPadDelay);
            AUTOK_ERR();
        }
        if (autok_rdata_scan[RealCKGEN].RawData & (unsigned int)(1 << RealPadDelay)) {
            printk("[ERR] Invalid pad delay(%d): NOT within the pass window\r\n", RealPadDelay);
            AUTOK_ERR();
        }
        ...
        if(reTuneCmd == 1) {
            printk("[ERR] CMD CRC error in tuning read!\r\n");
            AUTOK_ERR();
        }
        ...
        /* In case of margin is NOT enough */
            ...
            printk("[WARN]Read data margin is NOT enough! Check layout!!!\r\n");
...
autok_doub_chk_wdat_int_bound(struct msdc_host *host, unsigned int *pRaw)
{
    ...
    *pRaw = 0;
    ...
    for (idx = rDatPattern.BoundReg1_S; 
        ...
        if(reTuneCmd == 1) {
            printk("[Err] CMD CRC error in tuning write!!\r\n");
            AUTOK_ERR();
        }
    ...
    if (data.score == 32) {
        ...
        if (WRIntPassWin >= 32) {
            ...
            /* Can NOT find best margin, but try best */
                ...
                printk("[ERR]Should NOT goto here in WR internal scan!!!\r\n");
                AUTOK_ERR();
            }
        }
        else {
...
static void autok_get_reg_field(unsigned int param, unsigned int *field)
{
    switch (param) {
        ...
        default:
            printk("[ERR]Can NOT find the delay cell field!!!\r\n");
            *field = 0;
        break;
    }
}
    ...
    struct autok_predata *p_single_autok)
{
    ...
    if ((p_single_autok == NULL) || (host == NULL)) {
        printk("NULL autok param pointer on STG1!\r\n");
        return -1;
    }
    ...
    if (!res)
        ...
        printk("[ERR]msdc_autok_stg1_cal returns %d\r\n", res);
        goto exit;
    }
    ...
    unsigned int vcore_uv_off)
{
    ...
    if (p_autok_data == NULL || host == NULL) {
        printk("NULL autok param pointer on STG2!\r\n");
        return -1;
    }
    ...
    if (autok_vcore_scan_num > AUTOK_VCORE_SCAN_NUM) {
        printk("Exceeds the Vcore scan number range\r\n");
        return -1;
    }
    for (idx = 0; idx < autok_vcore_scan_num; idx++) {
    ...
    if((pAutok + E_AUTOK_FREQ)->freq != host->mclk) {
        printk("Now operation freq(%d) not meet autok freq(%d)\r\n", 
    ...
    if (!res)
        ...
        printk("[ERR]msdc_autok_stg2_cal returns %d\r\n", res);
