...
{
    printk(KERN_EMERG "=============== AXI BUS INFO ============="); 
    printk(KERN_EMERG "reg[0xf0001224]=0x%x", sdr_read32(0xf0001224)); 
    printk(KERN_EMERG "reg[0xf0201000]=0x%x", sdr_read32(0xf0201000)); 
    printk(KERN_EMERG "reg[0xf0201018]=0x%x", sdr_read32(0xf0201018)); 
    printk(KERN_EMERG "reg[0xf000320c]=0x%x", sdr_read32(0xf000320c)); 
    printk(KERN_EMERG "reg[0xf0003210]=0x%x", sdr_read32(0xf0003210)); 
    printk(KERN_EMERG "reg[0xf0003214]=0x%x", sdr_read32(0xf0003214)); 
}
...
static void dump_emi_info(void)
{
    ...
    printk(KERN_EMERG "=============== EMI INFO ============="); 
    printk(KERN_EMERG "before, reg[0xf02034e8]=0x%x", sdr_read32(0xf02034e8));  
    printk(KERN_EMERG "before, reg[0xf0203400]=0x%x", sdr_read32(0xf0203400));    
    sdr_write32(0xf02034e8, 0x2000000);
    sdr_write32(0xf0203400, 0xff0001);
    printk(KERN_EMERG "after, reg[0xf02034e8]=0x%x", sdr_read32(0xf02034e8));  
    printk(KERN_EMERG "after, reg[0xf0203400]=0x%x", sdr_read32(0xf0203400)); 
    
    for(i = 0; i < 5; i++){
        for(addr = 0; addr < 0x78; addr += 4){
            printk(KERN_EMERG "reg[0x%x]=0x%x", (0xf0203500 + addr), sdr_read32((0xf0203500 + addr))); 
            if(addr % 0x10 == 0) 
                mdelay(1);
        }
    }
...
{
    int i = 0;
    while(sdr_read32(0xf0003214) & 0xc) {
       	if(++i < 300){
       	    mdelay(10);
       	}else {
       	    printk(KERN_EMERG "[%s]: check peri-bus: 0x%x at %d\n", __func__,  sdr_read32(0xf0003214), line);        
       	    printk(KERN_EMERG "######################## AXI bus hang! start #############################");
       	    printk(KERN_EMERG "======EMI======"); 
       	    dump_emi_info();
       	    mdelay(10);
       	    printk(KERN_EMERG "======AXI======"); 
       	    dump_axi_bus_info();
       	    mdelay(10);
       	    printk(KERN_EMERG "======AUDIO======"); 
       	    dump_audio_info();
       	    mdelay(10);
       	    printk(KERN_EMERG "======MSDC======"); 
       	    msdc_dump_info(0);
       	    mdelay(10);
       	    printk(KERN_EMERG "======GPD/BD======"); 
       	    msdc_dump_gpd_bd(0); 
       	    printk(KERN_EMERG "######################## AXI bus hang! end #############################");
       	    if(dead == 0)
       	        break;
       	    else 
       	        i = 0;
...
static void msdc_sd_power(struct msdc_host *host,u32 on)
{
    ...
    pwrap_read( 0x54e, &ldo_en );
    pwrap_read( 0x566, &ldo_vol );
    printk(KERN_ERR "[%s]: msdc%d, VMC_EN[0x54e][bit10=1]=0x%d, VMC_VOL[0x566][bit4~6=1]=0x%x\n", __func__, host->id, ldo_en, ldo_vol);
    pwrap_read( 0x550, &ldo_en );
    pwrap_read( 0x568, &ldo_vol );
    printk(KERN_ERR "[%s]: msdc:%d, VMCH_EN[0x550][bit10=1]=0x%d, VMCH_VOL[0x568][bit6=1]=0x%x\n", __func__, host->id, ldo_en, ldo_vol);
...
static int ot_thread_func(void *data)
{
    ...
    {
        pr_debug("[SDIO_OnlineTuning][%s] sdio card is not ready\n", __func__);
		if(chg_volt)
		{
		    host->ot_work.chg_volt = 0;
			atomic_set(&host->sdio_stopping, 0);
    ...
    {
        pr_debug("[SDIO_OnlineTuning][%s] Non SDIO device enter msdc_online_tuning!!!! \n", __func__);
		if(chg_volt)
		{
		    host->ot_work.chg_volt = 0;
			atomic_set(&host->sdio_stopping, 0);
    ...
    if(chg_volt == 0)
    {
        ...
        pr_debug("[SDIO_OnlineTuning][%s] cur_temperature = %d, host->pre_temper = %d\n", __func__, cur_temperature, host->pre_temper);
...
{
    pr_debug("[%s]: msdc%d change clk_src from %dKHz to %dKHz\n", __func__, host->id, (host->hclk/1000), (hclks[clksrc]/1000));
...
static void msdc_set_mclk(struct msdc_host *host, unsigned int state, u32 hz)
{
    ...
    if (!hz) { // set mmc system clock to 0
        pr_debug("msdc%d -> set mclk to 0",host->id);
    ...
    pr_debug("msdc%d -> !!! Set<%dKHz> Source<%dKHz> -> sclk<%dKHz> state<%d> mode<%d> div<%d> hs400_src<%d>" ,
...
static unsigned int ot_find_next_gear(u32 CRCResult, const u32 tune_wind_size, const u32 cur_gear, u32 *fTestedGear)
{
	...
	pr_debug("[SDIO_OnlineTuning][%s], cur_gear = 0x%x, new_gear = 0x%x, tune_wind_size = 0x%x, CRCResult = 0x%x\n", __func__, cur_gear, new_gear, tune_wind_size, CRCResult);
...
static int ot_command_resp_polling(struct msdc_host *host, struct mmc_command *cmd, int tune, unsigned long timeout, u32 *pintsts)
{
    ...
    while (1){
        ...
        if (time_after(jiffies, tmo)) {
            cmd->error = (unsigned int)-ETIMEDOUT;
            host->sw_timeout++;
            pr_debug("[SDIO_OnlineTuning][%s] SW timeout, intsts = 0x%x\n", __func__, intsts);
            msdc_dump_info(host->id);
            msdc_reset_hw(host->id);
            goto out;
        }
    ...
    if (intsts & flagTmo)
    {
        pr_debug("[SDIO_OnlineTuning]Device CMD/DAT timeout, MSDC_INT = 0x%x\n", intsts);
        cmd->error = (unsigned int)-ETIMEDOUT;
        msdc_reset_hw(host->id);
    }
...
static int ot_do_command(struct msdc_host *host, u32 rawcmd, u32 rawarg, u32 *pintsts)
{
    ...
    while(sdc_is_cmd_busy() || sdc_is_busy())
    {
        if (time_after(jiffies, tmo)) {
            if(sdc_is_cmd_busy())
                pr_debug("[SDIO_OnlineTuning]XXX sdc_cmd_busy timeout\n");
            else if(sdc_is_busy())
                pr_debug("[SDIO_OnlineTuning]XXX sdc_busy timeout\n");
            cmd->error = (unsigned int)-ETIMEDOUT;
            msdc_reset_hw(host->id);
    ...
    {
        pr_debug("[SDIO_OnlineTuning]ot_command_resp_polling fail, err = %d, resp = 0x%.8x\n", cmd->error, cmd->resp[0]);
        return cmd->error;
    }
...
_retry:
    ...
    if (time_after(jiffies, tmo))
    {
        pr_debug("[SDIO_OnlineTuning]online tuning timeout\n");
...
out:
    ...
    if(ret < 0)
    {
        ...
        pr_debug("[SDIO_OnlineTuning]ret = %d, vcore_uv = 0x%x, cmd delay = 0x%x\n", ret, vcore_uv, otData.orig_cmdrdly);
    }
    else if(otData.orig_ddlsel == 0)
    {
...
static struct msdc_reg_control hs400_backup_reg_list[HS400_BACKUP_REG_NUM] = {
    ...
    {(MSDC0_BASE + OFFSET_MSDC_PATCH_BIT0),        (MSDC_PB0_INT_DAT_LATCH_CK_SEL), 0x0, 0x0, NULL},
    {(MSDC0_BASE + OFFSET_MSDC_PATCH_BIT1),        (MSDC_PB1_WRDAT_CRCS_TA_CNTR), 0x0, 0x0, hs400_restore_pb1}, // the defalut init value is 0x1, but HS400 need the value be 0x0
    {(MSDC0_BASE + OFFSET_MSDC_IOCON),             (MSDC_IOCON_R_D_SMPL | MSDC_IOCON_DDLSEL | MSDC_IOCON_R_D_SMPL_SEL | MSDC_IOCON_R_D0SPL | MSDC_IOCON_W_D_SMPL_SEL | MSDC_IOCON_W_D_SMPL), 0x0, 0x0, NULL},
    {(MSDC0_BASE + OFFSET_MSDC_PAD_TUNE),          (MSDC_PAD_TUNE_DATWRDLY | MSDC_PAD_TUNE_DATRRDLY), 0x0, 0x0, hs400_restore_pad_tune},
    {(MSDC0_BASE + OFFSET_MSDC_DAT_RDDLY0),        (MSDC_DAT_RDDLY0_D3 | MSDC_DAT_RDDLY0_D2 | MSDC_DAT_RDDLY0_D1 | MSDC_DAT_RDDLY0_D0), 0x0, 0x0, hs400_restore_ddly0},
    {(MSDC0_BASE + OFFSET_MSDC_DAT_RDDLY1),        (MSDC_DAT_RDDLY1_D7 | MSDC_DAT_RDDLY1_D6 | MSDC_DAT_RDDLY1_D5 | MSDC_DAT_RDDLY1_D4), 0x0, 0x0, hs400_restore_ddly1},
    {(MSDC0_BASE + OFFSET_EMMC50_PAD_CMD_TUNE),    (MSDC_EMMC50_PAD_CMD_TUNE_TXDLY), 0x0, 0x00000200, hs400_restore_cmd_tune},
    {(MSDC0_BASE + OFFSET_EMMC50_PAD_DS_TUNE),     (MSDC_EMMC50_PAD_DS_TUNE_DLY1 | MSDC_EMMC50_PAD_DS_TUNE_DLY3), 0x0, 0x0, hs400_restore_ds_tune},
    {(MSDC0_BASE + OFFSET_EMMC50_PAD_DAT01_TUNE),  (MSDC_EMMC50_PAD_DAT0_TXDLY | MSDC_EMMC50_PAD_DAT1_TXDLY), 0x0, 0x01000100, hs400_restore_dat01_tune},
    {(MSDC0_BASE + OFFSET_EMMC50_PAD_DAT23_TUNE),  (MSDC_EMMC50_PAD_DAT2_TXDLY | MSDC_EMMC50_PAD_DAT3_TXDLY), 0x0, 0x01000100, hs400_restore_dat23_tune},
    {(MSDC0_BASE + OFFSET_EMMC50_PAD_DAT45_TUNE),  (MSDC_EMMC50_PAD_DAT4_TXDLY | MSDC_EMMC50_PAD_DAT5_TXDLY), 0x0, 0x01000100, hs400_restore_dat45_tune},
    {(MSDC0_BASE + OFFSET_EMMC50_PAD_DAT67_TUNE),  (MSDC_EMMC50_PAD_DAT6_TXDLY | MSDC_EMMC50_PAD_DAT7_TXDLY), 0x0, 0x01000100, hs400_restore_dat67_tune},
};
...
static void emmc_hs400_backup(void)
{
    ...
    {
        sdr_get_field(hs400_backup_reg_list[i].addr, hs400_backup_reg_list[i].mask,  hs400_backup_reg_list[i].value);
        if(hs400_backup_reg_list[i].restore_func){
            err = hs400_backup_reg_list[i].restore_func(0);
            if(err) {
                printk(KERN_ERR "[%s]: failed to restore reg[0x%x][0x%x], expected value[0x%x], actual value[0x%x] err=0x%x",
                        __func__, hs400_backup_reg_list[i].addr, hs400_backup_reg_list[i].mask, hs400_backup_reg_list[i].default_value, sdr_read32(hs400_backup_reg_list[i].addr), err);
            }
        }
    }
...
static void emmc_hs400_restore(void)
{
    ...
    for(i = 0; i < HS400_BACKUP_REG_NUM; i++)
    {
        sdr_set_field(hs400_backup_reg_list[i].addr, hs400_backup_reg_list[i].mask,  hs400_backup_reg_list[i].value);
        if(hs400_backup_reg_list[i].restore_func){
            err = hs400_backup_reg_list[i].restore_func(1);
            if(err) {
                printk(KERN_ERR "[%s]:failed to restore reg[0x%x][0x%x], expected value[0x%x], actual value[0x%x] err=0x%x",
                        __func__, hs400_backup_reg_list[i].addr, hs400_backup_reg_list[i].mask, hs400_backup_reg_list[i].value, sdr_read32(hs400_backup_reg_list[i].addr), err);
            }
        }
        pr_debug("[%s]:i:%d, reg=0x%x, value=0x%x\n", __func__, i, hs400_backup_reg_list[i].addr, sdr_read32(hs400_backup_reg_list[i].addr));
    }
...
static void msdc_apply_ett_settings (struct msdc_host *host, int mode) {
    ...
    for(i = 0; i < host->hw->ett_count; i++) {
        ett = (struct msdc_ett_settings *)((u32)host->hw->ett_settings + sizeof(struct msdc_ett_settings) * i);
...
static void msdc_ops_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
    ...
    if((msdc_host_mode[host->id] != mmc->caps) || (msdc_host_mode2[host->id] != mmc->caps2)) {
        ...
        if (1 == g_emmc_mode_switch){
            ...
            pr_debug(KERN_ERR "[%s]: msdc%d, ext_csd.card_type=0x%x\n", __func__, host->id, mmc->card->ext_csd.card_type);
        }
#endif
...
void msdc_dump_gpd_bd(int id)
{
    ...
    if(id < 0 || id >= HOST_MAX_NUM)
        printk("[%s]: invalide host id: %d\n", __func__, id); 
        
    host = mtk_msdc_host[id];
    if(host == NULL){
        printk("[%s]: host0 or host0->dma is NULL\n", __func__); 
        return; 
    }
    gpd = host->dma.gpd; 
    bd  = host->dma.bd;
    ...
    printk("==========================MSDC GPD INFO ===========================\n");     
    if(gpd == NULL){
        printk("GPD is NULL\n"); 
    }else {
        printk("gpd addr:0x%x\n", (u32)(host->dma.gpd_addr));
        printk("hwo:0x%x, bdp:0x%x, rsv0:0x%x, chksum:0x%x,intr:0x%x,rsv1:0x%x,next:0x%x\n", 
            gpd->hwo, gpd->bdp, gpd->rsv0, gpd->chksum, gpd->intr, gpd->rsv1, (unsigned int)gpd->next);           
        printk("ptr:0x%x, buflen:0x%x, extlen:0x%x, rsv2:0x%x, arg:0x%x,blknum:0x%x,cmd:0x%x\n", 
    ...
    if(bd == NULL){
        ...
        printk("bd addr:0x%x\n", (u32)(host->dma.bd_addr));
        for(i = 0; i < host->dma.sglen; i++){
            printk("the %d BD\n", i); 
            printk("    eol:0x%x, rsv0:0x%x, chksum:0x%x, rsv1:0x%x,blkpad:0x%x,dwpad:0x%x,\n", 
                bd->eol, bd->rsv0, bd->chksum, bd->rsv1, bd->blkpad, bd->dwpad);           
            printk("    rsv2:0x%x, next:0x%x, ptr:0x%x, buflen:0x%x, rsv3:0x%x\n", 
...
static void msdc_init_gpd_bd(struct msdc_host *host, struct msdc_dma *dma)
{
    ...
    gpd->ptr = (void *)((u32)dma->bd_addr); /* physical address */

    memset(bd, 0, sizeof(bd_t) * bdlen);
    ptr = bd + bdlen - 1;
    while (ptr != bd) {
        prev = ptr - 1;
        prev->next = (void *)((u32)dma->bd_addr + sizeof(bd_t) *(ptr - bd));
        ptr = prev;
    }
}
        ...
        if(is_card_sdio(mtk_msdc_host[hostidx]))
        {
            ...
            while(1)
            {
                if (atomic_read(&host->ot_done)) {
                    pr_debug("[%s] msdc%d prev. online tuning done, host->ot_work.chg_volt = %d\n", __func__, host->id, host->ot_work.chg_volt);
                    break;
                }
...
int sdio_start_ot_transfer(void)
{
	...
	count--;
	if(count != 0)
		pr_debug("[%s] sdio stop and start are not pairwise, count = %d\n", __func__, count);

    for(hostidx = 0; hostidx < HOST_MAX_NUM; hostidx++)
    {
        if(mtk_msdc_host[hostidx]==NULL)
        {
            pr_debug("[%s] mtk_msdc_host[%d] = NULL\n", __func__, hostidx);
            continue;
        }
        if(is_card_sdio(mtk_msdc_host[hostidx]))
        {
            ...
            if(vcore_sel == prev_vcore_sel) {
                pr_debug("[%s] voltage doesn't change\n", __func__);
                atomic_set(&host->sdio_stopping, 0);
                complete(&host->ot_work.ot_complete);
                autok_release_host(host);
                continue;
...
/*
    ...
    printk(KERN_ERR "[%s]: msdc%d, pll[0xf020925c][bit0~1=0x1]=0x%x, pll[0xf0209250][bit0=0x1]=0x%x, mux[0xf0000070][bit15,23,31=0x1]=0x%x, clk[0xf0003018][bit13=msdc0, bit14=msdc1]=0x%x\n",
                     __func__, host->id, sdr_read32(0xf020925c), sdr_read32(0xf0209250),  sdr_read32(0xf0000070),  sdr_read32(0xf0003018));
#endif
