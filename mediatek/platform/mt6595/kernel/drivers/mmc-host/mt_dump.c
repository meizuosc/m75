...
static u32 u_msdc_base[HOST_MAX_NUM] = {MSDC0_BASE, MSDC1_BASE, MSDC2_BASE, MSDC3_BASE};
...
static void msdc_mdelay(u32 time)
{
   ...
   while(t_end > msdc_read_timer()) cpu_relax();
...
static void simp_msdc_dump_register(struct simp_msdc_host *host)
{
    ...
    if(host->id == 0) {
        ...
        printk(KERN_EMERG "R[%x]=0x%x  R[%x]=0x%x\n", 
...
{
    /* step1: open pll */
    clk_setl(0xF020925C, 0x1);
    msdc_mdelay(1);
    clk_clrl(0xF020925C, 0x2);
    
    clk_setl(0xF0209250, 0x1);
    msdc_mdelay(1);

    /* step2: enable mux */
    mt_reg_sync_writel(0x02060301, 0xF0000070); 
    
    /* step3: enable clock */
    clk_setl(0xF0003010, 0x6000);    
#if MTK_MMC_DUMP_DBG
    printk(KERN_EMERG "[pll]0xF020925C =0x%x, 0xF0209250 =0x%x, [mux]0xF0000070 =0x%x, [clk]0xF0003018 =0x%x\n", 
    		sdr_read32(0xF020925C), sdr_read32(0xF0209250), sdr_read32(0xF0000070), sdr_read32(0xF0003018)); 
#endif
...
*/
    switch (host->id) {
        case 0:
            sdr_set_field(MSDC0_GPIO_CLK_BASE, GPIO_MSDC_DRV_MASK, msdc0_hw.clk_drv); /* feifei.wang -- not use hardcode here */
            ...
            //printk(KERN_EMERG "host0 clk= %d, cmd= %d, dat= %d, rst=%d, ds=%d.\r\n", msdc0_hw.clk_drv, msdc0_hw.cmd_drv, msdc0_hw.dat_drv, msdc0_hw.rst_drv, msdc0_hw.ds_drv);
            break;
        case 1:
            #if defined(CFG_DEV_MSDC1)
            sdr_set_field(MSDC1_GPIO_CLK_BASE, GPIO_MSDC_DRV_MASK, msdc1_hw.clk_drv); /* feifei.wang -- not use hardcode here */
