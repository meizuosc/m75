...
#define MTK_GPIO_REG                    0xF3FFF030
...
int gpu_pm_restore_noirq(struct device *device)
{
    mt_irq_set_sens(SYS_MTK_RGX_IRQ, MT_LEVEL_SENSITIVE);
    mt_irq_set_polarity(SYS_MTK_RGX_IRQ, MT_POLARITY_LOW);
    return 0;
}
#endif

static PVRSRV_RGXDEV_INFO* MTKGetRGXDevInfo(IMG_VOID)
{
...
#ifdef MTK_DEBUG
        xlog_printk(ANDROID_LOG_INFO, "PVR_K", "3DFreq=%d, Volt=%d\n", ui32GPUFreq, mt_gpufreq_get_cur_volt());
#endif
...
static IMG_UINT32 MTKCalPowerIndex(IMG_VOID)
{
    ...
    DRV_WriteReg32(MTK_GPIO_REG, DRV_Reg32(MTK_GPIO_REG) | 0x1);
    ...
    {
        if (0x10000 & DRV_Reg32(MTK_GPIO_REG))
        {
            bTimeout = IMG_FALSE;
            break;
        }
    ...
    ui32State = DRV_Reg32(MTK_GPIO_REG) >> 24;

    //read POWER_ESTIMATE_RESULT
    ui32Result = DRV_Reg32(0xF3006328);

    //writes 1 to GPIO_OUTPUT_ACK, bit[17] 
    DRV_WriteReg32(MTK_GPIO_REG, DRV_Reg32(MTK_GPIO_REG)|0x20000);
...
{
    PVRSRV_RGXDEV_INFO* psDevInfo = MTKGetRGXDevInfo();
    if (psDevInfo && psDevInfo->pfnGetGpuUtilStats)
    {
        ...
        if (sGpuUtilStats.bPoweredOn)
        {
            ...
            *pui32Loading = sGpuUtilStats.ui32GpuStatActive/100;
...
module_param(gpu_block, ulong, 0644);
module_param(gpu_idle, ulong, 0644);
module_param(gpu_power, ulong, 0644);
module_param(gpu_dvfs_enable, ulong, 0644);
module_param(boost_gpu_enable, ulong, 0644);
module_param(gpu_debug_enable, ulong, 0644);
module_param(gpu_dvfs_force_idle, ulong, 0644);
module_param(gpu_dvfs_cb_force_idle, ulong, 0644);
module_param(gpu_bottom_freq, ulong, 0644);
module_param(gpu_cust_boost_freq, ulong, 0644);
module_param(gpu_cust_upbound_freq, ulong, 0644);
module_param(gpu_freq, ulong, 0644);
