    ...
    if(g_da9210_driver_ready==1)
    {
        if(g_da9210_hw_exist==1)
        {
           ...
           do{
                 ...
                 da9210_read_interface(0x5D,&data,0x1,0);
                 if(data == en_bit)
                     break;
                 da9210_config_interface(0x0, 0x1, 0x1, 7); // page reverts to 0 after one access
                 da9210_config_interface(0x0, 0x2, 0xF, 0); // select to page 2,3
                 da9210_read_interface(0x5,&data,0xF,4);
                 printk("%s(), da9210 ID = %x\n", __func__, data);
            }while(--try_count);
            ///@}
...
{   
    ...
    else
    {
        /* turn on ca15l vsram */
		pmic_read_interface(0x63C, &reg_val,0x3,13);
		if (ca15_vsram_type < 0) {
			do{
                for(i=0; i<5; i++)
                    pmic_read_interface(0x63C,&efuse_data[i],0x3,13);
                for(i=0; i<4; i++){
                    if(efuse_data[i] != efuse_data[i+1])
                        ret = 1;
                }
            }while(--try_count && ret);
            ca15_vsram_type = efuse_data[0];
        }
        reg_val = ca15_vsram_type;
        ///@}d_interface(0x63C,&reg_val,0x3,13);
...
static ssize_t store_da9210_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    if(buf != NULL && size != 0)
    {
        xlog_printk(ANDROID_LOG_INFO, "Power/PMIC","[store_da9210_access] buf is %s and size is %d \n",buf,size);
