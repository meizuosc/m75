...
void swchr_hw_init(void)
{
    ...
    //Reg[0x806C]   
    mt6332_upmu_set_rg_ch_complete_det_off(0);//charge complete detection enable  
    mt6332_upmu_set_rg_ch_complete_pwm_off(1);
    mt6332_upmu_set_rg_ch_complete_m3_off(0);
    //Reg[0x803A]
    mt6332_upmu_set_rg_iterm_sel(0x0);            //[15:13] terminater charger 200mA
...
// chr_plug_out_sw_detect    
    ...
    if(upmu_is_chr_det()==1) //sync USB device/otg state
    {
        ...
        if(cv_val == 1)
        {
            ...
            //pmic_config_interface(0x8074, 0x1, 0x1, 9);//MEIZU_BSP tangxingyan2014/09/09
