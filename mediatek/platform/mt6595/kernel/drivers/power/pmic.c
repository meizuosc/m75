...
EXPORT_SYMBOL(upmu_get_rgs_chrdet);
    ...
    if(mt6331_upmu_get_pwrkey_deb()==1)                
    {
        ...
        if(g_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT && timer_pre != 0)
        {
                ...
                if(long_pwrkey_press)   //500ms
                {
                    ...
                    //arch_reset(0, NULL);  // do not reset
                }
        }        
        #endif
...
void chrdet_int_handler(void)
{
    ...
    if (!upmu_get_rgs_chrdet())
    {
        ...
        ///MEIZU_BSP{@
        /*Unplug Charger/USB in meta mode, Shutdown OS!*/
        if(boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT || boot_mode == LOW_POWER_OFF_CHARGING_BOOT 
            || boot_mode ==  META_BOOT || boot_mode == ADVMETA_BOOT)
        {
...
void dct_pmic_VCAMD_sel(kal_uint32 volt)
{
    ...
    else if(volt == 1200)    {mt6331_upmu_set_rg_vcamd_vosel(3); mt6331_upmu_set_rg_vcamd_cal(1);} //1.2v
...
static int __init pmic_mt_init(void)
{
    ...
    //if(get_mt6331_pmic_chip_version() == 0x3110)
//        mt6331_upmu_set_rg_vgp2_ndis_en(0);
