...
CHARGER_TYPE tsu_charger_type = CHARGER_UNKNOWN;
...
/**********************************************************
  ...
  *********************************************************/
#if 0
static int bq24196_notifier_event(struct notifier_block *this, unsigned long event, void *ptr)
{
	tsu_charger_type = event;
	return event;
}
#endif
//CON0----------------------------------------------------
...
kal_uint32 bq24196_get_charger_type(void)
{
    kal_uint32 vbus_stat = 0;
    vbus_stat = bq24196_get_vbus_stat();

    tsu6721_charger_type(&tsu_charger_type);

    if (vbus_stat && !tsu_charger_type) {
	if (vbus_stat == 1)
	    tsu_charger_type = STANDARD_HOST; 
	else if (vbus_stat == 2)
	    tsu_charger_type = STANDARD_CHARGER;
    }
	return tsu_charger_type;	
}
...
#if 0
    charger_notifier.notifier_call = bq24196_notifier_event;
    tsu6721_notifier_chain_register(&charger_notifier);
#endif
    return 0;                                                                                       
