...
//  I2CERR("first:khz=%d,mode=%d sclk=%d,min_diff=%d,max_step_cnt_div=%d\n",khz,mode,sclk,min_diff,max_step_cnt_div);
  ...
  if (mode == HS_MODE) {
	...
	if(0 == i2c->timing_reg)
	{
		I2CLOG("hs base address 0x%x,tmp =0x%x\n",i2c->base,tmp);
	...
	if(0 == i2c->timing_reg)
	{
		I2CLOG("n-hs base address 0x%x, tmp=0x%x, sample_cnt_div=0x%x, step_cnt_div=%x\n",i2c->base,tmp,sample_cnt_div,step_cnt_div);
		//aee_kernel_warning(TAG, "@%s():%d, \n", __func__, __LINE__);
	}
    //I2C_TIMING_REG_BACKUP[i2c->id]=tmp;
    //i2c_writel(i2c, OFFSET_TIMING, tmp);
...
void _i2c_dump_info(mt_i2c *i2c)
{
  ...
  /*clock*/
  I2CLOG("Clock stat(0x10003018) is 0x%08x, %s\n",__raw_readl((void*)0xF0003018), (__raw_readl((void*)0xF0003018)>>23) & (1 << i2c->id)?"disable":"enable");
...
static void _i2c_write_reg(mt_i2c *i2c)
{
  ...
  if(i2c->dma_en)
  {
    ...
    //dma warm reset
	mt65xx_reg_sync_writel(0x0001, i2c->pdmabase + OFFSET_RST);
...
static void mt_i2c_power_ctl(int id,int val)
{
	...
	pr_debug("mt_i2c(%d)_power_ctl (%d) \n ",id,val);
	switch(id)
	{
		case 0:
			if(0 == val)
			{
				pr_debug("mt_i2c(%d)_power_ctl disable \n ",id);
				//mt6331_upmu_set_rg_vcam_io_en(0);
				hwPowerDown(MT6331_POWER_LDO_VCAM_IO,"I2C_DRV");
			}
			else if(1 == val)
			{
				pr_debug("mt_i2c(%d)_power_ctl enable \n ",id);
