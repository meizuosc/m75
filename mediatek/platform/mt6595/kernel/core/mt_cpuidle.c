...
#if 1 // !defined (MTK_FORCE_CLUSTER1)
...
#endif //#if !defined (MTK_FORCE_CLUSTER1)

#if defined (MTK_FORCE_CLUSTER1)
	if ((cluster_id() == 0) && 
...
__naked void cpu_resume_wrapper(void)
{
	...
	if (likely(is_cpu_type(CA7_TYPEID))) {
		 ...
		 * rg_acinactm	Prevent L2 memory from accepting any new request.
		 **/
		val = reg_read(NOMMU(CA7_BUS_CONFIG));
		val = _and(val, ~(1<<4));
		reg_write(NOMMU(CA7_BUS_CONFIG),  val);

		//able to issue dvm/snoop request from this interface?
		val = reg_read(NOMMU(CCI400_CA7_SNOOP_CTLR));
		val = _or(val, 0x3);
		reg_write(NOMMU(CCI400_CA7_SNOOP_CTLR),  val);
	...
	if (likely((cluster_id() == 0) && 
		...
		//ACINACTM & AINACTS
		val = reg_read(NOMMU(CA15L_MISCDBG));
		val = _and(val, ~(0x11));
		reg_write(NOMMU(CA15L_MISCDBG),  val);

                ver = get_data_nommu(dormant_data[0].poc.chip_ver);
                if (CHIP_SW_VER_02 <= ver) {
                        //able to issue dvm/snoop request from this interface ?
                        val = reg_read(NOMMU(CCI400_CA15L_SNOOP_CTLR));
                        val = _or(val, 0x3);
                        reg_write(NOMMU(CCI400_CA15L_SNOOP_CTLR),  val);
                        while (reg_read(NOMMU(CCI400_STATUS)) & 0x1);
                }
                /******************************************/
