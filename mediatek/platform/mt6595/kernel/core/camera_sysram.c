...
static int fsysram_flag_show(struct seq_file *m, void *v)
{
    seq_printf(m, "\r\n[SYSRAM_ReadFlag]\r\n");
    seq_printf(m, "=========================================\r\n" );
    seq_printf(m, "Sysram.DebugFlag = 0x%08lX\r\n",Sysram.DebugFlag);
    return 0;
}
...
static int fsysram_show(struct seq_file *m, void *v)
{
    MUINT32 Index = 0;
    SYSRAM_MEM_NODE_STRUCT* pCurrNode = NULL;
    //
    seq_printf(m, "\n[SYSRAM_DumpLayoutToProc]\n");
    seq_printf(m, "AllocatedTbl = 0x%08lX\n",Sysram.AllocatedTbl);
    seq_printf(m, "=========================================\n" );
    for (Index = 0; Index < SYSRAM_MEM_BANK_AMOUNT; Index++)
    {
        seq_printf(m, "\n [Mem Pool %ld] (IndexTbl, UserCount)=(%lX, %ld)\n",
                        ...
                        SysramMemPoolInfo[Index].IndexTbl,
                        SysramMemPoolInfo[Index].UserCount);
        seq_printf(m, "[Locked Time] [Owner   Offset   Size  Index pCurrent pPrevious pNext]  [pid tgid] [Proc Name / Owner Name]\n");
        pCurrNode = &SysramMemPoolInfo[Index].pMemNode[0];
        while ( NULL != pCurrNode )
        {
            SYSRAM_USER_ENUM const User = pCurrNode->User;
...
static int fsysram_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, fsysram_show, NULL);
}

static int fsysram_flag_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, fsysram_flag_show, NULL);
}

static const struct file_operations fsysram_proc_fops = { 
	.open = fsysram_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.write = NULL,
...
//fixed in Ubuntu, reading from /proc/sysram_flag no longer crashes
#if 1
