...
static struct power_mode_info *power_mode_data;
...
#define DVFS_LITTLE_F7 (494000)  /* KHz */
...
// Less verbosity for the Ubuntu kernel
#undef cpufreq_ver
#define cpufreq_ver(fmt, args...)
#undef cpufreq_dbg
#define cpufreq_dbg(fmt, args...)
...
static unsigned int read_efuse_speed(void) // TODO: remove it latter
{
	...
	case 14: // 1110
	case 15: // 1111
		return CPU_LEVEL_1; /* 2.0GHz + 1.7GHz */
	default:
		return CPU_LEVEL_2; /* free (2.2GHz + 1.7GHz) */
	}
}
...
static unsigned int get_cur_vsram_big(struct mt_cpu_dvfs *p)
{
	...
	if (rdata) { /* enabled i.e. not 0mv */
		...
		rdata = PMIC_VAL_TO_VOLT(rdata & 0x7F);
	}
	...
	if (dev) {
	   	if (TURBO_MODE_BOUNDARY_CPU_NUM == (cpu_dvfs_is(p, MT_CPU_DVFS_LITTLE) ? num_online_cpus_little : num_online_cpus_big)) {
			...
			case CPU_UP_CANCELED:
				if (cpu_dvfs_is(p, MT_CPU_DVFS_LITTLE)) num_online_cpus_little_delta = 0;
...
#if defined(CONFIG_POWER_MODE)

static DEFINE_MUTEX(powermode_change_mutex);

static int power_mode_setting(struct power_mode_info *power_mode_data) {
	struct cpufreq_policy *policy;
	struct mt_cpu_dvfs *p;
	int i,ret;
	mutex_lock(&powermode_change_mutex);
	for_each_cpu_dvfs(i, p) {
		if (!cpu_dvfs_is_availiable(p))
			continue;
		...
		if (cpu_dvfs_is(p,MT_CPU_DVFS_BIG)) {
			p->turbo_mode = power_mode_data->big_turbo,
			p->limited_max_freq = power_mode_data->cpu_freq_big;
			policy->max = power_mode_data->cpu_freq_big;
			cpufreq_driver_target(policy, p->limited_max_freq, CPUFREQ_RELATION_H);
			cpufreq_cpu_put(policy);

		} else {
			p->turbo_mode = power_mode_data->little_turbo,
			p->limited_max_freq = power_mode_data->cpu_freq_little;
			policy->max = power_mode_data->cpu_freq_little;
			cpufreq_driver_target(policy, p->limited_max_freq, CPUFREQ_RELATION_H);
			cpufreq_cpu_put(policy);
		}
	...
	ret = hps_set_cpu_num_limit(LIMIT_POWER_MODE, power_mode_data->little_num, power_mode_data->big_num);
	if (ret)
		cpufreq_err("hps_set_cpu_num_limit cpu %d faild. big %d, little %d \n", p->cpu_id, power_mode_data->big_num, power_mode_data->little_num);

	/* Update the temp setting for turbo */
	turbo_mode_cfg[TURBO_MODE_2].temp = power_mode_data->turbo_temp * 1000;
	turbo_mode_cfg[TURBO_MODE_1].temp = turbo_mode_cfg[TURBO_MODE_2].temp + 20000;
	turbo_mode_cfg[TURBO_MODE_NONE].temp = turbo_mode_cfg[TURBO_MODE_1].temp + 40000;

	mutex_unlock(&powermode_change_mutex);
	return ret;
}

static int power_mode_state_change(struct notifier_block *nb,
		unsigned long index, void *data)
{
	int ret;
	power_mode_data = data;
	ret = power_mode_setting(power_mode_data);
	if (ret)
		return -EINVAL;
	return NOTIFY_OK;
}

static struct notifier_block power_mode_notifier = {
	.notifier_call = power_mode_state_change,
};
...
unsigned int mt_cpufreq_get(unsigned int cpu)
{
	struct mt_cpu_dvfs *p;
...
static int __init _mt_cpufreq_pdrv_init(void)
{
	...
	{
		int i;
		unsigned int vcore_ao_temp;
		unsigned int vcore_pdn_temp;
		...
		case CHIP_SW_VER_02:
			pmic_read_interface(PMIC_ADDR_VCORE_AO_VOSEL_ON, &vcore_ao_temp, 0x7F, 0);
			pmic_read_interface(PMIC_ADDR_VCORE_PDN_VOSEL_ON, &vcore_pdn_temp, 0x7F, 0);
			pw.set[PMIC_WRAP_PHASE_DEEPIDLE]._[IDX_DI_VCORE_AO_NORMAL].cmd_wdata = vcore_ao_temp;
			pw.set[PMIC_WRAP_PHASE_DEEPIDLE]._[IDX_DI_VCORE_PDN_NORMAL].cmd_wdata = vcore_pdn_temp;
			pw.set[PMIC_WRAP_PHASE_DEEPIDLE_BIG]._[IDX_DI_VCORE_AO_NORMAL].cmd_wdata = vcore_ao_temp;
			pw.set[PMIC_WRAP_PHASE_DEEPIDLE_BIG]._[IDX_DI_VCORE_PDN_NORMAL].cmd_wdata = vcore_pdn_temp;
		default:
			break;
		}
...
#if defined(CONFIG_POWER_MODE)
	power_mode_register_notifier(&power_mode_notifier);
#endif
