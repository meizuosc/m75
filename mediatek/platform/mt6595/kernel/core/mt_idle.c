...
#endif
static unsigned int     soidle_time_critera = 150000; //FIXME: need fine tune modified by Mars 20140816
...
#ifdef CONFIG_SMP
    //if ((atomic_read(&is_in_hotplug) == 1)||(atomic_read(&hotplug_cpu_count) != 1) || mt_cpufreq_get(0) >= 806000) {
	//TODO: this will be fixed later to solve the bad soidle condition test. Mars.
