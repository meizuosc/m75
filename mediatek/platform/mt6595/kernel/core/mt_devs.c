...
extern void init_boot_common(unsigned int line);
...
//added by mars
extern struct tag_lk_info g_lk_info;
//end
//
struct {
	u32 base;
...
static int mt_find_string(char str[], const char* fingerprint, int *offset)
{
        char *curr=str;
        int i = 0;
        int str_len;
...
static int mt_find_fingerprint(char str[], int *offset)
{
        /**
        ...
        for(i=0; i<sizeof(fingerprint)/sizeof(char*); i++){
                if(mt_find_string(str, fingerprint[i], offset) != 0)
                        return i; // Find it.
        }
        return -1; // Not find
}
static int mt_modify_fingerprint(char str[])
{
    char cmd_str[COMMAND_LINE_SIZE];
    int str_size;
    int offset, i;

    if(NULL == str)
        return 0;
    mt_find_fingerprint(str, &offset);
    if(offset>0){
        memset(cmd_str, 0, COMMAND_LINE_SIZE);
        str_size = strlen(str);
        for(i=str_size; i>offset; i--)//insert 14-13 char; (14 = strlen("console=ttyFIQ"); 13 = strlen("console=ttyMT"))
	        str[i] = str[i-1];
        strncpy(str+offset, "console=ttyFIQ", 14);
    }
    return 1;
}
...
void __init mt_fixup(struct tag *tags, char **cmdline, struct meminfo *mi)
{
    ...
    strlcpy(boot_cmdline, boot_command_line, COMMAND_LINE_SIZE);

    mt_modify_fingerprint(boot_cmdline);//replace MTx --->FIQx

    if ((console_ptr=strstr(boot_cmdline, "ttyFIQ")) != 0)
    {
        uart_port = console_ptr[6] - '0';
        if (uart_port > 3)
            uart_port = -1;
    ...
    strlcpy(*cmdline, boot_cmdline, COMMAND_LINE_SIZE);

    printk("mt_fixup: boot cmdline = %s\n", *cmdline);
}
#else
void mt_fixup(struct tag *tags, char **cmdline, struct meminfo *mi)
{
    ...
    for (; tags->hdr.size; tags = tag_next(tags)) {
		...
		///MEIZU_BSP{@
        } else if(tags->hdr.tag == ATAG_LK_INFO) {
			/*here we only use lk_version and lk_mode, the others use recently*/	
			memcpy(&g_lk_info, &tags->u.lk_info, sizeof(struct tag_lk_info));
		}
		///@}
    }
...
void __init mt_dt_init_early(void)
{
    ...
    if (tags)
      parse_dfo_tag(tags);
	///MEIZU_BSP{@
	tags = of_get_flat_dt_prop(node, "atag,lk_info", NULL);
	if (tags) {
		memcpy(&g_lk_info, &tags->u.lk_info, sizeof(struct tag_lk_info));
	}
	tags = of_get_flat_dt_prop(node, "atag,system_rev", NULL);
	if (tags) {
		system_rev = tags->u.revision.rev;
	}
...
#ifdef CONFIG_MEIZU_FIQ_DEBUGGER_CONSOLE
        if(i == mt_fiq_debugger.id){
            printk("skip register uart device %d\n", i);
            continue;
        }
#endif
...
#if !defined(MTK_FORCE_CLUSTER1) ||defined(CONFIG_MEIZU_FIQ_DEBUGGER_CONSOLE)
#ifdef CONFIG_FIQ_DEBUGGER
...
//#endif
    ...
    retval = platform_device_register(&hotplug_strategy_pdev);
    if (retval != 0){
        return retval;
    }
...
static void __init display_early_memory_info(void)
{
	int node;
	struct tag *tags;
	__u64 fb_base = 0x12345678, fb_size = 0;
	__u64 extm_base = 0x22345678, extm_size = 0;
	/* system memory */
	...
	if (node) {
		/* frame buffer */
		fb_size = (__u64)DISP_GetVRamSizeBoot(NULL);
		fb_base = (__u64)mtkfb_get_fb_base();
