...
static struct mt_gpufreq_table_info mt6595_gpufreqs_1[] = {
...
static struct mt_gpufreq_table_info mt6595_gpufreqs_2[] = {
...
/* 650MHz reserved */
static struct mt_gpufreq_table_info mt6595_gpufreqs_3[] = {
...
static struct mt_gpufreq_table_info mt6595_gpufreqs_4[] = {
    GPUOP(GPU_DVFS_FREQ3_1, GPU_DVFS_VOLT2, 0),
    GPUOP(GPU_DVFS_FREQ4,   GPU_DVFS_VOLT2, 1),
    GPUOP(GPU_DVFS_FREQ5,   GPU_DVFS_VOLT2, 2),
    GPUOP(GPU_DVFS_FREQ6,   GPU_DVFS_VOLT2, 3),
...
//static unsigned int mt_gpufreq_dvfs_bond_2 = 0;
//static unsigned int mt_gpufreq_dvfs_bond_01 = 0;
static unsigned int mt_gpufreq_dvfs_function_code = 0;
static unsigned int mt_gpufreq_dvfs_config_95 = 0;
...
{
	#if 1
	/* Only need to check function code */
	unsigned int function_code = 0, ret = 0;

	function_code = (get_devinfo_with_index(24) >> 24) & 0xF;
	
	mt_gpufreq_dvfs_function_code = function_code;
	...
	if(CHIP_SW_VER_02 == mt_get_chip_sw_ver())
	{
		...
		/* 95 flavor build */
		ret = 0;
		mt_gpufreq_dvfs_config_95 = 1;
		#else
		#endif
	}

	#else
    unsigned int bond_01 = 0, bond_2 = 0, ret = 0;

    bond_01 = (get_devinfo_with_index(3) >> 30) & 0x3;
	bond_2 = (get_devinfo_with_index(3) >> 23) & 0x1;

	mt_gpufreq_dvfs_bond_01 = bond_01;
	mt_gpufreq_dvfs_bond_2 = bond_2;
	...
	else
	{
		...
		case 2:
			ret = 4; /* eFuse 550MHz, SW DVFS table 450MHz */
			break;
...
void mt_gpufreq_disable_by_ptpod(void)
{
    ...
    {
        if(mt_gpufreqs_default[i].gpufreq_volt == GPU_DVFS_PTPOD_DISABLE_VOLT) // GPU_DVFS_VOLT1 = 1.125v
        {
	...
	{
		// Not found 1.125V opp
		mt_gpufreq_ptpod_disable_idx = 0;
		
		mt_gpufreq_voltage_enable_set(1);
		mt_gpufreq_target(0);	

		// Force to 1.125V for PTPOD
		mutex_lock(&mt_gpufreq_lock);
...
{   
	...
	seq_printf(m, "mt_gpufreq_dvfs_config_95 = %d\n", mt_gpufreq_dvfs_config_95);
...
static int mt_gpufreq_pdrv_probe(struct platform_device *pdev)
{
    ...
    ***********************/
	...
	if(mt_gpufreq_dvfs_table_type == 0)
    	mt_setup_gpufreqs_table(OPPS_ARRAY_AND_SIZE(mt6595_gpufreqs_0)); // 600MHz
	else if(mt_gpufreq_dvfs_table_type == 1)
		mt_setup_gpufreqs_table(OPPS_ARRAY_AND_SIZE(mt6595_gpufreqs_1)); // 700MHz
	else if(mt_gpufreq_dvfs_table_type == 2)
		mt_setup_gpufreqs_table(OPPS_ARRAY_AND_SIZE(mt6595_gpufreqs_2)); // 450MHz
	else if(mt_gpufreq_dvfs_table_type == 3)
		mt_setup_gpufreqs_table(OPPS_ARRAY_AND_SIZE(mt6595_gpufreqs_3)); // 650MHz, reserved
	else if(mt_gpufreq_dvfs_table_type == 4)
		mt_setup_gpufreqs_table(OPPS_ARRAY_AND_SIZE(mt6595_gpufreqs_4)); // eFuse 550MHz, SW DVFS table 450MHz
	else
    ...
    ***********************/
    if(mt_gpufreqs[0].gpufreq_khz >= GPU_DVFS_FREQ3)
	{
	    for (i = 0; i < mt_gpufreqs_num; i++) {
	...
	#ifdef MT_GPUFREQ_INPUT_BOOST	
	///MEIZU_BSP{@
	mt_gpufreq_up_task = kthread_run(mt_gpufreq_input_boost_task, NULL, "mt_gpufreq_input_boost_task");
	///@}
