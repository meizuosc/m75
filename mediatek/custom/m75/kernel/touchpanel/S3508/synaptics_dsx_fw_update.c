...
#define __UPDATE_FW_FROM_SD__

#ifdef __UPDATE_FW_FROM_SD__
...
extern void synaptics_rmi4_set_gesture_addr(int fw_id);
...
static ssize_t fwu_sysfs_sdfw_show(struct device *dev,
		struct device_attribute *attr, char *buf);
static ssize_t fwu_sysfs_appid_show(struct device *dev,
		struct device_attribute *attr, char *buf);
...
#define FW_SD_PATH "/sdcard/M752TPK.img.ihex"

static struct image_info m7x_images[] = {
   /* The primary TPK touchpanel pattern*/
{ .product_id = "M752TPKB",.img_path = "rmi4/M752TPKB.img", 1622268,-1},
   /*TPK +two prong patten  +TOL +black color ;just for 7/3 test */
{ .product_id = "75TPK1TOLB",.img_path = "rmi4/75TPK1TOLB.img", 1622268,-1},
  /*TPK + DH prong patten  +TOL +black color ;just for 7/3 test */
{ .product_id = "75TPK2TOLB",.img_path = "rmi4/75TPK2TOLB.img", 1622268,-1},
{ .product_id = "75TPKB",.img_path = "rmi4/75TPKB.img", 1622268,-1},
 /*TPK + two prong patten +TOL+white color ;just for 7/3 test */
{ .product_id = "75TPK1TOLW",.img_path = "rmi4/75TPK1TOLB.img", 1622268,-1},
  /*TPK + DH prong patten +TOL+white color ;just for 7/3 test */
{ .product_id = "75TPK2TOLW",.img_path = "rmi4/75TPK2TOLW.img", 1622268,-1},
{ .product_id = "75TPKW",.img_path = "rmi4/75TPKW.img", 1622268,-1},
 /*TPK OGS Black */
{ .product_id = "75TPKOB",.img_path = "rmi4/75TPKOB.img", 1622268,-1},

   /*wintek +synaptics IC + OGS + black color*/
{ .product_id = "75WTKSOGSB",.img_path = "rmi4/75WTKSOGSB.img", 1622268,-1},
{ .product_id = "75WTKB",.img_path = "rmi4/75WTKB.img", 1622268,-1},
{ .product_id = "75WTKW",.img_path = "rmi4/75WTKW.img", 1622268,-1},
  /*wintek +atmel  IC + OGS + black color*/
{ .product_id = "75WTKAOGSB",.img_path = "rmi4/75WTKAOGSB.img", 1622268,-1},
  /*biel +synaptics 3508 IC + OGS + black color*/
{ .product_id = "75BIEB",.img_path = "rmi4/75BIEB.img", 1747430,-1},
 /*biel + synaptics 3501 IC + GFF +White color panel */
{ .product_id = "S35BIEGFFW",.img_path = "rmi4/75BIEWGFF.img", 1747430,-1},
    ...
    int padSzBuf)
{
    ...
    iRet = synaptics_read_file(pPatchName, &pfw->data, 0, padSzBuf);
    set_fs(orig_fs);
...
{
	int retval;
	enum flash_area flash_area = NONE_UP;
	...
	fwu->image_fw_id = image_fw_id; 
	synaptics_rmi4_set_gesture_addr(image_fw_id);
	...
	//device_config_id = simple_strtoul(&config_id[1],NULL,10);
	dev_err(rmi4_data->pdev->dev.parent,
	...
	/* Get image config ID */
	//image_config_id = simple_strtoul(fwu->config_data+1,NULL,10);
	dev_err(rmi4_data->pdev->dev.parent,
...
#else 
	/*Just compare string value */
	info_printk("image config[%s],device image[%s]\n",fwu->config_data,config_id);
	if(strncmp(fwu->config_data,config_id,4)){
		flash_area = CONFIG_AREA ;
		goto exit ;
	}
#endif
...
exit:
	if (flash_area == NONE_UP) {
		strncpy(fwu->config_id,config_id,4);
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: No need to do reflash\n",
				__func__);
	} else {
	       strncpy(fwu->config_id,fwu->config_data,4);
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Updating %s\n",
				__func__,
				flash_area == UI_FIRMWARE ?
		...
		unsigned char command)
{
	...
	info_printk("start to write block\n");
	for (block_num = 0; block_num < block_cnt; block_num++) {
		if(block_num%progress==0)
		info_printk("write block_num:%d/%d\n",block_num,block_cnt);
...
static int fwu_start_reflash(void)
{
	...
	printk("%s: Start of reflash process:%s\n", __func__,fwu->product_id);
	fwu->fw_id = -1 ;
	...
	if(fwu->fw_id!=-1&&(!fwu->ext_image_name)){
	  strncpy(fwu->image_name, m7x_images[fwu->fw_id].img_path, MAX_IMAGE_NAME_LEN);
	  dev_err(rmi4_data->pdev->dev.parent,
   ...
   }else if(fwu->ext_image_name){
   		printk("%s: :pid[%d]:%s:filename[%s]\n",__func__,current->pid,current->comm,FW_SD_PATH);
		retval = synaptics_patch_get(FW_SD_PATH,&fw_entry,0);		
	    if(retval<0)
			goto exit ;
   }else {
		dev_err(rmi4_data->pdev->dev.parent,
			"%s:Firmware product id not available\n",__func__);
              goto exit;	
   	}
	dev_err(rmi4_data->pdev->dev.parent,
	...
	if (fwu->bl_version != header.bootloader_version) {
		dev_err(rmi4_data->pdev->dev.parent,
				...
				__func__);
	info_printk("device bootloader id[%d],image id[%d]\n",fwu->bl_version,header.bootloader_version);
	...
	info_printk("fw size[%x],config_size[%x]\n",header.firmware_size,header.config_size);
...
exit:
	...
	if(retval<0){
		fwu->fw_id = rmi4_data->firmware_id;
		memset(fwu->config_id,0,sizeof(fwu->config_id));
	}

	mutex_unlock(&fwu->fw_mutex);
...
static void fwu_sd_fw_update_work(struct work_struct *work)
{
...
static ssize_t fwu_sysfs_appid_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%s:%s:S3508:%u\n",
		fwu->product_id,fwu->config_id,fwu->fw_id);
}
...
{
	info_printk("%s:%s:S3508:%u\n",
		fwu->product_id,fwu->config_id,fwu->fw_id);

}
#define F01_BUID_ID_OFFSET 18

int fwu_get_product_id(struct synaptics_rmi4_data * rmi4_data)
{
	int retval ;	
	unsigned char build_id[SYNAPTICS_RMI4_BUILD_ID_SIZE];
	int device_fw_id  ;
	char device_config_id[5] ;
	...
	device_fw_id = (unsigned int)build_id[0] +
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
				fwu->f34_fd.ctrl_base_addr,
				device_config_id,
				sizeof(device_config_id));
	device_config_id[4] = 0 ;
	info_printk("d_fw(%d),image(%d)\n",device_fw_id,fwu->image_fw_id);

   return device_fw_id!=fwu->image_fw_id ;
		...
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n",
		fwu_get_product_id(fwu->rmi4_data));
}
...
{
	char * ret = "NG" ;
	if(!fwu->ext_image_name){
	info_printk("start sd fw update work\n");
	fwu->ext_image_name = true;
	fwu->force_update = true ;
	fwu->config_area = UI_CONFIG_AREA;
	fwu_start_reflash();
	fwu->ext_image_name = false ;
	fwu->force_update = false ;
	info_printk("end sd fw update \n");
	ret = "OK" ;
	}
