...
static struct i2c_board_info __initdata synaptics_board =
{ 
  I2C_BOARD_INFO(TPD_DEVICE, (0x20))
};
...
#define MZ_KOBJ_NAME  "mx_tsp"
#define SYNAPICS_KOBJ_NAME "sensor00"
...
extern  struct platform_device *synaptics_dsx_i2c_device;
extern int synaptics_rmi4_i2c_remove(struct i2c_client *client);
...
static int  synaptics_rmi4_remove(struct i2c_client *pdev);
...
static ssize_t synaptics_rmi4_gpio_show(struct device *dev,
		struct device_attribute *attr, char *buf);
...
static ssize_t synaptics_rmi4_suspend_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
static int synaptics_hall_control(struct synaptics_rmi4_data *rmi4_data,bool on);
...
DEFINE_RATELIMIT_STATE(ratelimit, 2* HZ, 1);
...
int ratelimit_printk( char *func)
{
	return ___ratelimit(&ratelimit, func);
}
...
static struct device_attribute attrs[] = {
#ifdef CONFIG_HAS_EARLYSUSPEND
	__ATTR(full_pm_cycle, (S_IRUGO | S_IWGRP|S_IWUSR),
	...
	__ATTR(gpio, S_IRUGO,synaptics_rmi4_gpio_show,
			synaptics_rmi4_show_error),
	__ATTR(touch_debug, S_IWUSR|S_IWGRP,synaptics_rmi4_show_error,
...
__ATTR(gesture_statistics, (S_IRUGO),
					synaptics_rmi4_statistics_gesture,
					NULL),
#endif
#endif			
};
		  ...
		  struct device_attribute *attr, char *buf)
{
	...
	char * pstr ;
	 if(rmi4_data->mode==SYN_HALL_COVER_MODE)
       pstr = "hall cover mode";
	 else if(rmi4_data->mode==SYN_HALL_UNCOVER_MODE)
	 	pstr= "hall uncover mode";
		...
		struct device_attribute *attr, char *buf)
{
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	 ...
	 if(!rmi4_data->data->in.hw_get_gpio_value)
		 return snprintf(buf,PAGE_SIZE,"hw get gpio function not set\n");
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
  ...
  info_printk("setvalue[%d]\n",value);
  
  if(rmi4_data->mode==SYN_GESTURM_MODE&&
  ...
  synaptics_set_sensor_mode(rmi4_data,(!!value)?SYN_HALL_COVER_MODE:\
  	                    SYN_HALL_UNCOVER_MODE);
  synaptics_hall_control(rmi4_data,!!value);
		...
		struct synaptics_rmi4_fn *fhandler)
{
  ...
  if(rmi4_data->gesture_enable){
  		info_printk("handler gestur begin\n");
		synaptics_rmi4_f12_handler_gesture(rmi4_data);
		info_printk("handler gesture done\n");
		return 0 ;
  }
#endif
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		if(rmi4_data->touch_debug)
			...
			/* */
			//if(synaptics_is_report_hall_mode(rmi4_data,x,y))
				//continue ;
#ifdef TYPE_B_PROTOCOL
			input_mt_slot(rmi4_data->input_dev, finger);
			input_mt_report_slot_state(rmi4_data->input_dev,
...
static int synaptics_hall_control(struct synaptics_rmi4_data *rmi4_data,bool on)
{
	 ...
	 info_printk("write hall mode data[%x],ret[%d]\n",report_data,retval);
	 return retval ;
}

static int synaptics_hall_notity_func(struct notifier_block *nb,
			unsigned long action, void *data)
{
	struct synaptics_rmi4_data * rmi4_data =
				container_of(nb, struct synaptics_rmi4_data,notify);

	info_printk("hall_notify action[%d]mode[%d]\n",action,rmi4_data->mode);

    if(HALL_COVER==action){
	  rmi4_data->gesture_enable= false ;
	 /*case4 :screen off->gesture mode->hall cover-->touch deepsleep */
	 if(rmi4_data->mode==SYN_GESTURM_MODE){
		info_printk("case4 :screen off->gesture mode->hall cover\n");
		synaptics_rmi4_irq_enable(rmi4_data, false);
	    synaptics_rmi4_sensor_sleep(rmi4_data); 
		synaptics_set_sensor_mode(rmi4_data,SYN_HALL_COVER_MODE);
		return 0;
	 }	 
	 synaptics_set_sensor_mode(rmi4_data,SYN_HALL_COVER_MODE);
	}else {	
	  synaptics_set_sensor_mode(rmi4_data,SYN_HALL_UNCOVER_MODE);
	}
	synaptics_hall_control(rmi4_data,(action==HALL_COVER)?1:0);
   return 0 ;
}


static synaptics_register_hall_notify(struct synaptics_rmi4_data *rmi4_data)
{
	rmi4_data->notify.notifier_call = synaptics_hall_notity_func ;
	register_gpio_key_notifier(&(rmi4_data->notify));
}
...
static int is_old_tp(struct synaptics_rmi4_data *rmi4_data)
{
	return strncmp(rmi4_data->rmi4_mod_info.product_id_string,"75",2);
}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (button = 0; button < 1; button++) {
		...
		if (rmi4_data->fingers_on_2d == false) {
			...
			touch_count++;
			#ifdef CONFIG_MTK_LEDS
				if((led_trigger_fadeonoff_func != NULL) && status)
...
static void synaptics_rmi4_sensor_report(void *arg)
{
	int retval;
	...
	struct synaptics_rmi4_device_info *rmi;
	struct synaptics_rmi4_data *rmi4_data = (struct synaptics_rmi4_data *)arg;

	if(!rmi4_data){
		info_printk("no rmi4 device to report finger \n");
		return -ENODEV ;
	}
	rmi = &(rmi4_data->rmi4_mod_info);
		...
		bool enable)
{
	...
	if (enable) {		
		...
		if(rmi4_data->data->in.hw_irq_enable)
				 retval = rmi4_data->data->in.hw_irq_enable();
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
		...
		if (rmi4_data->irq_enabled) {
			if(rmi4_data->data->in.hw_irq_disable)
				 retval = rmi4_data->data->in.hw_irq_disable();
...
static int synaptics_rmi4_set_input_dev(struct synaptics_rmi4_data *rmi4_data)
{
	...
	rmi4_data->sensor00 = &rmi4_data->input_dev->dev.kobj ;
	rmi4_data->devices  = sysfs_get_devices_parent(); 
	sysfs_create_link(rmi4_data->devices,rmi4_data->sensor00, MZ_KOBJ_NAME);
	sysfs_create_link(rmi4_data->devices,rmi4_data->sensor00, SYNAPICS_KOBJ_NAME);
...

{
    ...
    struct tp_driver_data *data=meizu_get_tp_hw_data();

	if(!data||data->probed){
		info_printk("atmel probed[%d]\n",data->probed);
		return -1 ;
    };
	
	info_printk("enter rmi4 probe\n");
	synaptics_rmi4_i2c_probe(client,dev_id);
	
	pdev = synaptics_dsx_i2c_device; 

	if(!pdev){
		info_printk("no rmi4 i2c devices \n");
		return -ENODEV ;
	}
...
#ifdef SYNP_HALL_SUPPORT
	synaptics_register_hall_notify(rmi4_data);
#endif
	
#ifdef CONFIG_HAS_EARLYSUSPEND
	rmi4_data->early_suspend.level =0;// EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 1;
	...
	data->out.report_func = synaptics_rmi4_sensor_report ;
	data->out.remove = synaptics_rmi4_remove ;
	data->out.driver_data = rmi4_data ;

	data->in.lancher_thread(data,1);
    data->in.hw_register_irq();
	data->probed = 1 ;
	...
	queue_delayed_work(exp_data.workqueue,
			&exp_data.work,
			0);
	mtk_tpd_register_misc();
	info_printk("synaptics probe success\n");
	return retval;
...
{
	unsigned char attr_count;
	struct synaptics_rmi4_data *rmi4_data = i2c_get_clientdata(pdev);
	const struct synaptics_dsx_board_data *bdata =
			rmi4_data->hw_if->board_data;
	...
	sysfs_remove_link(rmi4_data->devices,MZ_KOBJ_NAME);
	sysfs_remove_link(rmi4_data->devices,SYNAPICS_KOBJ_NAME);
	
	kfree(rmi4_data);

	synaptics_rmi4_i2c_remove(pdev);
...
static void synaptics_rmi4_early_suspend(struct early_suspend *h)
{
	...
	struct synaptics_rmi4_data *rmi4_data =
			...
			early_suspend);
	 fwu_show_product_id();
	 /*clear all finger in touch */	
     info_printk("mode[%d]\n",rmi4_data->mode);
#ifdef M75_TP_GESTURE_SUPPORT
		if(!rmi4_data->disable_all&&!(rmi4_data->mode==SYN_HALL_COVER_MODE)){
		   synaptics_rmi4_gesture_enable(rmi4_data);
		   synaptics_set_sensor_mode(rmi4_data,SYN_GESTURM_MODE) ;		   
...
{
	info_printk("synatpcis register driver\n");
	
	i2c_register_board_info(2, &synaptics_board, 1);
	
    if(i2c_add_driver(&synaptics_i2c_driver)!=0){
		info_printk("register i2c driver error\n");
		return -1 ;
    }
...
{
	info_printk("synaptics rmi4 exit\n");
