		...
		struct device_attribute *attr, char *buf)
{
	...
	attn_state = gpio_get_value(rmidev->rmi4_data->hw_if->board_data->irq_gpio);
...
static int rmidev_init_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	rmidev->fn_ptr->read = synaptics_rmi4_reg_read;
	rmidev->fn_ptr->write = synaptics_rmi4_reg_write;
...
#if 0
	retval = gpio_export(rmi4_data->hw_if->board_data->irq_gpio, false);
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
		...
		retval = gpio_export_link(&(rmi4_data->input_dev->dev),
				"attn", rmi4_data->hw_if->board_data->irq_gpio);
		if (retval < 0) {
			...
			dev_dbg(&rmi4_data->input_dev->dev,
					...
					__func__, rmi4_data->hw_if->board_data->irq_gpio);
		}
	}
#endif
