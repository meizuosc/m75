...
#define PFX "IMX220_liteon_camera_sensor"
...
#define	OTP_LSC_SIZE	704
#define	E2PROM_WRITE_ID	0xA8
#define OTP_LSC_START_REG	0x0037
#define IMX220_LSC_START_REG	0xC100

static u8 *liteon_otp_buf;

extern int platform_match;

struct imx220_write_buffer {
	u8 addr[2];
	u8 data[MAX_READ_WRITE_SIZE];
};

static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = IMX220_SENSOR_ID & 0x0FFF,
...
{
	struct imx220_write_buffer buffer;
	int i;
	int ret;
...
static void sensor_init(void)
{
	...
	if (liteon_otp_buf != NULL)
		imx220_write_otp_sensor(IMX220_LSC_START_REG, liteon_otp_buf);
#endif
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	if (platform_match == 0) {
		ret = imx220_read_otp(0x0000, &platform_id);
		if ((ret >= 0) && (platform_id == 0x01)) {
			platform_match = 1;
		} else {
			platform_match = 0;
...
otp_read:
	...
	imx220_read_otp_lsc(OTP_LSC_START_REG, liteon_otp_buf);
#endif
	liteon_otp_buf = NULL;
...
static kal_uint32 open(void)
{
	...
	if ((ret < 0) || (imgsensor_info.sensor_id != sensor_id) || (module_id != 0x01))
		return ERROR_SENSOR_CONNECT_FAIL;
