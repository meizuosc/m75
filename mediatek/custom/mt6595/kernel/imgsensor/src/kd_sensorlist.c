...
#if !defined( SUPPORT_I2C_BUS_NUM1_SP) || !defined(SUPPORT_I2C_BUS_NUM2_SP)
    #define SUPPORT_I2C_BUS_NUM1_SP     100
    #define SUPPORT_I2C_BUS_NUM2_SP     200
#endif
         ...
         if (g_pSensorFunc) {
            ...
            if(ERROR_NONE != err) {
                PK_DBG(" ERROR:SensorOpen(), turn off power, Mclk1(0x%x), ISPClk(0x%x), MclkPll(0x%x)\n",
                        (unsigned int)SENSOR_RD32(0xf5008204),
                        (unsigned int)SENSOR_RD32(0xf5000000),
                        (unsigned int)SENSOR_RD32(0xf0000060)
                       );
...
//ifdef Flyme_EDIT   //Multimedia Camera scx@meizu.com
    SensorInfo.ZsdDelayFrame                      = pInfo[IDNum]->ZsdDelayFrame; 
    SensorInfo.SlimCaptureDelayFrame                      = pInfo[IDNum]->SlimCaptureDelayFrame; 
    SensorInfo.VideoFourkDelayFrame                      = pInfo[IDNum]->VideoFourkDelayFrame; 
//endif  /*Flyme_EDIT */    
...
//ifdef Flyme_EDIT   //Multimedia Camera scx@meizu.com
    SensorInfo.SensorGrabStartX_ZSD                     = pInfo4[IDNum]->SensorGrabStartY;
    SensorInfo.SensorGrabStartY_ZSD                     = pInfo4[IDNum]->SensorGrabStartY;
    SensorInfo.SensorGrabStartY_CAP1                     = pInfo4[IDNum]->SensorGrabStartY;
    SensorInfo.SensorGrabStartY_CAP1                     = pInfo4[IDNum]->SensorGrabStartY;
    SensorInfo.SensorGrabStartX_VD4k                     = pInfo4[IDNum]->SensorGrabStartY;
    SensorInfo.SensorGrabStartY_VD4k                     = pInfo4[IDNum]->SensorGrabStartY;
//endif  /*Flyme_EDIT */    
...
static int CAMERA_HW_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    //set I2C clock rate
//ifdef Flyme_EDIT Multimedia Camera
    //g_pstI2Cclient->timing = SUPPORT_I2C_BUS_NUM1_SP;//200k
...
static int CAMERA_HW_i2c_probe2(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    //set I2C clock rate
//ifdef Flyme_EDIT Multimedia Camera
    //  g_pstI2Cclient2->timing = SUPPORT_I2C_BUS_NUM2_SP;//200k
    g_pstI2Cclient2->timing = 400;//200k
//endif 
	spin_unlock(&kdsensor_drv_lock);
