...
#include "imx208sunnymipiraw_Sensor.h"

#define PFX "IMX208_sunny_camera_sensor"
//#define LOG_WRN(format, args...) xlog_printk(ANDROID_LOG_WARN ,PFX, "[%S] " format, __FUNCTION__, ##args)
//#defineLOG_INF(format, args...) xlog_printk(ANDROID_LOG_INFO ,PFX, "[%s] " format, __FUNCTION__, ##args)
//#define LOG_DBG(format, args...) xlog_printk(ANDROID_LOG_DEBUG ,PFX, "[%S] " format, __FUNCTION__, ##args)
#define LOG_INF(format, args...)	xlog_printk(ANDROID_LOG_INFO   , PFX, "[%s] " format, __FUNCTION__, ##args)

#define SUNNY_E2PROM_WRITE_ID	0x6E

int sunny_status = 0; 
extern int qtech_status; 

static DEFINE_SPINLOCK(imgsensor_drv_lock);


static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = IMX208_SUNNY_SENSOR_ID & 0x0FFF,
...
static int imx208_sunny_read_otp(u16 addr, u8 *buf)
{
	int ret = 0;
	u8 pu_send_cmd[2] = {(u8)(addr >> 8), (u8)(addr & 0xFF)};

	ret = iReadRegI2C(pu_send_cmd, 2, (u8*)buf, 1, SUNNY_E2PROM_WRITE_ID);
	if (ret < 0)
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	do {
		*sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
		ret = imx208_sunny_read_otp(0x3508, &module_id);
		if ((ret >= 0) && (*sensor_id == imgsensor_info.sensor_id) && (module_id == 0x01)) {				
			...
			sunny_status = 0;
			*sensor_id = IMX208_SUNNY_SENSOR_ID;
			return ERROR_NONE;
		}	
	...
	if (*sensor_id == imgsensor_info.sensor_id && qtech_status == 1) {
...
static kal_uint32 open(void)
{
	...
	if ((imgsensor_info.sensor_id != sensor_id) || (sunny_status == 1 && qtech_status == 0))
		return ERROR_SENSOR_CONNECT_FAIL;
	
	LOG_INF("imx208sunny open successfully.\n");
...
UINT32 IMX208_SUNNY_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
