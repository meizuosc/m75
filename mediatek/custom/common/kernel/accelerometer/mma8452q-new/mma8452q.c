...
static int mma8452q_open_report_data(int open)
{
...
static int mma8452q_enable_nodata(int en)
{
	int res =0;
	bool power = false;
	...
	res = MMA8452Q_SetPowerMode(obj_i2c_data->client, power);
	if(res != MMA8452Q_SUCCESS)
	{
		GSE_ERR("MMA8452Q_SetPowerMode fail!\n");
		return -1;
	}
	GSE_LOG("mma8452q_enable_nodata OK!\n");
	return 0;
}

static int mma8452q_set_delay(u64 ns)
{
    int value =0;
	int sample_delay=0;
	int err;
	...
	mutex_lock(&mma8452q_op_mutex);
	err = MMA8452Q_SetBWRate(obj_i2c_data->client, sample_delay);
	if(err != MMA8452Q_SUCCESS ) //0x2C->BW=100Hz
	{
		GSE_ERR("Set delay parameter error!\n");
	}
	...
	{					
		...
		obj_i2c_data->fir.sum[MMA8452Q_AXIS_X] = 0;
		obj_i2c_data->fir.sum[MMA8452Q_AXIS_Y] = 0;
		obj_i2c_data->fir.sum[MMA8452Q_AXIS_Z] = 0;
		atomic_set(&obj_i2c_data->filter, 1);
	}
	
	GSE_LOG("mma8452q_set_delay (%d)\n",value);
	return 0;
}

static int mma8452q_get_data(int* x ,int* y,int* z, int* status)
{
	char buff[MMA8452Q_BUFSIZE];
	MMA8452Q_ReadSensorData(obj_i2c_data->client, buff, MMA8452Q_BUFSIZE);
...
static int mma8452q_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ctl.open_report_data= mma8452q_open_report_data;
	ctl.enable_nodata = mma8452q_enable_nodata;
	ctl.set_delay  = mma8452q_set_delay;
	...
	data.get_data = mma8452q_get_data;
...
static int __init mma8452q_init(void)
{
	...
	acc_driver_add(&mma8452q_init_info);
	return 0;    
}
/*----------------------------------------------------------------------------*/
