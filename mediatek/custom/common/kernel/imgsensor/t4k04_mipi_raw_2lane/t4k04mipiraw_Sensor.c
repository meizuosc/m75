...
void T4K04_write_shutter(kal_uint32 shutter)
{
	...
	if(t4k04.T4K04AutoFlickerMode == KAL_TRUE)
	{
		...
		switch(T4K04CurrentScenarioId)
		{
        	case MSDK_SCENARIO_ID_CAMERA_ZSD:
				min_framelength = (t4k04.capPclk*10000) /(T4K04_FULL_PERIOD_PIXEL_NUMS + t4k04.DummyPixels)/148*10 ;
				break;
	...
	else
	{
		...
		if ( SENSOR_MODE_PREVIEW == t4k04.sensorMode )	//SXGA size output
		{
			line_length = T4K04_PV_PERIOD_PIXEL_NUMS + t4k04.DummyPixels; 
			frame_length = T4K04_PV_PERIOD_LINE_NUMS+ t4k04.DummyLines + extra_lines ; 
		}
...
void T4K04_SetGain(UINT16 iGain)
{
	...
	T4K04DB("featurecontrol--iGain=%d\n",iGain);
...
{
	T4K04DB("T4K04_Sensor_Init enter_2lane :\n ");	

	// ==========Summary========
	// Init setting
	// LSSC OFF, PresetWB balanced at 5000K
	// Extclk= 24Mhz  PCK = 130.2Mhz   DCLK = 81.375MHz  MIPI Lane CLK = 325.5MHz  ES 1 line = 26.2058us
	// FPS = 30.19fps   Image output size = 1640x1232
	...
	T4K04_write_cmos_sensor(0x0114,0x01);//-/-/-/-/-/CSI_LANE_MODE[2:0];
	...
	T4K04_write_cmos_sensor(0x0301,0x0A);//-/-/-/-/VT_PIX_CLK_DIV[3:0];
	...
	T4K04_write_cmos_sensor(0x0307,0xD9);//PLL_MULTIPLIER[7:0];
	T4K04_write_cmos_sensor(0x0309,0x08);//-/-/-/-/OP_PIX_CLK_DIV[3:0];
	T4K04_write_cmos_sensor(0x030B,0x01);//-/-/-/-/OP_SYS_CLK_DIV[3:0];
	T4K04_write_cmos_sensor(0x0340,0x04);//FR_LENGTH_LINES[15:8];
	T4K04_write_cmos_sensor(0x0341,0xF0);//FR_LENGTH_LINES[7:0];
	...
	T4K04_write_cmos_sensor(0x0406,0x00);//[RO] RO_SCALE_N[15:8];
	T4K04_write_cmos_sensor(0x0407,0x10);//[RO] RO_SCALE_N[7:0];
	...
	T4K04_write_cmos_sensor(0x040C,0x06);//-/-/-/DCROP_WIDTH[12:8];
	T4K04_write_cmos_sensor(0x040D,0x68);//DCROP_WIDTH[7:0];
	T4K04_write_cmos_sensor(0x040E,0x04);//-/-/-/-/DCROP_HIGT[11:8];
	T4K04_write_cmos_sensor(0x040F,0xD0);//DCROP_HIGT[7:0];
	...
	T4K04_write_cmos_sensor(0x0820,0xC3);//MSB_LBRATE[31:24];
	T4K04_write_cmos_sensor(0x0821,0x07);//MSB_LBRATE[23:16];
	...
	T4K04_write_cmos_sensor(0x3277,0x80);//PWB_WLRG[7:0];
	...
	T4K04_write_cmos_sensor(0x330E,0x10);//EXT_HCNT_MAX_ON/-/-/HCNT_MAX_MODE/-/-/-/MLT_SPL_MODE;//JASON 1129
	...
	T4K04_write_cmos_sensor(0x331F,0x03);//ESREAD_ALT_OFF/-/-/ELEC_INJ_MODE/-/-/AUTO_READ_W/AUTO_ESREAD_2D;
	...
	T4K04DB("T4K04_Sensor_Init exit_2lane:\n ");	
...
{	
	...
	// Extclk= 24Mhz  PCK = 130.2Mhz   DCLK = 81.375MHz  MIPI Lane CLK = 325.5MHz  ES 1 line = 26.2058us
	// FPS = 30.19fps   Image output size = 1640x1232
	T4K04DB("T4K04PreviewSetting enter_2lane :\n ");	
	...
	T4K04DB("T4K04PreviewSetting exit_2lane :\n ");
}
...
{
	T4K04DB("T4K04VideoSetting_2lane sync preview_setting :\n ");
	T4K04PreviewSetting();
}
...
{	
    T4K04DB("T4K04CaptureSetting enter_2lane :\n ");
	// ==========Summary========
	// Extclk= 24Mhz  PCK = 130.2Mhz   DCLK = 81.375MHz  MIPI Lane CLK = 325.5MHz  ES 1 line = 26.7281us
	// FPS = 14.99fps   Image output size = 3280x2464
	...
	T4K04DB("T4K04CaptureSetting exit_2lnae :\n ");
}
...
void T4K04_SetShutter(kal_uint32 iShutter)
{
	...
	T4K04DB("featurecontrol:iShutter=%d\n",iShutter);
