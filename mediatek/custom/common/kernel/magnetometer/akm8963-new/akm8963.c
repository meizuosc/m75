...
static int akm8963_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	sobj_m.self = data;
	sobj_m.polling = 1;
	sobj_m.mag_operate = akm8963_operate;
	...
	sobj_o.self = data;
	sobj_o.polling = 1;
	sobj_o.mag_operate = akm8963_orientation_operate;
	...
	err = batch_register_support_info(ID_MAGNETIC,data->hw->is_batch_supported);
	if(err)
	{
		AKMDBG("register mag batch support err = %d\n", err);
		goto exit_kfree;
	}
	err = batch_register_support_info(ID_ORIENTATION,data->hw->is_batch_supported);
	if(err)
	{
		AKMDBG("register ori batch support err = %d\n", err);
		goto exit_kfree;
	}
