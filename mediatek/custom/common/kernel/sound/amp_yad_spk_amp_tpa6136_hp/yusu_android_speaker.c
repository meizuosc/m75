...
bool Speaker_Init(void)
{
   ...
   PRINTK("+HeadsetAmp Init Success");
   mt_set_gpio_mode(GPIO_AUD_EXTHPBUF_SDB_PIN,GPIO_MODE_00);  // gpio mode
//   mt_set_gpio_pull_enable(GPIO_AUD_EXTHPBUF_SDB_PIN,GPIO_PULL_ENABLE);
   
   mt_set_gpio_mode(GPIO_AUD_EXTHPBUF_HI_Z_PIN,GPIO_MODE_00);  // gpio mode
//   mt_set_gpio_pull_enable(GPIO_AUD_EXTHPBUF_HI_Z_PIN,GPIO_PULL_ENABLE);
   
   mt_set_gpio_mode(GPIO_AUD_EXTHPBUF_GAIN_PIN,GPIO_MODE_00);  // gpio mode
//   mt_set_gpio_pull_enable(GPIO_AUD_EXTHPBUF_GAIN_PIN,GPIO_PULL_ENABLE);
   PRINTK("-HeadsetAmp Init Success");
   return true;
}
...
int Audio_eamp_command(unsigned int type, unsigned long args, unsigned int count)
{
    ...
    {        
        mt_set_gpio_dir(GPIO_AUD_EXTHPBUF_HI_Z_PIN,GPIO_DIR_OUT); // output
        mt_set_gpio_out(GPIO_AUD_EXTHPBUF_HI_Z_PIN, GPIO_OUT_ZERO);
        
        mt_set_gpio_dir(GPIO_AUD_EXTHPBUF_GAIN_PIN,GPIO_DIR_OUT); // output
        mt_set_gpio_out(GPIO_AUD_EXTHPBUF_GAIN_PIN, GPIO_OUT_ZERO);
        
        mt_set_gpio_dir(GPIO_AUD_EXTHPBUF_SDB_PIN,GPIO_DIR_OUT); // output
        mt_set_gpio_out(GPIO_AUD_EXTHPBUF_SDB_PIN,GPIO_OUT_ONE); // high
    }
    else if (type == EAMP_HEADPHONE_CLOSE)
    {
        ...
        mt_set_gpio_dir(GPIO_AUD_EXTHPBUF_SDB_PIN,GPIO_DIR_OUT); // output
        mt_set_gpio_out(GPIO_AUD_EXTHPBUF_SDB_PIN, GPIO_OUT_ZERO);//low
    }
	return 0;
}
