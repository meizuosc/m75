...
int iWriteCAM_CAL(u16 a_u2Addr  , u32 a_u4Bytes, u8 puDataInBytes, u16 i2c_id)
{
    int  i4RetValue = 0;
	int retry = 3;
...
//Address: 2Byte, Data: 1Byte
int iReadCAM_CAL(u16 a_u2Addr, u32 ui4_length, u8 * a_puBuff, u16 i2c_id)
{
    int  i4RetValue = 0;
...
static int get_module_type()
{
	int ret = 0;
	u8 module_id = 0;
	u16 puSendCmd = 0x0001;	/* module id reg addr = 0x0001 for imx220 liteon module */

	ret = iReadCAM_CAL(puSendCmd , 1, &module_id, IMX220OTP_LITEON_DEVICE_ID);
	if ((ret < 0) || (module_id != 0x01))
		CAM_CALDB("[CAM_CAL]I2C read liteon e2prom failed!! \n");
	else
		return LITEON_MODULE;

	/* module id reg addr = 0x0000 for imx220 sharp module */
	puSendCmd = 0x0000;
	ret = iReadCAM_CAL(puSendCmd , 1, &module_id, IMX220OTP_SHARP_DEVICE_ID);
	if ((ret < 0) || (module_id != 0x02))
		CAM_CALDB("[CAM_CAL] I2C read sharp e2prom failed!! \n");
	else
		return SHARP_MODULE;

	CAM_CALDB("[CAM_CAL] get module type failed!! \n");
	return -ENODEV;
}

static kal_uint8 IMX220_ReadOtp(kal_uint16 address,unsigned char *iBuffer,unsigned int buffersize)
{
		kal_uint16 i = 0;
		u8 readbuff ;
		int ret ;
		...
		if (get_module_type() == LITEON_MODULE) {
			i2c_id = IMX220OTP_LITEON_DEVICE_ID;
		} else if (get_module_type() == SHARP_MODULE) {
			i2c_id = IMX220OTP_SHARP_DEVICE_ID;
			if (address == 0x0100 && sharp_otp_buf != NULL) {
				/*
				 * lsc data had been read out in sensor driver,
				 * so just copy it to user.
				 */
				memcpy(iBuffer, sharp_otp_buf, SHARP_LSC_SIZE);
				return 0;
			}
		}
...
static int iWriteData(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
   ...
   do 
   {
       ...
       i4RetValue = iWriteCAM_CAL((u16)u4CurrentOffset, 1, pBuff[0], i2c_id);
       if (i4RetValue != 0)
       {
...
static int iReadData(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
   int  i4RetValue = 0;
    ...
    /* read otp */
    IMX220_ReadOtp(ui4_offset,pinputdata, ui4_length);
...
static unsigned short force[] = {CAM_CAL_I2C_GROUP_ID, IMX220OTP_DEVICE_ID, I2C_CLIENT_END, I2C_CLIENT_END};   
#else
//static unsigned short force[] = {IMG_SENSOR_I2C_GROUP_ID, IMX220OTP_DEVICE_ID, I2C_CLIENT_END, I2C_CLIENT_END};   
#endif
...
//    spin_lock_init(&g_CAM_CALLock);
    ...
    g_pstI2Cclient->addr = IMX220OTP_LITEON_DEVICE_ID>>1;
    spin_unlock(&g_CAM_CALLock); // for SMP    
