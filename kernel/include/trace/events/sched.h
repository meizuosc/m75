...
TRACE_EVENT(sched_dynamic_threshold,
	...
	TP_STRUCT__entry(
		...
		__field(int, B_cpu_capacity)
		__field(int, B_acap)
		__field(int, B_ncpu)
		__field(int, L_cpu_capacity)
		__field(int, L_acap)
		__field(int, L_ncpu)
	),
	...
	TP_fast_assign(
		...
		__entry->B_cpu_capacity   = B->cpu_capacity;
		__entry->B_acap           = B->acap;
		__entry->B_ncpu           = B->ncpu;
		__entry->L_cpu_capacity   = L->cpu_capacity;
		__entry->L_acap           = L->acap;
		__entry->L_ncpu           = B->ncpu;
	),

	TP_printk("pid=%4d prio=%d status=0x%4x dyn=%4u task-load=%4d curr-cpu=%d(%4d) target=%d(%4d) L(cap/acap/cpu#)=(%4d/%5d/%d) B(cap/acap/cpu#)=(%4d/%5d/%d) comm=%s",
				__entry->pid,
				__entry->prio,
				__entry->status,
				__entry->threshold,
				...
				__entry->target_cpu,
				__entry->target_load,
				__entry->L_cpu_capacity,
				__entry->L_acap,
				__entry->L_ncpu,
				__entry->B_cpu_capacity,
				__entry->B_acap,
				__entry->B_ncpu,
				__entry->comm)
);
