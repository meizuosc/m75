...
extern struct arizona *global_arizona;
...
static int arizona_irq_task(void)
{
	struct arizona *arizona = global_arizona;

	mt_eint_mask(arizona->irq);
	queue_work(arizona->workqueue, &arizona->irq_task);

	return IRQ_HANDLED;
}

static irqreturn_t arizona_irq_thread(struct work_struct *work)
{
	bool poll;
	unsigned int val;
	int ret;
	struct arizona *arizona = container_of(work, struct arizona, irq_task);
...
#ifdef CONFIG_ARCH_MT6595
	arizona->workqueue = create_workqueue("irq_workqueue");
	INIT_WORK(&arizona->irq_task, arizona_irq_thread);
#endif
...
#ifdef CONFIG_ARCH_MT6595
	/* these code need to debug, maybe use mt_deint_registration() to handle interrupt ASAP */
	mt_eint_registration(arizona->irq,
			     CUST_EINTF_TRIGGER_LOW, /* Low active */
			     arizona_irq_task, 0);
	mt_eint_unmask(arizona->irq);
