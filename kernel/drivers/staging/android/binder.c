		...
		size_t size, int is_async)
{
	...
	if (size > threshold)
	{
		...
		else
		{
			...
			pr_debug("sender pid %d (%s) is not javap, pls check SYS_BINDER_BACKTRACE!\n",
	...
	else
	{
		...
		pr_debug("target_proc %p pid %d (%s) is not javap, pls check SYS_BINDER_BACKTRACE!\n",
	...
	for (; offp < off_end; offp++) {
		...
		case BINDER_TYPE_FD: {
			...
			if (target_fd < 0) {
				fput(file);
#ifdef MTK_BINDER_DEBUG
				binder_user_error("%d:%d no unused fd available, %d\n",
	...
	while (ptr < end && thread->return_error == BR_OK) {
		...
		case BC_CLEAR_DEATH_NOTIFICATION: {
			...
			binder_debug(BINDER_DEBUG_DEATH_NOTIFICATION,
				     "[DN #%s]binder: %d:%d %s %d cookie 0x%016llx\n",
...
#ifdef MTK_BINDER_DEBUG
static int binder_node_release(struct binder_node *node, int refs, int *reg)
{
	struct binder_ref *ref;
	int death = 0;
...
#ifdef MTK_DEATH_NOTIFY_MONITOR
		binder_debug(BINDER_DEBUG_DEATH_NOTIFICATION,
				"[DN #3]binder: %d: cookie 0x%016llx\n",
				node->proc->pid, (u64)ref->death->cookie);
#endif
		if (!(*reg) &&
...
static void binder_deferred_release(struct binder_proc *proc)
{
	...
	while ((n = rb_first(&proc->nodes))) {
		...
		rb_erase(&node->rb_node, &proc->nodes);
#ifdef MTK_BINDER_DEBUG
		incoming_refs = binder_node_release(node, incoming_refs, &sys_reg);
#else
		incoming_refs = binder_node_release(node, incoming_refs);
#endif
	}
#ifdef MTK_BINDER_DEBUG
	pr_debug("%d:%s exits %s system_server DeathNotify\n",
...
static int binder_timeout_log_show(struct seq_file *m, void *unused)
{
	...
	if (latest >= ARRAY_SIZE(log->entry) || latest < 0) {
		...
		for (j = -4; j <= 3; j++) {
			...
			pr_alert("0x%08x %08x %08x %08x %08x  "
					  "%08x %08x %08x %08x\n",
					  (unsigned int)tmp,
