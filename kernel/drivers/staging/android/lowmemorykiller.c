...
/* Conversion between oom_score_adj and oom_adj */
...
static short enable_candidate_log = 1;
#else
#define output_expect(x) unlikely(x)
static short enable_candidate_log = 0;
#endif
...
#else /* !defined (MTK_LCA_RAM_OPTIMIZE) */
...
static int lowmem_shrink(struct shrinker *s, struct shrink_control *sc)
{
	...
	/* Restricted printing of extra information */
	int print_extra_info = 0;
	static unsigned long lowmem_print_extra_info_timeout = 0;
	
	/*LCH add for monkey issue debug*/
#ifdef CONFIG_MT_ENG_BUILD
	static pid_t pid_selected = -1;
    static char pid_name_selected[TASK_COMM_LEN] = {0};
    static int pid_same_selected_cnt = 0;
#endif
    
#if defined (CONFIG_MTK_AEE_FEATURE) && defined (CONFIG_MT_ENG_BUILD) && defined (MTK_INTERNAL_BUILD) /* MTK internal */
	/* Dump memory info when framework low memory */
	int pid_dump = -1;   /* Process which needs to be dump */
	int pid_sec_mem = -1;
	int max_mem = 0;
#endif
...
#if defined (CONFIG_MTK_AEE_FEATURE) && defined (CONFIG_MT_ENG_BUILD) && defined (MTK_INTERNAL_BUILD) /* MTK internal */
		/* Dump memory info when framework low memory - record the first two pid which consumed most memory */
		if (min_score_adj <= lowmem_kernel_warn_adj) {
...
#define MSG_SIZE_TO_AEE 70
        ...
        /*LCH add for monkey issue debug*/
#ifdef CONFIG_MT_ENG_BUILD
        if ((pid_selected == selected->pid) && (!strcmp(pid_name_selected, selected->comm))) {
            pid_same_selected_cnt += 1;
            if(pid_same_selected_cnt >= 3) {
            	struct task_struct *g, *t;

            	lowmem_print(1, "LMK select %d (%s) over %d times\n", pid_selected, pid_name_selected, pid_same_selected_cnt);
            	read_lock(&tasklist_lock);
	            do_each_thread(g, t) {
	            	if (t->tgid == pid_selected) {
	            		lowmem_print(1, "thread tid %d (%s) tgid:%d\n", t->pid, t->comm, t->tgid);
                        show_stack(t, NULL);
	            	}
                ...
                aee_kernel_warning_api("LMK", 0,
					...
					"LMK select the same process over 3 times\n", "LMK trigger kernel warning");
				pid_same_selected_cnt = 0;
            }
        }
        else {
        	pid_same_selected_cnt = 0;/*reset*/
            pid_selected = selected->pid;
		    strlcpy(pid_name_selected, selected->comm, TASK_COMM_LEN);
        }
#endif
...
size_t query_lmk_minfree(int index)
{
	...
	/* Convert it to a proper value */
	index = CONVERT_ADJ(index);
...
#endif
module_param_named(candidate_log, enable_candidate_log, short, S_IRUGO | S_IWUSR);
