 ...
 * function.
 */
static void cpufreq_interactiveplus_timer_start(
	struct cpufreq_interactiveplus_tunables *tunables, int cpu)
{
...
static unsigned int freq_to_targetload(
	struct cpufreq_interactiveplus_tunables *tunables, unsigned int freq)
{
	int i;
	unsigned int ret;
	unsigned long flags;
...
static void cpufreq_interactiveplus_timer(unsigned long data)
{
	...
	if (pcpu->target_freq >= tunables->hispeed_freq &&
		...
		trace_cpufreq_interactiveplus_notyet(
	...
	if (pcpu->target_freq == new_freq) {
		trace_cpufreq_interactiveplus_already(
			data, cpu_load, pcpu->target_freq,
			pcpu->policy->cur, new_freq);
#ifdef DEBUG_LOG
		pr_info
...
static ssize_t show_above_hispeed_delay(
	struct cpufreq_interactiveplus_tunables *tunables, char *buf)
{
	int i;
	ssize_t ret = 0;
	unsigned long flags;
...
static ssize_t show_hispeed_freq(struct cpufreq_interactiveplus_tunables *tunables,
		char *buf)
{
...
static ssize_t show_timer_rate(struct cpufreq_interactiveplus_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%lu\n", tunables->timer_rate);
}
...
static ssize_t show_timer_slack(struct cpufreq_interactiveplus_tunables *tunables,
		char *buf)
{
...
static ssize_t show_boost(struct cpufreq_interactiveplus_tunables *tunables,
			  char *buf)
{
	return sprintf(buf, "%d\n", tunables->boost_val);
}
...
static ssize_t show_io_is_busy(struct cpufreq_interactiveplus_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->io_is_busy);
}
