...
#define DEF_CPU_INPUT_BOOST_ENABLE          (1) // <-XXX
...
static cpu_hotplug_work_type_t g_trigger_hp_work = 0;
static unsigned int g_next_hp_action = 0;
static struct delayed_work hp_work;
struct workqueue_struct *hp_wq = NULL;

static int g_tlp_avg_current = 0;       //set global for information purpose
static int g_tlp_avg_sum = 0;
static int g_tlp_avg_count = 0;
static int g_tlp_avg_index = 0;
static int g_tlp_avg_average = 0;       //set global for information purpose
static int g_tlp_avg_history[MAX_CPU_RUSH_TLP_TIMES] = {0};

static int g_tlp_iowait_av = 0;
...
/*
 * cpu hotplug - forward reference
 */
//get chip version api
typedef enum {
...
static void dbs_freq_increase(struct cpufreq_policy *p, unsigned int freq)
{
	...
	if (hp_tuners->powersave_bias)
		freq = hp_ops.powersave_bias_target(p, freq,
		                                    CPUFREQ_RELATION_H);
	else if (p->cur == p->max) {
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
		if (dbs_ignore == 0) {
...
void hp_based_cpu_num(int num)
{
	...
	if (online_cpus_count < num && online_cpus_count < hp_tuners->cpu_num_limit) {
		...
		if (hp_wq == NULL)
			printk(KERN_EMERG "[power/hotplug] %s():%d, impossible\n", __func__, __LINE__);
		else
...
static void hp_work_handler(struct work_struct *work)
{
	...
	if (mutex_trylock(&hp_onoff_mutex)) {
		if (!hp_tuners->is_cpu_hotplug_disable) {
			...
			if (hp_tuners->gov_debug > 2)
				printk("[power/hotplug] hp_work_handler(%d)(%d)(%d)(%d)(%ld)(%ld)(%d)(%d) begin\n", g_trigger_hp_work, g_tlp_avg_average, g_tlp_avg_current,
			       	g_cpus_sum_load_current, g_cpu_up_sum_load, g_cpu_down_sum_load,hp_tuners->cpu_num_base, hp_tuners->cpu_num_limit);

			switch (g_trigger_hp_work) {
			case CPU_HOTPLUG_WORK_TYPE_RUSH:
				for (i = online_cpus_count; i < min(g_next_hp_action, hp_tuners->cpu_num_limit); ++i)
					cpu_up(i);

				break;

			case CPU_HOTPLUG_WORK_TYPE_BASE:
				for (i = online_cpus_count; i < min(hp_tuners->cpu_num_base, hp_tuners->cpu_num_limit); ++i)
					cpu_up(i);
			...
			default:
				for (i = online_cpus_count; i < min(hp_tuners->cpu_input_boost_num, hp_tuners->cpu_num_limit); ++i)
					cpu_up(i);
...
static void hp_check_cpu(int cpu, unsigned int load_freq)
{
	...
	//printk(KERN_EMERG "***** cpu: %d, load_freq: %u, smp_processor_id: %d *****\n", cpu, load_freq, smp_processor_id());
	...
	if (load_freq > hp_tuners->up_threshold * policy->cur) {
		...
		if (policy->cur < policy->max)
			dbs_info->rate_mult =
			    hp_tuners->sampling_down_factor;
		
		if (hp_tuners->gov_debug > 1)
			printk(KERN_EMERG "++hp_check_cpu: policy->cur = %d , policy->max = %d @@\n", policy->cur, policy->max);
	 ...
	 */
	if (load_freq < hp_tuners->adj_up_threshold
	    * policy->cur) {
		...
		if (hp_tuners->gov_debug > 1)
			printk(KERN_EMERG "--hp_check_cpu: policy->cur = %d , freq_next = %d , adj_up_threshold %d @@\n ", policy->cur, freq_next, hp_tuners->adj_up_threshold);
...
hp_check: {
		...
		g_tlp_avg_index = (g_tlp_avg_index + 1 == hp_tuners->cpu_rush_tlp_times) ? 0 : g_tlp_avg_index + 1;
		g_tlp_avg_count++;
		...
		if (hp_tuners->cpu_rush_boost_enable) {
			...
			if (g_cpus_sum_load_current > hp_tuners->cpu_rush_threshold * online_cpus_count)
				++g_cpu_rush_count;
			else
				g_cpu_rush_count = 0;
			...
			if ((g_cpu_rush_count >= hp_tuners->cpu_rush_avg_times) &&
				...
				if (hp_tuners->gov_debug > 1)
					printk(KERN_EMERG "### add cpu for RUSH: policy->cur = %d , policy->max = %d @@\n ", policy->cur, policy->max);
		...
		if (online_cpus_count < hp_tuners->cpu_num_base && online_cpus_count < hp_tuners->cpu_num_limit) {
			if (hp_tuners->gov_debug > 1)
				printk(KERN_EMERG "### add cpu for BASE: policy->cur = %d , policy->max = %d @@\n ", policy->cur, policy->max);
		...
		if (online_cpus_count > hp_tuners->cpu_num_limit) {
			if (hp_tuners->gov_debug > 1)
				printk(KERN_EMERG "### add cpu for LIMIT: policy->cur = %d , policy->max = %d @@\n ", policy->cur, policy->max);
		...
		if (online_cpus_count < num_possible_cpus()) {
			...
			g_cpu_up_count++;
			g_cpu_up_load_index = (g_cpu_up_load_index + 1 == hp_tuners->cpu_up_avg_times) ? 0 : g_cpu_up_load_index + 1;
			...
			if (g_cpu_up_count >= hp_tuners->cpu_up_avg_times) {
				...
				if (g_cpu_up_sum_load >
				    (hp_tuners->cpu_up_threshold * online_cpus_count  * hp_tuners->cpu_up_avg_times)) {
					if (online_cpus_count < hp_tuners->cpu_num_limit) {
						if (hp_tuners->gov_debug > 1)
							printk(KERN_EMERG "### add cpu for UP: policy->cur = %d , policy->max = %d @@\n ", policy->cur, policy->max);
		...
		if (online_cpus_count > 1) {
			...
			g_cpu_down_count++;
			g_cpu_down_load_index = (g_cpu_down_load_index + 1 == hp_tuners->cpu_down_avg_times) ? 0 : g_cpu_down_load_index + 1;
			...
			if (g_cpu_down_count >= hp_tuners->cpu_down_avg_times) {
				...
				cpu_down_threshold = ((hp_tuners->cpu_up_threshold - hp_tuners->cpu_down_differential) * hp_tuners->cpu_down_avg_times);
				...
				//printk("### g_next_hp_action: %d, tlp_cpu_num: %d, g_cpu_down_sum_load / hp_tuners->cpu_down_avg_times: %d ###\n", g_next_hp_action, tlp_cpu_num, g_cpu_down_sum_load / hp_tuners->cpu_down_avg_times);
			...
			printk("dbs_check_cpu: cpu_down_threshold = %d\n", ((hp_tuners->cpu_up_threshold - hp_tuners->cpu_down_differential) * (online_cpus_count - 1)));
#endif
		}
                                 ...
                                 unsigned int new_rate)
{
	...
	hp_tuners->sampling_rate = new_rate = max(new_rate,
...
///MEIZU_BSP{@
static ssize_t store_gov_debug(struct dbs_data *dbs_data, const char *buf,
                                   size_t count)
{
	struct hp_dbs_tuners *hp_tuners = dbs_data->tuners;
	unsigned int input;
                                ...
                                size_t count)
{
	...
	for_each_online_cpu(j) {
		...
		dbs_info->cdbs.prev_cpu_idle = get_cpu_idle_time(j,
		                               &dbs_info->cdbs.prev_cpu_wall, hp_tuners->io_is_busy);
	}
	return count;
}
...
static ssize_t store_cpu_down_differential(struct dbs_data *dbs_data, const char *buf, size_t count)
{
	...
	if (ret != 1
	    || input > MAX_CPU_DOWN_DIFFERENTIAL
                            ...
                            unsigned int code, int value)
{
	...
	if ((type == EV_KEY) && (code == BTN_TOUCH) && (value == 1) && (dbs_data->cdata->governor == GOV_HOTPLUG && hp_tuners->cpu_input_boost_enable)) {
		//if (!in_interrupt())
		//{
		unsigned int online_cpus_count = num_online_cpus();

		printk(KERN_DEBUG "@%s():%d, online_cpus_count = %d, cpu_input_boost_num = %d\n", __func__, __LINE__, online_cpus_count, hp_tuners->cpu_input_boost_num);
...
static int hp_cpufreq_governor_dbs(struct cpufreq_policy *policy,
                                   unsigned int event)
{
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
	struct dbs_data *dbs_data;
	...
	//printk(KERN_EMERG "***** policy->cpu: %d, event: %u, smp_processor_id: %d, have_governor_per_policy: %d *****\n", policy->cpu, event, smp_processor_id(), have_governor_per_policy());
	switch (event) {
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	///MEIZU_BSP{@
	freq_up_task = kthread_run(touch_freq_up_task, NULL, "touch_freq_up_task");
	///@}
