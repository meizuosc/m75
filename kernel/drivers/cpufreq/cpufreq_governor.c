...
void dbs_check_cpu(struct dbs_data *dbs_data, int cpu)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		//printk(KERN_EMERG "***** cpu: %d, load: %u, g_cpus_sum_load_current: %d, smp_processor_id: %d, num_online_cpus(): %d *****\n", j, load, g_cpus_sum_load_current, smp_processor_id(), num_online_cpus());
#endif
		// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
		...
		// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
#if defined(CONFIG_CPU_FREQ_GOV_HOTPLUG) && defined(DEBUG_LOG)
		printk(KERN_DEBUG "dbs_check_cpu: cpu = %d\n", j);
		printk(KERN_DEBUG "dbs_check_cpu: wall_time = %d, idle_time = %d, load = %d\n", wall_time, idle_time, load);
		printk(KERN_DEBUG "dbs_check_cpu: freq_avg = %d, max_load = %d, g_cpus_sum_load_current = %d\n", freq_avg, max_load, g_cpus_sum_load_current);
#endif
		// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	}

	if(hp_tuners->gov_debug)
		printk(KERN_EMERG "==dbs_check_cpu: policy->cur = %d,freq_avg = %d, max_load = %d, g_cpus_sum_load_current = %d @@\n", policy->cur, freq_avg, max_load, g_cpus_sum_load_current );
