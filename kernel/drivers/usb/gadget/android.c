		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	printk("%s [0]=%d,[1]=%d,[2]=%d,[3]=%d, num=%d\n", __func__, val[0], val[1], \
								val[2], val[3], num);
...
#define max(a, b) ((a) > (b) ? (a) : (b))
	...
	config->fsg.nluns = max(LUN_NUM,1);
				  ...
				  struct usb_composite_dev *cdev)
{
	...
	for (; (f = *functions++); index++) {
		...
		xlog_printk(ANDROID_LOG_INFO, USB_LOG, "%s: f->dev_name = %s, f->name = %s\n", __func__, f->dev_name, f->name);
		/* Added for USB Develpment debug, more log for more debuging help */
		...
		xlog_printk(ANDROID_LOG_INFO, USB_LOG, "%s: dev->dev->devt is 0x%x, f->dev->devt is 0x%x\n", __func__, dev->dev->devt, f->dev->devt);
		...
		if (f->init) {
			...
			if (err) {
				...
				xlog_printk(ANDROID_LOG_INFO, USB_LOG, "%s: init %s success!!\n", __func__, f->name);
			}
		}
			       ...
			       struct usb_configuration *c)
{
	...
	/* Added for USB Develpment debug, more log for more debuging help */
	xlog_printk(ANDROID_LOG_INFO, USB_LOG, "%s: ", __func__);
	/* Added for USB Develpment debug, more log for more debuging help */

	list_for_each_entry(f, &dev->enabled_functions, enabled_list) {
		xlog_printk(ANDROID_LOG_INFO, USB_LOG, "bind_config function '%s'/%p\n", f->name, f);
			       ...
			       struct usb_configuration *c)
{
	...
	list_for_each_entry(f, &dev->enabled_functions, enabled_list) {
		xlog_printk(ANDROID_LOG_INFO, USB_LOG, "unbind_config function '%s'/%p\n", f->name, f);
		if (f->unbind_config)
			f->unbind_config(f, c);
	}
}
...
static int android_enable_function(struct android_dev *dev, char *name)
{
	...
	while ((f = *functions++)) {
		...
		/* Added for USB Develpment debug, more log for more debuging help */
		xlog_printk(ANDROID_LOG_INFO, USB_LOG, "%s: name = %s, f->name=%s \n", __func__, name, f->name);
		/* Added for USB Develpment debug, more log for more debuging help */
			       ...
			       const char *buff, size_t size)
{
	...
	/* Added for USB Develpment debug, more log for more debuging help */
	xlog_printk(ANDROID_LOG_INFO, USB_LOG, "%s: \n", __func__);
	/* Added for USB Develpment debug, more log for more debuging help */
			    ...
			    const char *buff, size_t size)
{
	...
	/* Added for USB Develpment debug, more log for more debuging help */
	xlog_printk(ANDROID_LOG_INFO, USB_LOG, "%s: device_attr->attr.name: %s\n", __func__, attr->attr.name);
	/* Added for USB Develpment debug, more log for more debuging help */
	...
	if (enabled && !dev->enabled) {
		...
		list_for_each_entry(f, &dev->enabled_functions, enabled_list) {
			xlog_printk(ANDROID_LOG_INFO, USB_LOG, "enable function '%s'/%p\n", f->name, f);
			if (f->enable)
				f->enable(f);
		}
		android_enable(dev);
		...
		/* Added for USB Develpment debug, more log for more debuging help */
		xlog_printk(ANDROID_LOG_INFO, USB_LOG, "%s: enable 0->1 case, device_desc.idVendor = 0x%x, device_desc.idProduct = 0x%x\n", __func__, device_desc.idVendor, device_desc.idProduct);
		/* Added for USB Develpment debug, more log for more debuging help */

	} else if (!enabled && dev->enabled) {

		/* Added for USB Develpment debug, more log for more debuging help */
		xlog_printk(ANDROID_LOG_INFO, USB_LOG, "%s: enable 1->0 case, device_desc.idVendor = 0x%x, device_desc.idProduct = 0x%x\n", __func__, device_desc.idVendor, device_desc.idProduct);
		/* Added for USB Develpment debug, more log for more debuging help */

		android_disable(dev);
		list_for_each_entry(f, &dev->enabled_functions, enabled_list) {
			xlog_printk(ANDROID_LOG_INFO, USB_LOG, "disable function '%s'/%p\n", f->name, f);
			if (f->disable) {
				f->disable(f);
			}
		}
...
static int android_start(void)
{
	...
	xlog_printk(ANDROID_LOG_INFO, USB_LOG, "android_start ===> \n");
...
{
	xlog_printk(ANDROID_LOG_INFO, USB_LOG, "android_stop ===> \n");
...
static ssize_t andoid_usbif_proc_write(struct file *file, const char __user *buf, size_t length, loff_t *ppos)
{
	...
	if (length >= sizeof(msg)) {
		xlog_printk(ANDROID_LOG_INFO, USB_LOG, "andoid_usbif_proc_write length error, the error len is %d\n", length);
		return -EINVAL;
	}
	...
	xlog_printk(ANDROID_LOG_INFO, USB_LOG, "andoid_usbif_proc_write: %s, current driver on/off: %d\n", msg, andoid_usbif_driver_on);

	if ((msg[0] == '1') && (andoid_usbif_driver_on == 0)){
		xlog_printk(ANDROID_LOG_INFO, USB_LOG, "start usb android driver ===> \n");
		printk("start usb android driver ===> \n");
		android_start() ;
		andoid_usbif_driver_on = 1 ;
		xlog_printk(ANDROID_LOG_INFO, USB_LOG, "start usb android driver <=== \n");
	}else if ((msg[0] == '0') && (andoid_usbif_driver_on == 1)){
		xlog_printk(ANDROID_LOG_INFO, USB_LOG, "stop usb android driver ===> \n");
