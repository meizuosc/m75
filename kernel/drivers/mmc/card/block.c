...
///MEIZU_BSP{@
#define PROTECT_PROINFO_PART_NAME	"proinfo"
#define PROTECT_SYSTEM_PART_NAME		"system"
#define PROTECT_CUSTOM_PART_NAME		"custom"
static sector_t system_part_start = 0;
static sector_t system_part_count = 0;
static sector_t proinfo_part_start = 0;
static sector_t proinfo_part_count = 0;
static sector_t custom_part_start = 0;
static sector_t custom_part_count = 0;
///@}
...
#if defined(MTK_EMMC_SUPPORT) && !defined(MTK_GPT_SCHEME_SUPPORT)
	extern void emmc_create_sys_symlink (struct mmc_card *card);
#endif
///MEIZU_BSP{@
struct hd_struct *emmc_get_part (struct mmc_card *card, char *name)
{
...
///MEIZU_BSP{@
	part = emmc_get_part(card, PROTECT_PROINFO_PART_NAME);
	if(part){
		pr_info("%s: [pro_info] start sect = %lld, nr_sects = %lld\n", __func__, part->start_sect, part->nr_sects);
		proinfo_part_start = part->start_sect;
		proinfo_part_count = part->nr_sects;
	}
	part = emmc_get_part(card, PROTECT_SYSTEM_PART_NAME);
	if(part){
		pr_info("%s: [system] start sect = %lld, nr_sects = %lld\n", __func__, part->start_sect, part->nr_sects);
		system_part_start = part->start_sect;
		system_part_count = part->nr_sects;
	}
	part = emmc_get_part(card, PROTECT_CUSTOM_PART_NAME);
	if(part){
		pr_info("%s: [custom] start sect = %lld, nr_sects = %lld\n", __func__, part->start_sect, part->nr_sects);
		custom_part_start = part->start_sect;
		custom_part_count = part->nr_sects;
	}
///@}
	return 0;
