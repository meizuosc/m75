...
void machine_power_off(void)
{
	...
	if(tsk->real_parent)
	{
	 ...
	 {
	   printk(KERN_EMERG "machine_shutdown: start, Process (%s:%d). father %s:%d. grandfather %s:%d.\n",
	 ...
	 {
	   printk(KERN_EMERG "machine_shutdown: start, Process (%s:%d). father %s:%d.\n",
		tsk->comm, tsk->pid,tsk->real_parent->comm,tsk->real_parent->pid);
	 }
	}
	else
	{
	  printk(KERN_EMERG "machine_shutdown: start, Process (%s:%d)\n", tsk->comm, tsk->pid);
	}
...
static void show_data(unsigned long addr, int nbytes, const char *name)
{
	...
	for (i = 0; i < nlines; i++) {
		 ...
		 */
		printk(KERN_EMERG "%04lx ", (unsigned long)p & 0xffff);
		for (j = 0; j < 8; j++) {
			u32	data;
