...
recheck:
	...
	if(rt_policy(policy)){                                                                                
		if (!check_mt_allow_rt((struct sched_param *)param)){
			pr_debug("[RT_MONITOR]WARNNING [%d:%s] SET NOT MT ALLOW RT Prio [%d] for proc [%d:%s]\n", current->pid, current->comm, param->sched_priority, p->pid, p->comm);
			//dump_stack();
		}
	}
...
void sched_show_task(struct task_struct *p)
{
	...
	printk(KERN_NOTICE "%-15.15s %c", p->comm,
...
void __might_sleep(const char *file, int line, int preempt_offset)
{
    ...
    #ifdef CONFIG_PANIC_ON_ATOMIC_SLEEP
    panic("Please check this sleep in atomic warning! if it is okay, disable CONFIG_PANIC_ON_ATOMIC_SLEEP and ignore this warning!\n");
    #endif
}
