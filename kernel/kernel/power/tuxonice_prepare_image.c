...
#ifdef CONFIG_MTK_HIBERNATION
#define TOI_BUFFER_RESERVED ((16*1024*1024) >> (PAGE_SHIFT))
...
static int is_memory_low(unsigned long wanted)
{
	...
	for_each_populated_zone(zone) {
		...
		if (!strcmp(zone->name, "Normal")) {
			...
			if (free_pages < (min_pages + wanted + TOI_BUFFER_RESERVED)) {
				hib_warn("memory status: free(%lu) < wanted(%lu)+min(%lu), min/high:(%lu/%lu)\n",
						 free_pages, wanted, min_pages, min_pages, high_pages);
				HIB_SHOW_MEMINFO();
				return 1;
			} else
				return 0;
