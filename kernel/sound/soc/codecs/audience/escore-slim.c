...
static int escore_codec_cfg_slim_tx(struct escore_priv *escore, int dai_id)
{
	...
	rc = escore_cfg_slim_tx(escore->gen0_client,
			       escore->slim_dai_data[dai_id - 1].ch_num,
			       escore->slim_dai_data[dai_id - 1].ch_tot,
			       escore->slim_dai_data[dai_id - 1].rate);
...
int escore_remote_cfg_slim_rx(int dai_id)
{
	...
	if (escore->slim_dai_data[dai_id - 1].ch_tot != 0) {
		/* start slim channels associated with id */
		rc = escore_cfg_slim_rx(escore->gen0_client,
				       escore->slim_dai_data[dai_id - 1].ch_num,
				       escore->slim_dai_data[dai_id - 1].ch_tot,
		...
		be_id = escore->slim_be_id[dai_id - 1];
		escore->slim_dai_data[be_id - 1].ch_tot =
				escore->slim_dai_data[dai_id - 1].ch_tot;
		escore->slim_dai_data[be_id - 1].rate =
				escore->slim_dai_data[dai_id - 1].rate;
...
int escore_remote_close_slim_rx(int dai_id)
{
	...
	if (escore->slim_dai_data[dai_id - 1].ch_tot != 0) {
		...
		escore->slim_dai_data[dai_id - 1].ch_tot = 0;
	}
			 ...
			 struct snd_soc_dai *dai)
{
	...
	case 5:
	case 6:
		escore->slim_dai_data[id - 1].ch_tot = channels;
		break;
	default:
		dev_err(codec->dev,
	...
	case 32000:
	case 48000:
		escore->slim_dai_data[id - 1].rate = rate;
		break;
	default:
		dev_err(codec->dev,
...
void escore_slim_get_logical_addr(struct escore_priv *escore)
{
	...
	dev_dbg(&escore->intf_client->dev, "%s(): gen0_client LA = %d\n",
...
static int escore_dt_parse_slim_dev_info(struct slim_device *slim_ifd)
{
	struct property *prop;
	u8 *e_addr;
	struct device *dev = &slim_ifd->dev;
	dev_dbg(dev, "%s()\n", __func__);
...
static int escore_slim_probe(struct slim_device *sbdev)
{
	...
	if (escore_priv.intf_client->dev.of_node) {
		rc = escore_dt_parse_slim_dev_info(escore_priv.intf_client);
		if (rc < 0)
