 ...
 * Description:
 * ------------
 *   Audio AWB data1 uplink I17I18 to O5O6
 *
 * Author:
 * -------
 * Tina Tsai
...
static struct snd_dma_buffer *adc2_awb_dma_buf  = NULL;
 ...
 */
static void StartAudioadc2_awbHardware(struct snd_pcm_substream *substream);
static void StopAudioadc2_awbHardware(struct snd_pcm_substream *substream);
void StartAudioadc2_awbAnalogHardware(void);
void StopAudioadc2_awbAnalogHardware(void);
static int mtk_adc2_awb_probe(struct platform_device *pdev);
static int mtk_adc2_awb_pcm_close(struct snd_pcm_substream *substream);
static int mtk_asoc_adc2_awb_pcm_new(struct snd_soc_pcm_runtime *rtd);
static int mtk_afe_adc2_awb_probe(struct snd_soc_platform *platform);

static struct snd_pcm_hardware mtk_adc2_awb_hardware =
{
...
static void StopAudioadc2_awbHardware(struct snd_pcm_substream *substream)
{
    pr_debug("StopAudioadc2_awbHardware \n");
    ...
    SetConnection(Soc_Aud_InterCon_DisConnect, Soc_Aud_InterConnectionInput_I17, Soc_Aud_InterConnectionOutput_O05);
    SetConnection(Soc_Aud_InterCon_DisConnect, Soc_Aud_InterConnectionInput_I18, Soc_Aud_InterConnectionOutput_O06);
...
static void StartAudioadc2_awbHardware(struct snd_pcm_substream *substream)
{
    pr_debug("StartAudioadc2_awbHardware \n");

    Config2ndAdcI2S(substream);
    Set2ndI2SAdcIn(mAudioDigitalI2S);
    ...
    SetConnection(Soc_Aud_InterCon_Connection, Soc_Aud_InterConnectionInput_I17, Soc_Aud_InterConnectionOutput_O05);
    SetConnection(Soc_Aud_InterCon_Connection, Soc_Aud_InterConnectionInput_I18, Soc_Aud_InterConnectionOutput_O06);

    EnableAfe(true);

}

static int mtk_adc2_awb_pcm_prepare(struct snd_pcm_substream *substream)
{
    //printk("mtk_adc2_awb_pcm_prepare substream->rate = %d  substream->channels = %d \n", substream->runtime->rate, substream->runtime->channels);
    return 0;
}

static int mtk_adc2_awb_alsa_stop(struct snd_pcm_substream *substream)
{
    AFE_BLOCK_T *Awb_Block = &(AWB_Control_context->rBlock);
    pr_debug("mtk_adc2_awb_alsa_stop \n");
    StopAudioadc2_awbHardware(substream);
    RemoveMemifSubStream(Soc_Aud_Digital_Block_MEM_AWB,substream);
    return 0;
}

static kal_int32 Previous_Hw_cur = 0;
static snd_pcm_uframes_t mtk_adc2_awb_pcm_pointer(struct snd_pcm_substream *substream)
{
    kal_int32 HW_memory_index = 0;
    kal_int32 HW_Cur_ReadIdx = 0;
    kal_uint32 Frameidx = 0;
    AFE_BLOCK_T *Awb_Block = &(AWB_Control_context->rBlock);
    PRINTK_AUD_AWB("mtk_adc2_awb_pcm_pointer Awb_Block->u4WriteIdx;= 0x%x \n", Awb_Block->u4WriteIdx);
...
static int mtk_adc2_awb_pcm_hw_params(struct snd_pcm_substream *substream,
                                      struct snd_pcm_hw_params *hw_params)
{
    struct snd_pcm_runtime *runtime = substream->runtime;
    struct snd_dma_buffer *dma_buf = &substream->dma_buffer;
    int ret = 0;
    pr_debug("mtk_adc2_awb_pcm_hw_params \n");

    dma_buf->dev.type = SNDRV_DMA_TYPE_DEV;
    dma_buf->dev.dev = substream->pcm->card->dev;
    dma_buf->private_data = NULL;
    pr_debug("adc2_awb_dma_buf = %p adc2_awb_dma_buf->area = %p\n", adc2_awb_dma_buf, adc2_awb_dma_buf->area);

    if (adc2_awb_dma_buf->area)
    {
        pr_debug("mtk_adc2_awb_pcm_hw_params adc2_awb_dma_buf->area\n");
        runtime->dma_bytes = adc2_awb_dma_buf->bytes;
        runtime->dma_area = adc2_awb_dma_buf->area;
        runtime->dma_addr = adc2_awb_dma_buf->addr;
        runtime->buffer_size = adc2_awb_dma_buf->bytes;
    }
    else
    {
        pr_debug("mtk_adc2_awb_pcm_hw_params snd_pcm_lib_malloc_pages\n");
        ret =  snd_pcm_lib_malloc_pages(substream, params_buffer_bytes(hw_params));
    }
    pr_debug("mtk_adc2_awb_pcm_hw_params dma_bytes = %d dma_area = %p dma_addr = 0x%x\n",
...
static int mtk_adc2_awb_pcm_hw_free(struct snd_pcm_substream *substream)
{
    pr_debug("mtk_adc2_awb_pcm_hw_free \n");
    if (adc2_awb_dma_buf->area)
    {
        return 0;
    }
...
static int mtk_adc2_awb_pcm_open(struct snd_pcm_substream *substream)
{
    struct snd_pcm_runtime *runtime = substream->runtime;
    int ret = 0;
    AudDrv_Clk_On();
    ...
    runtime->hw = mtk_adc2_awb_hardware;
    memcpy((void *)(&(runtime->hw)), (void *)&mtk_adc2_awb_hardware , sizeof(struct snd_pcm_hardware));
    ...
    pr_debug("mtk_adc2_awb_pcm_open runtime rate = %d channels = %d \n", runtime->rate, runtime->channels);
    if (substream->stream == SNDRV_PCM_STREAM_CAPTURE)
    {
        pr_debug("SNDRV_PCM_STREAM_CAPTURE mtkalsa_adc2_awb_constraints\n");
    }
    else
    {
    ...
    if (ret < 0)
    {
        pr_debug("mtk_adc2_awb_pcm_close\n");
        mtk_adc2_awb_pcm_close(substream);
        return ret;
    }
    pr_debug("mtk_adc2_awb_pcm_open return\n");
    return 0;
}
...
static int mtk_adc2_awb_alsa_start(struct snd_pcm_substream *substream)
{
    pr_debug("mtk_adc2_awb_alsa_start \n");
    SetMemifSubStream(Soc_Aud_Digital_Block_MEM_AWB, substream);
    StartAudioadc2_awbHardware(substream);
    return 0;
}

static int mtk_adc2_awb_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
    pr_debug("mtk_adc2_awb_pcm_trigger cmd = %d\n", cmd);

    switch (cmd)
    {
        case SNDRV_PCM_TRIGGER_START:
        case SNDRV_PCM_TRIGGER_RESUME:
            return mtk_adc2_awb_alsa_start(substream);
        case SNDRV_PCM_TRIGGER_STOP:
        case SNDRV_PCM_TRIGGER_SUSPEND:
            return mtk_adc2_awb_alsa_stop(substream);
    }
    return -EINVAL;
}
...
static int mtk_adc2_awb_pcm_copy(struct snd_pcm_substream *substream,
                                 ...
                                 void __user *dst, snd_pcm_uframes_t count)
{
    ...
    PRINTK_AUD_AWB("mtk_adc2_awb_pcm_copy pos = %lucount = %lu \n ", pos, count);
    ...
    {
        msleep(50);
        pr_debug("Awb_Block->u4BufferSize <= 0  =%d\n", Awb_Block->u4BufferSize);
        return 0;
    }
    ...
    {
        PRINTK_AUD_AWB("AudDrv_MEMIF_Read u4DataRemained=%x > u4BufferSize=%x" , Awb_Block->u4DataRemained, Awb_Block->u4BufferSize);
...
static int mtk_adc2_awb_pcm_silence(struct snd_pcm_substream *substream,
                                    int channel, snd_pcm_uframes_t pos,
...
static struct page *mtk_adc2_awb_pcm_page(struct snd_pcm_substream *substream,
...
static struct snd_pcm_ops mtk_afe_adc2_awb_ops =
{
    .open =     mtk_adc2_awb_pcm_open,
    .close =    mtk_adc2_awb_pcm_close,
    .ioctl =    snd_pcm_lib_ioctl,
    .hw_params =    mtk_adc2_awb_pcm_hw_params,
    .hw_free =  mtk_adc2_awb_pcm_hw_free,
    .prepare =  mtk_adc2_awb_pcm_prepare,
    .trigger =  mtk_adc2_awb_pcm_trigger,
    .pointer =  mtk_adc2_awb_pcm_pointer,
    .copy =     mtk_adc2_awb_pcm_copy,
    .silence =  mtk_adc2_awb_pcm_silence,
    .page =     mtk_adc2_awb_pcm_page,
};

static struct snd_soc_platform_driver mtk_soc_platform =
{
    .ops        = &mtk_afe_adc2_awb_ops,
    .pcm_new    = mtk_asoc_adc2_awb_pcm_new,
    .probe      = mtk_afe_adc2_awb_probe,
};

static int mtk_adc2_awb_probe(struct platform_device *pdev)
{
    pr_debug("mtk_adc2_awb_probe\n");
    if (pdev->dev.of_node)
    {
        dev_set_name(&pdev->dev, "%s", MT_SOC_ADC2_AWB_PCM);
    }
...
static u64 adc2_awb_pcm_dmamask = DMA_BIT_MASK(32);
...
static int mtk_asoc_adc2_awb_pcm_new(struct snd_soc_pcm_runtime *rtd)
{
    ...
    {
        card->dev->dma_mask = &adc2_awb_pcm_dmamask;
    }
    if (!card->dev->coherent_dma_mask)
    {
        card->dev->coherent_dma_mask = DMA_BIT_MASK(32);
    }
    pr_debug("mtk_asoc_adc2_awb_pcm_new \n");
    return ret;
}
...
static int mtk_afe_adc2_awb_probe(struct snd_soc_platform *platform)
{
    pr_debug("mtk_afe_adc2_awb_probe\n");
    AudDrv_Allocate_mem_Buffer(Soc_Aud_Digital_Block_MEM_AWB, UL1_MAX_BUFFER_SIZE);
    adc2_awb_dma_buf =  Get_Mem_Buffer(Soc_Aud_Digital_Block_MEM_AWB);
    mAudioDigitalI2S =  kzalloc(sizeof(AudioDigtalI2S), GFP_KERNEL);
    return 0;
}


static int mtk_adc2_awb_remove(struct platform_device *pdev)
{
    pr_debug("%s\n", __func__);
...
static struct platform_driver mtk_afe_adc2_awb_driver =
{
    .driver = {
        .name = MT_SOC_ADC2_AWB_PCM,
        .owner = THIS_MODULE,
    },
    .probe = mtk_adc2_awb_probe,
    .remove = mtk_adc2_awb_remove,
};

static struct platform_device *soc_mtkafe_adc2_awb_dev;

static int __init mtk_soc_adc2_awb_platform_init(void)
{
    int ret = 0;
    pr_debug("%s\n", __func__);
    soc_mtkafe_adc2_awb_dev = platform_device_alloc(MT_SOC_ADC2_AWB_PCM, -1);
    if (!soc_mtkafe_adc2_awb_dev)
    {
        return -ENOMEM;
    }

    ret = platform_device_add(soc_mtkafe_adc2_awb_dev);
    if (ret != 0)
    {
        platform_device_put(soc_mtkafe_adc2_awb_dev);
        return ret;
    }

    ret = platform_driver_register(&mtk_afe_adc2_awb_driver);
    return ret;
}
module_init(mtk_soc_adc2_awb_platform_init);

static void __exit mtk_soc_platform_exit(void)
{

    pr_debug("%s\n", __func__);
    platform_driver_unregister(&mtk_afe_adc2_awb_driver);
}
