                                    ...
                                    struct snd_pcm_hw_params *hw_params)
{
    ...
    pr_debug("mtk_bt_dai_pcm_hw_params dma_bytes = %d dma_area = %p dma_addr = 0x%x\n",
                               ...
                               void __user *dst, snd_pcm_uframes_t count)
{
    ...
    if (DMA_Read_Ptr + read_size < Dai_Block->u4BufferSize)
    {
        ...
        {
            pr_debug("mtk_bt_dai_pcm_copy 1, read_size:0x%x, DataRemained:0x%x, DMA_Read_Ptr:0x%x, DMAReadIdx:0x%x \r\n",
    ...
    else
    {
        ...
        if (DMA_Read_Ptr != Dai_Block->u4DMAReadIdx)
        {
            ...
            pr_debug("mtk_bt_dai_pcm_copy 2, read_size1:0x%x, DataRemained:0x%x, DMA_Read_Ptr:0x%x, DMAReadIdx:0x%x \r\n",
        ...
        if (DMA_Read_Ptr != Dai_Block->u4DMAReadIdx)
        {
            ...
            pr_debug("mtk_bt_dai_pcm_copy 3, read_size2:%x, DataRemained:%x, DMA_Read_Ptr:0x%x, DMAReadIdx:%x \r\n",
