...
{
     AudDrv_Allocate_mem_Buffer(Soc_Aud_Digital_Block_MEM_DL1,	(24*1024));
...
void OpenAfeDigitaldl1(bool bEnable)
{
    if (bEnable == true)
    {
        ...
        volatile unsigned int *Sramdata = Dl1_Playback_dma_buf->area;
...
static bool APLL1State = false;
void EnableApll1(bool bEnable)
{
    pr_debug("+%s bEnable = %d\n",__func__,bEnable);
    if ((bEnable== true) && (APLL1State == false))
    {
    ...
    else if((bEnable == false) && (APLL1State == true))
    {
...
static bool APLL2State = false;
void EnableApll2(bool bEnable)
{
    pr_debug("+%s bEnable = %d\n",__func__,bEnable);
    if ((bEnable== true) && (APLL2State == false))
    {
...
bool Set2ndI2SAdcIn(AudioDigtalI2S *DigtalI2S)
{
    ...
    else
    {
        ...
        pr_debug("%s Audio_I2S_Adc = 0x%x", __FUNCTION__, Audio_I2S_Adc);
...
bool SetModemPcmConfig(int modem_index , AudioDigitalPCM p_modem_pcm_attribute)
{
    ...
    else if (modem_index == MODEM_2 || modem_index == MODEM_EXTERNAL)    // MODEM_2 use PCM_INTF_CON1 (0x530) !!!
    {
        ...
        pr_debug("%s(), PCM_INTF_CON1(0x%x) = 0x%x", __FUNCTION__, PCM_INTF_CON, reg_pcm_intf_con1);
        ...
        case Soc_Aud_Digital_Block_MEM_HDMI:
        {
            ...
            if (Audio_dma_buf[MemBlock]  != NULL)
            {
                ...
                if (Audio_dma_buf[MemBlock] ->area == NULL)
                {
                    ...
                    Audio_dma_buf[MemBlock] ->area = dma_alloc_coherent(0, Buffer_length,
