                        ...
                        void __user *dst, snd_pcm_uframes_t count)
{
    ...
    if (copy_size != 0)
    {
        ...
        if (Afe_WriteIdx_tmp + copy_size < Afe_Block->u4BufferSize) // copy once
        {
            ...
            {
                PRINTK_AUDDRV("AudDrv_write 0ptr invalid data_w_ptr=0x%x, size=%d", (kal_uint32)data_w_ptr, copy_size);
        ...
        else  // copy twice
        {
            ...
            {
                printk("AudDrv_write 1ptr invalid data_w_ptr=%x, size_1=%d", (kal_uint32)data_w_ptr, size_1);
