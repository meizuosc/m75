                                     ...
                                     struct snd_pcm_hw_params *hw_params)
{
    ...
    pr_debug("mtk_capture2_pcm_hw_params dma_bytes = %d dma_area = %p dma_addr = 0x%x\n",
                                ...
                                void __user *dst, snd_pcm_uframes_t count)
{
    ...
    PRINTK_AUD_UL2("mtk_capture2_pcm_copy finish0, read_count:%x, read_size:%x, u4DataRemained:%x, u4DMAReadIdx:0x%x, u4WriteIdx:%x \r\n",
    ...
    if (DMA_Read_Ptr + read_size < Vul_Block->u4BufferSize)
    {
        ...
        {
            pr_debug("mtk_capture2_pcm_copy 1, read_size:%x, DataRemained:%x, DMA_Read_Ptr:0x%x, DMAReadIdx:%x \r\n",
        ...
        PRINTK_AUD_UL2("mtk_capture2_pcm_copy finish1, copy size:%x, u4DMAReadIdx:0x%x, u4WriteIdx:%x, u4DataRemained:%x \r\n",
    ...
    else
    {
        ...
        if (DMA_Read_Ptr != Vul_Block->u4DMAReadIdx)
        {
            ...
            pr_debug("mtk_capture2_pcm_copy 2, read_size1:%x, DataRemained:%x, DMA_Read_Ptr:0x%x, DMAReadIdx:%x \r\n",
        ...
        if (DMA_Read_Ptr != Vul_Block->u4DMAReadIdx)
        {
            ...
            pr_debug("mtk_capture2_pcm_copy 3, read_size2:%x, DataRemained:%x, DMA_Read_Ptr:0x%x, DMAReadIdx:%x \r\n",
