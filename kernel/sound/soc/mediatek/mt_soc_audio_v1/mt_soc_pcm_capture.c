                                     ...
                                     struct snd_pcm_hw_params *hw_params)
{
    ...
    if (mCaptureUseSram == true)
    {
        ...
        pr_debug("mtk_capture_pcm_hw_params mCaptureUseSram dma_bytes = %d \n", runtime->dma_bytes);
        substream->runtime->dma_area = (unsigned char *)Get_Afe_SramBase_Pointer();
        substream->runtime->dma_addr = Get_Afe_Sram_Phys_Addr();
    }
    else if (Capture_dma_buf->area)
    {
        pr_debug("Capture_dma_buf = %p Capture_dma_buf->area = %p apture_dma_buf->addr = 0x%x\n",
            Capture_dma_buf, Capture_dma_buf->area, Capture_dma_buf->addr);
        runtime->dma_bytes = params_buffer_bytes(hw_params);
