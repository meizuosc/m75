...
//        runtime->dma_bytes = params_buffer_bytes(hw_params);
    ...
    pr_debug("mtk_capture_tdm_hw_params dma_bytes = %d dma_area = %p dma_addr = 0x%x\n",
...
static int mtk_capture_pcm_open(struct snd_pcm_substream *substream)
{
    ...
    {
        pr_debug("SNDRV_tdm_STREAM_CAPTURE mtkalsa_capture_constraints\n");
    }
    else
    {
...
static int mtk_afe_capture_probe(struct snd_soc_platform *platform)
{
//    AudDrv_Allocate_mem_Buffer(Soc_Aud_Digital_Block_MEM_VUL, ULTDM_MAX_BUFFER_SIZE);
