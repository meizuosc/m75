...
//        runtime->buffer_size = HDMI_dma_buf->bytes;
    ...
    PRINTK_AUD_HDMI("2 dma_bytes = %d dma_area = %p dma_addr = 0x%x\n",
...
static int mtk_pcm_hdmi_start(struct snd_pcm_substream *substream)
{
    ...
    pr_debug("[mtk_pcm_hdmi_start] AFE_IRQ_MCU_STATUS =0x%x AFE_IRQ_MCU_EN= 0x%x, AFE_IRQ_CNT5=0x%x, AFE_IRQ_DEBUG =0x%x\n", u4RegValue, u4tmpValue, u4tmpValue1, u4tmpValue2);
                             ...
                             void __user *dst, snd_pcm_uframes_t count)
{
    ...
    if (copy_size != 0)
    {
        ...
        if (Afe_WriteIdx_tmp + copy_size < Afe_Block->u4BufferSize) // copy once
        {
            ...
            {
                PRINTK_AUD_HDMI("[mtk_pcm_hdmi_copy] 0ptr invalid data_w_ptr=%x, size=%d", (kal_uint32)data_w_ptr, copy_size);
        ...
        else  // copy twice
        {
            ...
            {
                PRINTK_AUD_HDMI("HDMI_write 1ptr invalid data_w_ptr=%x, size_1=%d", (kal_uint32)data_w_ptr, size_1);
            ...
            {
                PRINTK_AUD_HDMI("HDMI_write 2ptr invalid data_w_ptr=%x, size_1=%d, size_2=%d", (kal_uint32)data_w_ptr, size_1, size_2);
                PRINTK_AUD_HDMI("HDMI_write u4BufferSize=%d, u4DataRemained=%d", Afe_Block->u4BufferSize, Afe_Block->u4DataRemained);
            }
            else
            {
...
static int mtk_afe_hdmi_probe(struct snd_soc_platform *platform)
{
    ...
    PRINTK_AUD_HDMI("mtk_afe_hdmi_probe dma_alloc_coherent HDMI_dma_buf->addr=%x \n", HDMI_dma_buf->addr);
