                                     ...
                                     struct snd_pcm_hw_params *hw_params)
{
    ...
    pr_debug("mtk_mgrrx_awb_pcm_hw_params dma_bytes = %d dma_area = %p dma_addr = 0x%x\n",
                                ...
                                void __user *dst, snd_pcm_uframes_t count)
{
    ...
    {
        pr_debug("AudDrv_MEMIF_Read u4DataRemained=%x > u4BufferSize=%x" , Awb_Block->u4DataRemained, Awb_Block->u4BufferSize);
    ...
    if (DMA_Read_Ptr + read_size <= Awb_Block->u4BufferSize)
    {
        ...
        {
            pr_debug("AudDrv_MEMIF_Read 1, read_size:0x%x, DataRemained:0x%x, DMA_Read_Ptr:0x%x, DMAReadIdx:0x%x \r\n",
    ...
    else
    {
        ...
        if (DMA_Read_Ptr != Awb_Block->u4DMAReadIdx)
        {
            pr_debug("AudDrv_MEMIF_Read 2, read_size1:0x%x, DataRemained:0x%x, DMA_Read_Ptr:0x%x, DMAReadIdx:0x%x \r\n",
...
static int mtk_afe_mrgrx_awb_probe(struct snd_soc_platform *platform)
{
    ...
    AudDrv_Allocate_mem_Buffer(Soc_Aud_Digital_Block_MEM_AWB, MRGRX_MAX_BUFFER_SIZE);
    ...
    if (Mrgrx_Awb_Capture_dma_buf == NULL)
    {
        ...
        if(Mrgrx_Awb_Capture_dma_buf != NULL)
        {
            Mrgrx_Awb_Capture_dma_buf->area = dma_alloc_coherent(0, MRGRX_MAX_BUFFER_SIZE, &Mrgrx_Awb_Capture_dma_buf->addr, GFP_KERNEL);
        }
